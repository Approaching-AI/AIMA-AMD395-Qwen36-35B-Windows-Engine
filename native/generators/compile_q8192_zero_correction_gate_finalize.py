#!/usr/bin/env python3
"""Compile the zero-correction routed gate/up finalizer for gfx1151.

The native WMMA route leaves gate and up projections in FP32 scratch.  When
no Hawkeye projection replay is requested, this kernel performs only the
model-visible BF16 endpoints and SiLU multiplication.  It deliberately does
not read the K2048 input or expert weights.
"""
...
def compile_kernel(output_dir: Path, metadata_path: Path) -> None:
    ...
    metadata = {
        ...
        "arithmetic": (
            "BF16 gate/up endpoints followed by SiLU multiplication; "
            "no K2048 correction scan"
        ),
        "shape": {
