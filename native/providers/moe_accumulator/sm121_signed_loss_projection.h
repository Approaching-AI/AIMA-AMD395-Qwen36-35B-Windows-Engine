...
__global__ __launch_bounds__(256) void produce(const uint16_t* weights,const uint16_t* inputs,
    ...
    shared::load<Rows>(weights,inputs,valid,first_row,first_token,width,0u,words);
    shared::publish<Rows>(tile,words);__syncthreads();
    const unsigned wr=wave%(Rows/16u)*16u+source,query_base=wave/(Rows/16u)*16u;
    const unsigned row=first_row+wr;
    F8 center[2]{},lower[2]{},upper[2]{};
    for(unsigned coarse=0;coarse<width;coarse+=64u) {
        const bool another=coarse+64u<width;
        if constexpr(Prefetch)if(another)
            shared::load<Rows>(weights,inputs,valid,first_row,first_token,width,coarse+64u,words);
...
#pragma unroll
        ...
        if(another) {
            ...
            __syncthreads();
            if constexpr(!Prefetch)shared::load<Rows>(weights,inputs,valid,first_row,first_token,width,coarse+64u,words);
            shared::publish<Rows>(tile,words);__syncthreads();
        }
    }
    if(row>=rows)return;
