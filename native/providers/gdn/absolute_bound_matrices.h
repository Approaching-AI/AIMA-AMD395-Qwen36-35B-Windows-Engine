...
// A conservative absolute bound can certify both rounded residual consumers.
// Compact rejected cells into a shared queue so exact replay uses full waves.
// The original K64 state update and every FP32 state value remain unchanged.
namespace qrt_fla_absolute_bound {
...
__global__ void state_kernel(const uint16_t* k,const uint16_t* u,const uint16_t* w,
    ...
    __shared__ unsigned replay[64u*Columns],replay_count;
    ...
    for (unsigned offset=0u;offset<count;offset+=64u) {
        ...
        if(tid<64u){
            norm::Row row;
            for(unsigned pair=0u;pair<64u;++pair){
                const uint32_t word=operands.weights[tid][pair];
                norm::include(row,uint16_t(word));norm::include(row,uint16_t(word>>16u));
            }
        ...
        if(tid<Columns){
            norm::Row row;
            for(unsigned pair=0u;pair<64u;++pair){
                const uint32_t word=rounded[pair][tid];
                norm::include(row,uint16_t(word));norm::include(row,uint16_t(word>>16u));
            }
        ...
        for(unsigned cell=tid;cell<64u*Columns;cell+=threads){
            ...
            if(row<valid){
                ...
                uint16_t updated,scaled;
                if(consumer::residual(norm::enclose(weight_norm[row],state_norm[column]),
                        from_bf16(u[index]),decay[row],&updated,&scaled)){
                    v_new[index]=updated;residual_words[row][column]=scaled;
                }else replay[atomicAdd(&replay_count,1u)]=cell;
            }else residual_words[row][column]=0u;
        }
        __syncthreads();
        for(unsigned item=tid;item<replay_count;item+=threads){
            const unsigned cell=replay[item],row=cell/Columns,column=cell%Columns;
            ...
            residual_words[row][column]=to_bf16(difference*decay[row]);
        }
        __syncthreads();
...
// Compute the unchanged local S/V dot before reusing its shared arenas for
// Q/H. A shared local result follows each cell when Q/H replay is compacted.
__global__ void output_kernel(const uint16_t* q,const uint16_t* v,const uint16_t* h,
    ...
    __shared__ norm::Norm query_norm[64],state_norm[columns];
    __shared__ float local[64][columns],decay[64];
    __shared__ unsigned replay[64u*columns],replay_count;
    ...
    for(unsigned cell=tid;cell<valid*columns;cell+=threads){
        const unsigned row=cell/columns,column=cell%columns;
        local[row][column]=dot<64u>(left.scores[row],right.values,column,score_ok[row]&&v_ok[column]);
    }
    __syncthreads();
    for(unsigned cell=tid;cell<64u*64u;cell+=threads){
    ...
    if(tid<64u){
        norm::Row row;
        for(unsigned pair=0u;pair<64u;++pair){
            const uint32_t word=left.queries[tid][pair];
            norm::include(row,uint16_t(word));norm::include(row,uint16_t(word>>16u));
        }
        query_norm[tid]=norm::finish(row);
    }
    if(tid<columns){
        norm::Row row;
        for(unsigned pair=0u;pair<64u;++pair){
            const uint32_t word=right.checkpoint[pair][tid];
            norm::include(row,uint16_t(word));norm::include(row,uint16_t(word>>16u));
        }
        state_norm[tid]=norm::finish(row);
    }
    ...
    for(unsigned cell=tid;cell<valid*columns;cell+=threads){
        const unsigned row=cell/columns,column=cell%columns;
        const float value=local[row][column];uint16_t result;
        if(consumer::output(norm::enclose(query_norm[row],state_norm[column]),
                {value,value},decay[row],&result)){
    ...
    for(unsigned item=tid;item<replay_count;item+=threads){
        const unsigned cell=replay[item],row=cell/columns,column=cell%columns;
        const float old=dot<128u>(left.queries[row],right.checkpoint,column,q_ok[row]&&h_ok[column]);
        constexpr float scale=0.08838834764831845f;
        const float prior=old*decay[row];
        output[(size_t(offset+row)*32u+head)*128u+first_column+column]=
            from_bf16(to_bf16(fmaf(local[row][column],scale,prior*scale)));
    }
}
} // namespace qrt_fla_absolute_bound
