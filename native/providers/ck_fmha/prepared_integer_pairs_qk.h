#ifndef QRT_PREPARED_INTEGER_PAIRS_QK_H
#define QRT_PREPARED_INTEGER_PAIRS_QK_H
#include "blackwell_attention.h"
#include "../moe_accumulator/sm121_prepared_integer_pairs.h"

// Component experiment only: two original BF16 products per packed integer
// multiply. Cooperative windows reuse a lossless nine-dword K16 operand row.
namespace qrt_prepared_integer_pairs_qk {
using namespace qrt_blackwell_attention;
using Row = qrt_sm121_prepared_integer_pairs::Row;
using qrt_sm121_prepared_integer_pairs::prepare;
...
#pragma unroll 1
            for(unsigned group=0u;group<groups;++group)
                carry=qrt_sm121_prepared_integer_pairs::accumulate(carry,left[group][row],right[group][column]);
        }
        __syncthreads();
    }
