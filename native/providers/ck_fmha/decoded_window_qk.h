...
#pragma unroll
        for (unsigned i=0u;i<16u;++i)
            products[i]=qrt_sm121_group16::pack_product(qrt_q1_moe_hawkeye::multiply_bf16(
                query[base+i],key[size_t(base+i)*key_stride],kBlackwellZeroExponent));
    ...
    return qrt_q1_moe_hawkeye::value_to_float(qrt_sm121_group16::finish_accumulator(carry))*kExactScale;
}
...
__global__ void scores(const uint16_t* query,const uint16_t* transposed_key,float* output,
    ...
    static_assert(QueryRows*KeyColumns==kThreads);
    ...
    for (unsigned window=0u;window<kHeadDim;window+=Window) {
        ...
        if (invalid_tile) {
            if (live) output[output_cell]=active ? raw_dot(
                ...
                transposed_key+size_t(kv_head)*kHeadDim*key_stride+key,key_stride) : -INFINITY;
            return;
        }
        if (active && !fallback) {
