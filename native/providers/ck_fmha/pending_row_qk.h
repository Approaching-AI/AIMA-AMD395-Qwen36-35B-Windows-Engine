...
// Isolated fixed-grid fallback for unresolved query/head rows. Already exact
// cells and certified rows remain untouched. Original 2x2 K16 arithmetic and
// the complete original-dot fallback scan are preserved.
namespace qrt_pending_row_qk {
...
__global__ void scores(const uint32_t* packed_query, const uint32_t* packed_key,
    ...
    float* errors,const unsigned* pending,
...
#pragma unroll
        for (unsigned k = 0u; k < KeyCells; ++k) {
            ...
            active[q][k] = row < count && key < stride && key <= start + row &&
                pending[row*16u+head] && errors[(size_t(row)*16u+head)*stride+key]!=0.0f;
