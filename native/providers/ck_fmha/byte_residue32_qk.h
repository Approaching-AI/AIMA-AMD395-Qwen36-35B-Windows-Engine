#ifndef QRT_BYTE_RESIDUE32_QK_H
#define QRT_BYTE_RESIDUE32_QK_H
#include "blackwell_attention.h"
#include "../moe_accumulator/sm121_byte_residue32_core.h"
namespace qrt_byte_residue32_qk {
using namespace qrt_blackwell_attention;
using Row = qrt_sm121_byte_residue32::Row;
using qrt_sm121_byte_residue32::prepare;
...
__global__ void scores(const Row* query, const Row* key, float* output,
    ...
    if (key_tile <= last_query) for (unsigned group = 0u; group < 16u; ++group) {
        ...
        __syncthreads();
        if (wave < Columns / 16u) {
            const auto result = qrt_sm121_byte_residue32::products(left[lane % 16u], right[wave * 16u + lane % 16u]);
#pragma unroll
            for (unsigned element = 0u; element < 8u; ++element)
                {
                const unsigned cell=(2u*element+lane/16u)*Columns+wave*16u+lane%16u;
                partials[cell]=result.approximate[element];residues[cell]=result.residue[element];
            }
        }
        __syncthreads();
#pragma unroll
        for (unsigned item = 0u; item < per_thread; ++item) {
            ...
            if (output_row < query_count && key_row < stride && key_row <= query_start + output_row) {
                ...
                if (!qrt_sm121_byte_residue32::recover(partials[cell],uint32_t(residues[cell]),&mathematical) ||
                    !qrt_sm121_byte_residue32::sum(accumulators[item], left[row], right[column], mathematical, &sum))
                    sum = qrt_sm121_byte_residue32::fallback(accumulators[item], left[row], right[column]);
