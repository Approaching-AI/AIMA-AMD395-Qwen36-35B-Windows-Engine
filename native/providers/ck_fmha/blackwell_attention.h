...
struct TiledQueryPair { float first, second; };

template<bool Prepared>
__device__ __forceinline__ TiledQueryPair blackwell_paired_query_qk_dot(
    const uint32_t (&queries)[kPairedTiledQueries][kHeadDim / 2u],
    const uint32_t (&keys)[kHeadDim / 2u][kTiledExactKeys], unsigned row, unsigned key) {
    qrt_q1_moe_hawkeye::Value first{0u, kBlackwellZeroExponent, false}, second = first;
#pragma unroll 1
    for (unsigned base = 0u; base < kHeadDim; base += kBlackwellMmaGroup) {
        uint32_t first_products[kBlackwellMmaGroup], second_products[kBlackwellMmaGroup];
#pragma unroll
        for (unsigned item = 0u; item < kBlackwellMmaGroup; item += 2u) {
            const unsigned pair = (base + item) / 2u;
            const uint32_t q0 = queries[row][pair], q1 = queries[row + kTiledExactQueries][pair];
            const uint32_t k = keys[pair][key];
            if constexpr (Prepared) {
                first_products[item] = qrt_sm121_prepared_bf16::multiply(uint16_t(q0), uint16_t(k));
                first_products[item + 1u] = qrt_sm121_prepared_bf16::multiply(uint16_t(q0 >> 16u), uint16_t(k >> 16u));
                second_products[item] = qrt_sm121_prepared_bf16::multiply(uint16_t(q1), uint16_t(k));
                second_products[item + 1u] = qrt_sm121_prepared_bf16::multiply(uint16_t(q1 >> 16u), uint16_t(k >> 16u));
            } else {
                first_products[item] = qrt_sm121_group16::pack_product(qrt_q1_moe_hawkeye::multiply_bf16(
                    uint16_t(q0), uint16_t(k), kBlackwellZeroExponent));
                first_products[item + 1u] = qrt_sm121_group16::pack_product(qrt_q1_moe_hawkeye::multiply_bf16(
                    uint16_t(q0 >> 16u), uint16_t(k >> 16u), kBlackwellZeroExponent));
                second_products[item] = qrt_sm121_group16::pack_product(qrt_q1_moe_hawkeye::multiply_bf16(
                    uint16_t(q1), uint16_t(k), kBlackwellZeroExponent));
                second_products[item + 1u] = qrt_sm121_group16::pack_product(qrt_q1_moe_hawkeye::multiply_bf16(
                    uint16_t(q1 >> 16u), uint16_t(k >> 16u), kBlackwellZeroExponent));
            }
        }
        const auto a = qrt_sm121_group16::sum_packed(first, first_products);
        const auto b = qrt_sm121_group16::sum_packed(second, second_products);
        first = qrt_sm121_wave16::normalize(a.value.magnitude, a.value.negative, a.max_exponent);
        second = qrt_sm121_wave16::normalize(b.value.magnitude, b.value.negative, b.max_exponent);
    }
    return {qrt_q1_moe_hawkeye::value_to_float(qrt_sm121_group16::finish_accumulator(first)) * kExactScale,
            qrt_q1_moe_hawkeye::value_to_float(qrt_sm121_group16::finish_accumulator(second)) * kExactScale};
}

// A wave owns the same32 keys for two query rows. One LDS key read feeds
// both independent ordered K16 chains; no products or carries cross rows.
__global__ void blackwell_paired_query_scores_kernel(
    const uint16_t* query, const uint16_t* transposed_key, float* scores,
    unsigned query_start, unsigned query_count, unsigned score_stride, unsigned key_stride) {
    __shared__ uint32_t queries[kPairedTiledQueries][kHeadDim / 2u];
    __shared__ uint32_t keys[kHeadDim / 2u][kTiledExactKeys];
    __shared__ unsigned fallback;
    const unsigned head = blockIdx.y, first_query = blockIdx.z * kPairedTiledQueries;
    const unsigned first_key = blockIdx.x * kTiledExactKeys;
    const unsigned local_query = threadIdx.x / kTiledExactKeys, local_key = threadIdx.x % kTiledExactKeys;
    const unsigned first_row = first_query + local_query, second_row = first_row + kTiledExactQueries;
    const unsigned key = first_key + local_key;
    const unsigned last_query = query_start + min(first_query + kPairedTiledQueries, query_count) - 1u;
    if (first_key > last_query) {
        for (unsigned part = 0u; part < 2u; ++part) {
            const unsigned row = first_row + part * kTiledExactQueries;
            if (row < query_count && key < score_stride)
                scores[(size_t(row) * kQueryHeads + head) * score_stride + key] = -INFINITY;
        }
        return;
    ...
    if (threadIdx.x == 0u) fallback = 0u;
    __syncthreads();
    blackwell_stage_tiled_qk<true, kTiledExactKeys, kTiledExactKeys, kPairedTiledQueries>(
        query, transposed_key, queries, keys, &fallback, head, first_query, first_key,
        query_start, query_count, score_stride, key_stride);
    __syncthreads();
    if (fallback) {
        blackwell_stage_tiled_qk<false, kTiledExactKeys, kTiledExactKeys, kPairedTiledQueries>(
            query, transposed_key, queries, keys, &fallback, head, first_query, first_key,
            query_start, query_count, score_stride, key_stride);
        __syncthreads();
    }
    if (first_row >= query_count || key >= score_stride) return;
    const bool first_live = key <= query_start + first_row;
    const bool second_live = second_row < query_count && key <= query_start + second_row;
    TiledQueryPair result{-INFINITY, -INFINITY};
    if (first_live || second_live) result = fallback
        ? blackwell_paired_query_qk_dot<false>(queries, keys, local_query, local_key)
        : blackwell_paired_query_qk_dot<true>(queries, keys, local_query, local_key);
    scores[(size_t(first_row) * kQueryHeads + head) * score_stride + key] = first_live ? result.first : -INFINITY;
    if (second_row < query_count)
        scores[(size_t(second_row) * kQueryHeads + head) * score_stride + key] = second_live ? result.second : -INFINITY;
}
...
// Eight independent K32 waves compute probabilities concurrently. Thread0
// retains both prefix maxima and the original ordered denominator recurrence;
// the positive FP32 sums are never reassociated across tiles. Inactive waves
// still participate in every barrier, and untouched causal tail slots stay so.
__global__ void blackwell_parallel_probability_kernel(
    const float* scores, uint16_t* probabilities, float* scales,
    unsigned query_start, unsigned score_stride,
    const unsigned char* exp2_table, bool vllm_sum) {
    constexpr unsigned waves = kThreads / 32u;
    const unsigned lane = threadIdx.x % 32u, wave = threadIdx.x / 32u;
    const unsigned row = blockIdx.y * kQueryHeads + blockIdx.x;
    const unsigned tokens = query_start + blockIdx.y + 1u;
    const unsigned tile_stride = (score_stride + 31u) / 32u;
    const unsigned tile_count = (tokens + 31u) / 32u;
    __shared__ float maxima[waves], prefix[waves + 1u], alphas[waves], sums[waves];
    float running_max = -INFINITY, running_sum = 1.0f;
    for (unsigned batch = 0u; batch < tile_count; batch += waves) {
        const unsigned tile = batch + wave, key = tile * 32u + lane;
        const float score = key < tokens ? scores[size_t(row) * score_stride + key] : -INFINITY;
        float maximum = score;
        for (unsigned mask = 16u; mask; mask >>= 1u)
            maximum = fmaxf(maximum, __shfl_xor(maximum, mask, 32u));
        if (lane == 0u) maxima[wave] = maximum;
        __syncthreads();
        const unsigned count = tile_count - batch < waves ? tile_count - batch : waves;
        if (threadIdx.x == 0u) {
            prefix[0] = running_max;
            for (unsigned i = 0u; i < count; ++i) {
                running_max = fmaxf(running_max, maxima[i]);
                prefix[i + 1u] = running_max;
            }
        }
        __syncthreads();
        if (tile < tile_count) {
            const float next_max = prefix[wave + 1u];
            const float alpha = blackwell_attention_exp(prefix[wave] - next_max, exp2_table);
            const float probability = key < tokens ? blackwell_attention_exp(score - next_max, exp2_table) : 0.0f;
            if (key < score_stride)
                probabilities[size_t(row) * score_stride + key] = f32_to_bf16(probability);
            float sum = probability;
            if (vllm_sum) {
                constexpr unsigned order[] = {1u, 4u, 2u, 16u, 8u};
#pragma unroll
            ...
            if (lane == 0u) { alphas[wave] = alpha; sums[wave] = sum; }
        }
        __syncthreads();
        if (threadIdx.x == 0u) {
            for (unsigned i = 0u; i < count; ++i) {
                running_sum = running_sum * alphas[i] + sums[i];
                scales[size_t(row) * (tile_stride + 1u) + batch + i] = alphas[i];
            }
        }
    }
    if (threadIdx.x == 0u)
        scales[size_t(row) * (tile_stride + 1u) + tile_stride] = running_sum;
}

// Three phases share one bounded row: all K32 maxima, independent probability
// waves, then the original ordered denominator recurrence. Unlike the earlier
// eight-wave loop, CTA barriers do not occur between every batch of eight
// tiles. Callers restrict this component to score_stride<=8192.
__global__ void blackwell_staged_probability_kernel(
    const float* scores, uint16_t* probabilities, float* scales,
    unsigned query_start, unsigned score_stride,
    const unsigned char* exp2_table, bool vllm_sum) {
    constexpr unsigned waves = kThreads / 32u, capacity = 8192u / 32u;
    const unsigned lane = threadIdx.x % 32u, wave = threadIdx.x / 32u;
    const unsigned row = blockIdx.y * kQueryHeads + blockIdx.x;
    const unsigned tokens = query_start + blockIdx.y + 1u;
    const unsigned tile_count = (tokens + 31u) / 32u;
    const unsigned tile_stride = (score_stride + 31u) / 32u;
    __shared__ float maxima[capacity], alphas[capacity], sums[capacity], block_maxima[waves];
    for (unsigned tile = wave; tile < tile_count; tile += waves) {
        const unsigned key = tile * 32u + lane;
        float maximum = key < tokens ? scores[size_t(row) * score_stride + key] : -INFINITY;
        for (unsigned mask = 16u; mask; mask >>= 1u)
            maximum = fmaxf(maximum, __shfl_xor(maximum, mask, 32u));
        if (!lane) maxima[tile] = maximum;
    }
    __syncthreads();
    // Maxima can be scanned associatively; no sums are reassociated.
    float prefix = threadIdx.x < tile_count ? maxima[threadIdx.x] : -INFINITY;
    ...
    if (lane == 31u) block_maxima[wave] = prefix;
    __syncthreads();
    if (!threadIdx.x) {
        float preceding = -INFINITY;
        for (unsigned i = 0u; i < waves; ++i) {
            const float current = block_maxima[i];
            block_maxima[i] = preceding;
            preceding = fmaxf(preceding, current);
        }
    }
    __syncthreads();
    if (threadIdx.x < tile_count) maxima[threadIdx.x] = fmaxf(block_maxima[wave], prefix);
    __syncthreads();
    for (unsigned tile = wave; tile < tile_count; tile += waves) {
        const unsigned key = tile * 32u + lane;
        const float next_max = maxima[tile];
        const float previous_max = tile ? maxima[tile - 1u] : -INFINITY;
        const float alpha = blackwell_attention_exp(previous_max - next_max, exp2_table);
        const float score = key < tokens ? scores[size_t(row) * score_stride + key] : -INFINITY;
        const float probability = key < tokens ? blackwell_attention_exp(score - next_max, exp2_table) : 0.0f;
        if (key < score_stride) probabilities[size_t(row) * score_stride + key] = f32_to_bf16(probability);
        float sum = probability;
        if (vllm_sum) {
            constexpr unsigned order[] = {1u, 4u, 2u, 16u, 8u};
#pragma unroll
        ...
        if (!lane) { alphas[tile] = alpha; sums[tile] = sum; }
    }
    __syncthreads();
    if (!threadIdx.x) {
        float running_sum = 1.0f;
        for (unsigned tile = 0u; tile < tile_count; ++tile) {
            running_sum = running_sum * alphas[tile] + sums[tile];
            scales[size_t(row) * (tile_stride + 1u) + tile] = alphas[tile];
        }
        scales[size_t(row) * (tile_stride + 1u) + tile_stride] = running_sum;
    }
}
...
constexpr bool split_separate_probability(unsigned layout) {
    return layout == 3u || (layout >= 5u && layout <= 9u) || layout == 13u || layout == 14u || layout == 22u || layout == 23u || layout == 24u;
}
...
inline size_t split_scratch_elements(unsigned int queries, unsigned int stride,
    ...
    return !split_separate_probability(memory_layout) ? cells : cells + cells / 2u +
        ...
        (memory_layout == 13u || memory_layout == 23u ? rows * kHeadDim :
            (memory_layout == 22u || memory_layout == 24u) ? 2u * rows * kHeadDim + 1u : 0u);
}
...
inline int launch_compacted_pv_replay(
    ...
    if (transposed_value) {
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(blackwell_compacted_pv_replay_kernel<false>),
            dim3(blocks), dim3(kThreads), 0u, stream,
            value, probabilities, scales, output, query_start, output_start, score_stride,
            rcp_table, raw_accumulator, raw_denominator, indices, count, nullptr, 0u, 0u);
    }
    status = hipGetLastError();
    if (status != hipSuccess) return int(status);
    return observe_split_stage(observer, 4u, stream);
...
// Dense enumeration reuses the exact compacted consumer's arithmetic while
// avoiding an approximate PV, error metadata and compaction. No producer has
// to initialize the output or candidate buffers before this call.
inline int launch_all_pv_replay(
    const uint16_t* value, const uint16_t* probabilities, const float* scales,
    float* output, unsigned query_start, unsigned query_count, unsigned output_start,
    unsigned score_stride, const unsigned char* rcp_table, float* raw_accumulator,
    float* raw_denominator, hipStream_t stream, SplitCompletionObserver* observer = nullptr,
    const uint16_t* transposed_value = nullptr, unsigned value_stride = 0u) {
    if (!value || !probabilities || !scales || !output || !query_count ||
        query_count > split_query_limit(22u, score_stride) || query_start >= score_stride ||
        query_count > score_stride - query_start || score_stride > kSplitMaxTokens ||
        output_start >= kSplitMaxTokens || query_count > kSplitMaxTokens - output_start)
        return int(hipErrorInvalidValue);
    if (transposed_value ? value_stride < score_stride || value_stride > kSplitMaxTokens : value_stride != 0u)
    ...
    const unsigned blocks = maximum_blocks < 1024u ? maximum_blocks : 1024u;
    if (transposed_value) {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(blackwell_compacted_pv_replay_kernel<true, true>),
            dim3(blocks), dim3(kThreads), 0u, stream,
            value, probabilities, scales, output, query_start, output_start, score_stride,
            rcp_table, raw_accumulator, raw_denominator, nullptr, nullptr, transposed_value, value_stride, cells);
    } else {
        hipLaunchKernelGGL(HIP_KERNEL_NAME(blackwell_compacted_pv_replay_kernel<false, true>),
            dim3(blocks), dim3(kThreads), 0u, stream,
            value, probabilities, scales, output, query_start, output_start, score_stride,
            rcp_table, raw_accumulator, raw_denominator, nullptr, nullptr, nullptr, 0u, cells);
    }
    const auto status = hipGetLastError();
    return status == hipSuccess ? observe_split_stage(observer, 4u, stream) : int(status);
}
...
inline int launch_queries(const uint16_t* q, const uint16_t* k,
    ...
    if ((tiled_qk_lanes != 1u && tiled_qk_lanes != 4u) ||
        ...
        ((tiled_qk_lanes == 4u || tiled_qk_rows_per_thread == 2u) && memory_layout != 15u && memory_layout != 16u &&
    ...
    if (memory_layout >= 2u) {
        ...
        if (split_separate_probability(memory_layout)) {
            ...
            } else if (memory_layout == 24u) {
                hipLaunchKernelGGL(blackwell_parallel_probability_kernel,
                    dim3(kQueryHeads, query_count), dim3(kThreads), 0u, stream,
                    score_scratch, probabilities, scales, query_start, stride, exp2_table, vllm_sum);
            } else {
            ...
            if (all_pv_replay) {
                return launch_all_pv_replay(v, probabilities, scales, output,
                    query_start, query_count, output_start, stride, rcp_table,
                    raw_accumulator, raw_denominator, stream, observer,
