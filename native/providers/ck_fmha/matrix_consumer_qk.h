#ifndef QRT_MATRIX_CONSUMER_QK_H
#define QRT_MATRIX_CONSUMER_QK_H
...
namespace qrt_matrix_consumer_qk {
...
#pragma unroll
                ...
                matrix=blackwell_integer_prepared_products(left[lane%16u].core,right[wave*16u+lane%16u].core);
            }
#pragma unroll
            for (unsigned element=0u;element<8u;++element) {
                const unsigned row=2u*element+lane/16u,column=wave*16u+lane%16u;
...
#pragma unroll
    for (unsigned item=0u;item<per_thread;++item) {
        ...
        if (row<query_count && key_row<stride)
            output[(size_t(row)*kQueryHeads+head)*stride+key_row]=key_row<=query_start+row?accumulators[item]*kExactScale:-INFINITY;
    }
}
} // namespace qrt_matrix_consumer_qk
#endif
