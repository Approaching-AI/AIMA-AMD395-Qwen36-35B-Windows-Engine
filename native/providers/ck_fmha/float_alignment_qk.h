#ifndef QRT_FLOAT_ALIGNMENT_QK_H
#define QRT_FLOAT_ALIGNMENT_QK_H
#include "blackwell_attention.h"
#include "../moe_accumulator/sm121_float_alignment.h"

// Component-only scalar replacement; no product dispatcher includes this file.
namespace qrt_float_alignment_qk {
...
#pragma unroll
        ...
        if (!alignment::sum(carry, group, &sum)) return false;
...
__global__ void scores(const uint16_t* query, const uint16_t* transposed_key, float* output,
    ...
    for (unsigned cell = threadIdx.x; cell < kTiledExactQueries * (kHeadDim / 2u); cell += kThreads) {
        ...
        invalid = invalid || !alignment::eligible(a) || !alignment::eligible(b);
    ...
    float value;
    if (fallback || !dot(queries, keys, local_query, local_key, &value))
