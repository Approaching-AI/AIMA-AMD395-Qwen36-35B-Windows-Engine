...
std::size_t gb10_native_gdn_pipeline(const void* q, const void* k, const void* v,
  ...
  if (s.persistent_prefill) {
    // The fused block owns its complete K128 state through every chunk.
    // Only the immutable cold input and final resident destination are global.
    ...
    s.persistent[index]->launch(aima::AotLaunchConfig{32, 16, 1, image.warps, 32, image.shared},
        {&q, &k, &w, &u, &g, &scores, &table, &incoming, &state, &output,
         &unused_debug, &unused_debug, &count});
    return launches + 1;
  }
