...
#include "../../native/providers/ck_fmha/native_rz_qk.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdio>
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    unsigned start,unsigned count,unsigned stride,unsigned key_stride,Prepared& prepared) {
    const dim3 grid((stride+15u)/16u,kQueryHeads,(count+15u)/16u);
#define QRT_QK_ARGS q,k,prepared.qp.as<uint32_t>()+guard,prepared.kp.as<uint32_t>()+guard,prepared.qf.as<unsigned>()+guard,prepared.kf.as<unsigned>()+guard,out,start,count,stride,key_stride
    if(!variant) hipLaunchKernelGGL((qrt_prepared_decoded_qk::scores<128u,true>),grid,dim3(kThreads),0u,nullptr,QRT_QK_ARGS);
    else if(variant==1u) hipLaunchKernelGGL((qrt_native_rz_qk::scores<false>),grid,dim3(kThreads),0u,nullptr,QRT_QK_ARGS);
    else if(variant==2u) hipLaunchKernelGGL((qrt_native_rz_qk::scores<true>),grid,dim3(kThreads),0u,nullptr,QRT_QK_ARGS);
    else throw std::runtime_error("invalid native RZ QK variant");
#undef QRT_QK_ARGS
    check(hipGetLastError());
}
std::array<unsigned long long,4> audit_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    unsigned start,unsigned count,unsigned stride,unsigned key_stride,Prepared& prepared) {
    ...
    const dim3 grid((stride+15u)/16u,kQueryHeads,(count+15u)/16u);
#define QRT_AUDIT_ARGS q,k,prepared.qp.as<uint32_t>()+guard,prepared.kp.as<uint32_t>()+guard,prepared.qf.as<unsigned>()+guard,prepared.kf.as<unsigned>()+guard,out,start,count,stride,key_stride,counters.as<unsigned long long>()+guard
    if(variant==1u) hipLaunchKernelGGL((qrt_native_rz_qk::scores<false,true>),grid,dim3(kThreads),0u,nullptr,QRT_AUDIT_ARGS);
    else if(variant==2u) hipLaunchKernelGGL((qrt_native_rz_qk::scores<true,true>),grid,dim3(kThreads),0u,nullptr,QRT_AUDIT_ARGS);
    else throw std::runtime_error("invalid native RZ QK audit variant");
#undef QRT_AUDIT_ARGS
    check(hipGetLastError());finish();const auto actual=download<unsigned long long>(counters,expected.size());
    for(unsigned i=0u;i<guard;++i)if(actual[i]!=marker || actual[guard+4u+i]!=marker)throw std::runtime_error("native RZ audit counter guard");
    const unsigned long long dots=(static_cast<unsigned long long>(count)*(2u*start+count+1u)/2u)*kQueryHeads;
    if(actual[guard]>dots*16u || actual[guard+1u]>dots || actual[guard+2u] || !actual[guard+3u])throw std::runtime_error("native RZ audit coverage or mode restoration");
...
void run(Case c) {
    ...
    if(c.mode>=8u && c.mode<=10u) {
        const unsigned qe=c.mode==9u?190u:76u;
        const unsigned ke=c.mode==9u?185u:c.mode==8u?77u:76u;
        for(size_t i=guard;i+guard<q.size();++i)q[i]=uint16_t((qe<<7u)|(i&127u));
        for(size_t i=guard;i+guard<k.size();++i)k[i]=uint16_t((ke<<7u)|(i*7u&127u)|((i&1u)?0x8000u:0u));
    }
    if(c.mode==11u) {
        for(size_t i=guard;i+guard<q.size();++i)q[i]=uint16_t(((64u+i%127u)<<7u)|(i&127u));
        for(size_t i=guard;i+guard<k.size();++i)k[i]=uint16_t(((64u+i*17u%127u)<<7u)|(i*7u&127u)|((i&1u)?0x8000u:0u));
    }
    ...
    for(unsigned variant:variants) {
        ...
        staged_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,candidate.as<float>()+guard,c.start,c.count,stride,c.tokens,prepared);
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("native RZ QK differs from original scores");
            }
        }
        ...
        if(variant) {
            ...
            stats=audit_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,audit.as<float>()+guard,c.start,c.count,stride,c.tokens,prepared);
            if(download<uint32_t>(audit,cells+2u*guard)!=b)throw std::runtime_error("generated production/audit parity");
        }
        std::printf("{\"kind\":\"native_rz_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"audit_invoked\":%u,\"audit_fast_groups\":%llu,\"audit_fallback_dots\":%llu,\"audit_mode_failures\":%llu,\"audit_waves\":%llu,\"production_diagnostic_parity\":true,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells,unsigned(variant!=0u),stats[0],stats[1],stats[2],stats[3]);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
        ...
        for(unsigned position=0u;position<variant_count;++position) {
            ...
            for(unsigned attempt=0u;attempt<attempts;++attempt) {
                ...
                staged_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,candidate.as<float>()+guard,
                    start,count,stride,tokens,prepared);
            ...
            if(variant) {
                ...
                const auto stats=audit_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,audit.as<float>()+guard,start,count,stride,tokens,prepared);
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"native_rz_qk_capture\",\"variant\":%u,\"query_rows\":16,\"key_columns\":16,\"native_rz_enabled\":%u,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"audit_invoked\":%u,\"audit_fast_groups\":%llu,\"audit_fallback_dots\":%llu,\"audit_mode_failures\":%llu,\"audit_waves\":%llu,\"production_diagnostic_parity\":true,\"round_state_restored\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,unsigned(variant==2u),tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
...
int main(int argc,char** argv) {
    try {
        ...
        const Case cases[]={{17,0,8,0},{17,0,9,2},{33,0,16,0},{33,1,31,1},{1,0,1,0},{9,0,9,1},{35,3,17,2},{67,33,32,3},{67,64,3,4},
            ...
            {35,3,17,8},{35,3,17,9},{35,3,17,10},{35,3,17,11}};
