...
#include "../../native/providers/gdn/tiled_scalar_matrices.h"
#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstdio>
...
namespace compact=qrt_fla_tiled_scalar;
...
void launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,Device& inverse,Device& g,Device& scores,Device& u,Device& w,Device& output,Device& h,Device& vn,Device& state,Device& table,bool alias){
 const auto* exp=reinterpret_cast<unsigned char*>(table.data<uint32_t>());
 auto* actual_u=alias?v.data<uint16_t>():u.data<uint16_t>();
 // Match the product's bounded 1024-token WU/state/output sequence. The same
 // FP32 state persists across segments; observations occur only at completion.
 for(unsigned offset=0u;offset<count;offset+=1024u){
  const unsigned n=std::min(1024u,count-offset),chunks=(n+63u)/64u;
#define QRT_WU_ARGS k.data<uint16_t>()+size_t(offset)*2048u,v.data<uint16_t>()+size_t(offset)*4096u,beta.data<uint16_t>()+size_t(offset)*32u,inverse.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,w.data<uint16_t>()+size_t(offset)*4096u,actual_u+size_t(offset)*4096u,n,exp
#define QRT_STATE_ARGS k.data<uint16_t>()+size_t(offset)*2048u,actual_u+size_t(offset)*4096u,w.data<uint16_t>()+size_t(offset)*4096u,g.data<float>()+size_t(offset)*32u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,vn.data<uint16_t>()+size_t(offset)*4096u,state.data<float>(),n,exp
#define QRT_OUTPUT_ARGS q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,n,exp
  if(!variant){
   hipLaunchKernelGGL(scalar::wu_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_WU_ARGS);check(hipGetLastError());
   hipLaunchKernelGGL((scalar::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,QRT_STATE_ARGS);check(hipGetLastError());
   hipLaunchKernelGGL(scalar::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_OUTPUT_ARGS);check(hipGetLastError());
  }else if(variant==1u){
   hipLaunchKernelGGL((compact::wu_kernel<1u,2u,256u>),dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_WU_ARGS);check(hipGetLastError());
   hipLaunchKernelGGL((compact::state_kernel<8u,1u,2u,256u>),dim3(16u,32u),dim3(256u),0u,nullptr,QRT_STATE_ARGS);check(hipGetLastError());
   hipLaunchKernelGGL((compact::output_kernel<1u,2u,256u>),dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_OUTPUT_ARGS);check(hipGetLastError());
  }else{
   require(variant==2u,"invalid tiled GDN variant");
   hipLaunchKernelGGL((compact::wu_kernel<2u,2u,128u>),dim3(16u,32u,chunks),dim3(128u),0u,nullptr,QRT_WU_ARGS);check(hipGetLastError());
   hipLaunchKernelGGL((compact::state_kernel<8u,2u,2u,128u>),dim3(16u,32u),dim3(128u),0u,nullptr,QRT_STATE_ARGS);check(hipGetLastError());
   hipLaunchKernelGGL((compact::output_kernel<2u,2u,128u>),dim3(16u,32u,chunks),dim3(128u),0u,nullptr,QRT_OUTPUT_ARGS);check(hipGetLastError());
  }
#undef QRT_WU_ARGS
#undef QRT_STATE_ARGS
#undef QRT_OUTPUT_ARGS
 }
}
...
void run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table){
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<6u;++position){
  ...
  const auto begin=std::chrono::steady_clock::now();launch(variant,count,dq,dk,dv,db,dinv,dg,ds,du,dw,output,h,vn,state,table,alias);finish();
  if(attempt){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"tiled-scalar GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"tiled-scalar GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"tiled-scalar GDN intermediate or alias differs");
 ...
 for(unsigned alias=0u;alias<2u;++alias)for(unsigned variant=0u;variant<3u;++variant){
  ...
  std::printf("{\"kind\":\"tiled_scalar_gdn_component\",\"tokens\":%u,\"mode\":%u,\"variant\":%u,\"tile_rows\":%u,\"tile_columns\":%u,\"threads\":%u,\"segment_tokens\":1024,\"segments\":%u,\"u_aliases_v\":%s,\"output_cells\":%zu,\"state_cells\":%zu,\"checkpoint_cells\":%zu,\"wu_and_residual_cells\":%zu,\"independent_cpu_dots\":%zu,\"warmups\":1,\"measured_attempts\":%u,\"complete_wu_state_output_ms\":%.6f,\"samples_ms\":[%.6f,%.6f,%.6f],\"intermediate_host_synchronization\":false,\"all_attempts_verified\":true,\"raw_bit_mismatches\":0,\"intermediate_and_alias_ownership_checked\":true,\"redzones_pass\":true,\"immutable_nonaliased_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",count,mode,variant,variant==2u?2u:1u,variant?2u:1u,variant==2u?128u:256u,(count+1023u)/1024u,alias?"true":"false",large,state_cells,checkpoints,large*3u,cpu_dots,measured,sorted[1],samples[alias][variant][0],samples[alias][variant][1],samples[alias][variant][2]);
 }
 std::fflush(stdout);
}
int main(int argc,char** argv)try{
 ...
 if(timing)run(8192u,1u,3u,dt,table);else for(unsigned count:{1u,63u,64u,65u,129u,1023u,1024u,1025u})for(unsigned mode=0u;mode<3u;++mode)run(count,mode,0u,dt,table);
