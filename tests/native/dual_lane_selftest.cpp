...
__global__ void execute(const uint16_t* left, const uint16_t* right, const unsigned* flags,
    ...
    const unsigned row = (blockIdx.x * blockDim.x + threadIdx.x) / 2u;
    if (row >= rows) return;
    qrt_sm121_dual_lane::Stats stats;
    const float value = qrt_sm121_dual_lane::dot<Trace>(left + size_t(row) * width,
        right + size_t(row) * width, width, flags[row] != 0u,
        Trace ? traces + size_t(row) * (width / 16u) : nullptr, Trace ? &stats : nullptr);
    if (!(threadIdx.x & 1u)) output[row] = {value, stats.floating, stats.integer};
}
...
void variant(unsigned rows, unsigned width, Device& left, Device& right,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(execute<true>), dim3((rows * 2u + 255u) / 256u),
    ...
    for (size_t i = 0u; i < groups; ++i) if (actual[i + guard] != expected[i]) {
        if (mismatches < 4u) std::printf("DIFF lanes=2 width=%u row=%zu group=%zu expected=%08x actual=%08x\n",
    ...
    check(hipMemset(output.value, 0xa5, (rows + 2u * guard) * sizeof(Result)));
    hipLaunchKernelGGL(HIP_KERNEL_NAME(execute<false>), dim3((rows * 2u + 255u) / 256u),
        dim3(256u), 0u, nullptr, left.data<uint16_t>(), right.data<uint16_t>(), flags.data<unsigned>(), output.data<Result>(),
        traces.data<uint32_t>(), rows, width);
    check(hipGetLastError()); check(hipDeviceSynchronize());
    ...
    std::printf("{\"kind\":\"dual_lane_safety\",\"lanes\":2,\"rows\":%u,\"width\":%u,\"ordered_groups\":%zu,\"raw_bit_mismatches\":%u,\"floating_groups\":%u,\"integer_groups\":%u,\"unaligned_operands\":true,\"production_diagnostic_parity\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",
        rows, width, groups, mismatches, floating, integer);
    std::fflush(stdout);
    if (mismatches || !floating || !integer || floating + integer != groups)
        throw std::runtime_error("dual-lane arithmetic or coverage failure");
}
void run(unsigned rows, unsigned width) {
    ...
    check(hipGetLastError());check(hipDeviceSynchronize());
    variant(rows, width, left, right, output, traces, flags, a, b, expected, expected_flags, expected_floating);
}
int main() try {
    ...
    return 0;
} catch (const std::exception& error) { std::fprintf(stderr, "dual_lane_error=%s\n", error.what()); return 1; }
