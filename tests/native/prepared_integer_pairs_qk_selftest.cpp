...
#include "../../native/providers/ck_fmha/prepared_integer_pairs_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
using Row = qrt_prepared_integer_pairs_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_prepared_integer_pairs_qk::prepare_rows<Kind>),
...
struct IntegerRows {
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            const Row value=qrt_prepared_integer_pairs_qk::prepare(original);
...
#define PREPARED_PAIRS_CASE(w) if(variant==w) hipLaunchKernelGGL((qrt_prepared_integer_pairs_qk::scores<w>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        PREPARED_PAIRS_CASE(64u);
        else PREPARED_PAIRS_CASE(128u);
        else PREPARED_PAIRS_CASE(256u);
        else throw std::runtime_error("invalid prepared integer pairs QK variant");
...
__global__ void paired_group_values(const Row* left,const Row* right,const uint32_t* carry,
    uint32_t* output,unsigned count) {
    const unsigned i=blockIdx.x*blockDim.x+threadIdx.x;
    if(i>=count)return;
    const qrt_q1_moe_hawkeye::Value input{carry[3u*i],int16_t(int32_t(carry[3u*i+1u])),carry[3u*i+2u]!=0u};
    const auto value=qrt_sm121_prepared_integer_pairs::accumulate(input,left[i],right[i]);
...
void paired_preflight() {
    constexpr unsigned count=65536u;
    ...
    for(unsigned group=0u;group<count;++group) {
        ...
        for(unsigned i=0u;i<16u;++i) {
            a[i]=uint16_t(group+i*4093u);b[i]=controls[i];
        ...
        left[guard+group]=qrt_sm121_prepared_integer_pairs::prepare(a);right[guard+group]=qrt_sm121_prepared_integer_pairs::prepare(b);
    ...
    Device a(left.size()*sizeof(Row)),b(right.size()*sizeof(Row)),c(carry.size()*4u),out(expected.size()*4u);
    upload(a,left);upload(b,right);upload(c,carry);check(hipMemset(out.pointer,0xa5,expected.size()*4u));
    hipLaunchKernelGGL(paired_group_values,dim3((count+255u)/256u),dim3(256u),0u,nullptr,
        a.as<Row>()+guard,b.as<Row>()+guard,c.as<uint32_t>()+guard,out.as<uint32_t>()+guard,count);
    check(hipGetLastError());finish();unchanged(out,expected);unchanged(a,left);unchanged(b,right);unchanged(c,carry);
    std::printf("{\"kind\":\"prepared_integer_pairs_preflight\",\"all_bf16_encodings\":65536,\"controls\":16,\"original_products\":1048576,\"raw_carry_states\":65536,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n");
    std::fflush(stdout);
}
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("prepared integer pairs QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"prepared_integer_pairs_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
