...
__global__ void execute(const uint16_t* a, const uint16_t* b, const uint16_t* transposed,
    ...
    if constexpr (Variant >= 4u)
        ...
        value = scalar::dot<false>(a + size_t(row) * width, b, rows, row, width, eligible);
...
void run(unsigned rows, unsigned width) {
    ...
    for (unsigned variant : {1u, 2u, 3u, 4u, 5u, 6u}) {
        ...
        std::printf("{\"kind\":\"scalar_projection_safety\",\"variant\":%u,\"rows\":%u,\"width\":%u,\"eligible_dots\":%u,\"fallback_dots\":%u,\"raw_bit_mismatches\":%u,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n", variant, rows, width, eligible_dots, rows - eligible_dots, bad);
...
} catch (const std::exception& error) { std::fprintf(stderr, "scalar_projection_error=%s\n", error.what()); return 2; }
