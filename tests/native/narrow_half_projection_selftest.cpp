...
namespace projection=qrt_sm121_narrow_half_projection;
...
__global__ void execute(const scaled::Row* a,const scaled::Row* b,const unsigned* fa,const unsigned* fb,Result* out,uint32_t* trace,unsigned rows,unsigned width){
 const unsigned row=(blockIdx.x*blockDim.x+threadIdx.x)/4u;if(row>=rows)return;
 projection::Stats stats;const float value=projection::dot<StagingGroups,Audit>(a+size_t(row)*(width/16u),b+size_t(row)*(width/16u),width,fa[row]&&fb[row],
...
void variant(unsigned rows,unsigned width,unsigned family,Device& a,Device& b,Device& pa,Device& pb,Device& fa,Device& fb,Device& out,Device& trace,
 ...
 hipLaunchKernelGGL((execute<StagingGroups,true>),dim3((rows*4u+255u)/256u),dim3(256u),0u,nullptr,pa.data<scaled::Row>(),pb.data<scaled::Row>(),fa.data<unsigned>(),fb.data<unsigned>(),out.data<Result>(),trace.data<uint32_t>(),rows,width);check(hipGetLastError());finish();
 ...
 if((width<=8192u&&!full_fast) || (family!=1u && !restarts))throw std::runtime_error("missing numerical path coverage");
 check(hipMemset(out.pointer,0xa5,(rows+2u*guard)*sizeof(Result)));
 hipLaunchKernelGGL((execute<StagingGroups,false>),dim3((rows*4u+255u)/256u),dim3(256u),0u,nullptr,pa.data<scaled::Row>(),pb.data<scaled::Row>(),fa.data<unsigned>(),fb.data<unsigned>(),out.data<Result>(),trace.data<uint32_t>(),rows,width);check(hipGetLastError());finish();
 ...
 for(unsigned side=0u;side<2u;++side){
  const auto& input=side?right:left;const auto flags=read<unsigned>(side?fb:fa,rows);guards(flags);const auto raw=read<uint16_t>(side?b:a,input.size());const auto packed=read<scaled::Row>(side?pb:pa,groups);guards(raw);guards(packed);
  if(std::memcmp(raw.data()+guard,input.data(),input.size()*2u))throw std::runtime_error("original operand changed");
  for(unsigned row=0u;row<rows;++row){
   bool eligible=width<=8192u;
   for(unsigned g=0u;g<width/16u;++g){unsigned low=255u,high=0u;
    for(unsigned i=0u;i<16u;++i){const uint16_t x=input[size_t(row)*width+g*16u+i];if(x&0x7fffu){const unsigned e=(x>>7u)&255u;eligible&=e>=95u&&e<=159u;low=std::min(low,e);high=std::max(high,e);}}
    eligible&=!high||high-low<=29u;
   }
   if(flags[guard+row]!=unsigned(eligible))throw std::runtime_error("whole-row classification differs");
  }
 ...
 std::printf("{\"kind\":\"narrow_half_projection_safety\",\"lanes\":4,\"staging_groups\":%u,\"rows\":%u,\"width\":%u,\"family\":%u,\"ordered_fp32_carry_endpoints\":%zu,\"admitted_groups\":%zu,\"original_dots\":%zu,\"fully_fast_dots\":%zu,\"raw_bit_mismatches\":0,\"unaligned_operands\":true,\"production_diagnostic_parity\":true,\"whole_row_flags_checked\":true,\"all_encoded_words_checked\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",StagingGroups,rows,width,family,groups,accepted,restarts,full_fast);std::fflush(stdout);
}
void run(unsigned rows,unsigned width,unsigned family=0u){
 ...
 for(unsigned row=0u;row<rows;++row){
  ...
  for(unsigned group=0u;group<width/16u;++group){
   ...
   for(unsigned i=0u;i<16u;++i){
    ...
    if(family==2u){const auto p=qrt_narrow_half_cases::input(row,group,i);input={p.x,p.y};}
    if(family==1u){const unsigned a=cases::random_word(unsigned(base+i)^0x3958192u),b=cases::random_word(a^0x8192395u);input={uint16_t((a&0x807fu)|((120u+a%12u)<<7u)),uint16_t((b&0x807fu)|((119u+b%14u)<<7u))};if((base+i)%37u==0u)input.x=uint16_t(a&0x8000u);}
 ...
 Device a((words+2u*guard)*2u),b((words+2u*guard)*2u),pa((groups+2u*guard)*sizeof(scaled::Row)),pb((groups+2u*guard)*sizeof(scaled::Row)),fa((rows+2u*guard)*4u),fb((rows+2u*guard)*4u),out((rows+2u*guard)*sizeof(Result)),trace((groups+2u*guard)*4u);
 ...
 hipLaunchKernelGGL(projection::classify_rows,dim3(rows+1u),dim3(128u),0u,nullptr,a.data<uint16_t>(),pa.data<scaled::Row>(),fa.data<unsigned>(),rows,width);check(hipGetLastError());
 hipLaunchKernelGGL(projection::classify_rows,dim3(rows+1u),dim3(128u),0u,nullptr,b.data<uint16_t>(),pb.data<scaled::Row>(),fb.data<unsigned>(),rows,width);check(hipGetLastError());finish();
 variant<2u>(rows,width,family,a,b,pa,pb,fa,fb,out,trace,left,right,expected,endpoints);
 variant<4u>(rows,width,family,a,b,pa,pb,fa,fb,out,trace,left,right,expected,endpoints);
}
int main()try{
 ...
 run(257u,16u);run(4096u,272u);run(2048u,2048u);run(1024u,4096u);run(129u,4112u);run(129u,8192u);run(2048u,2048u,1u);
 for(unsigned width:{16u,32u,256u,2048u,4096u,8192u,8208u})run(257u,width,2u);
 return 0;
