...
#include "../../native/providers/ck_fmha/scalar_integer_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
constexpr unsigned variants[] = {0u, 164u, 264u, 1128u, 2128u};
...
using Row = qrt_scalar_integer_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_scalar_integer_qk::prepare_rows<Kind>),
...
struct IntegerRows {
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            qrt_scalar_integer_qk::prepare(value);
...
#define SCALAR_INTEGER_CASE(v,w,p) if(variant==v) hipLaunchKernelGGL((qrt_scalar_integer_qk::scores<w,p>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        SCALAR_INTEGER_CASE(164u,64u,false);
        else SCALAR_INTEGER_CASE(264u,64u,true);
        else SCALAR_INTEGER_CASE(1128u,128u,false);
        else SCALAR_INTEGER_CASE(2128u,128u,true);
        else throw std::runtime_error("invalid scalar integer QK variant");
#undef SCALAR_INTEGER_CASE
    }
    check(hipGetLastError());
}
...
__global__ void dot4_values(const uint32_t* left,const uint32_t* right,const int32_t* carry,
    int32_t* output,unsigned count) {
    const unsigned i=blockIdx.x*blockDim.x+threadIdx.x;
    if(i>=count)return;
    using qrt_sm121_scalar_integer_core::dot4;
    output[4u*i]=dot4<false,false>(left[i],right[i],carry[i]);
    output[4u*i+1u]=dot4<true,false>(left[i],right[i],carry[i]);
    output[4u*i+2u]=dot4<false,true>(left[i],right[i],carry[i]);
    output[4u*i+3u]=dot4<true,true>(left[i],right[i],carry[i]);
}
void dot4_preflight() {
    constexpr unsigned count=1048576u;
    std::vector<uint32_t> left(count+2u*guard,0xa5a5a5a5u),right=left;
    std::vector<int32_t> carries(count+2u*guard,int32_t(0xa5a5a5a5u));
    unsigned state=0x3958192u;
    auto random=[&](){state^=state<<13u;state^=state>>17u;state^=state<<5u;return state;};
    const uint32_t edges[]={0u,0x7f7f7f7fu,0x80808080u,0xffffffffu,0x00807fffu,0xff7f8000u};
    for(unsigned i=0u;i<count;++i) {
        left[guard+i]=i<36u?edges[i%6u]:random();
        right[guard+i]=i<36u?edges[i/6u]:random();
        carries[guard+i]=int32_t(random()%134217729u)-67108864;
    }
    Device a(left.size()*4u),b(right.size()*4u),c(carries.size()*4u),o((size_t(count)*4u+2u*guard)*4u);
    upload(a,left);upload(b,right);upload(c,carries);
    check(hipMemset(o.pointer,0xa5,(size_t(count)*4u+2u*guard)*4u));
    hipLaunchKernelGGL(dot4_values,dim3((count+255u)/256u),dim3(256u),0u,nullptr,
        a.as<uint32_t>()+guard,b.as<uint32_t>()+guard,c.as<int32_t>()+guard,o.as<int32_t>()+guard,count);
    check(hipGetLastError());finish();
    const auto out=download<int32_t>(o,size_t(count)*4u+2u*guard);
    for(unsigned i=0u;i<count;++i) for(unsigned policy=0u;policy<4u;++policy) {
        int64_t expected=carries[guard+i];
        for(unsigned byte=0u;byte<4u;++byte) {
            int x=int((left[guard+i]>>(8u*byte))&255u),y=int((right[guard+i]>>(8u*byte))&255u);
            if((policy&1u) && x>=128)x-=256;
            if((policy&2u) && y>=128)y-=256;
            expected+=x*y;
        }
        if(int64_t(out[guard+4u*i+policy])!=expected) {
            std::fprintf(stderr,"dot4 index=%u policy=%u expected=%lld actual=%d\n",i,policy,static_cast<long long>(expected),out[guard+4u*i+policy]);
            throw std::runtime_error("gfx1151 scalar byte dot4 differs from independent integer reference");
        }
    }
    for(unsigned i=0u;i<guard;++i)
        if(uint32_t(out[i])!=0xa5a5a5a5u || uint32_t(out[guard+size_t(count)*4u+i])!=0xa5a5a5a5u)
            throw std::runtime_error("dot4 output guard changed");
    unchanged(a,left);unchanged(b,right);unchanged(c,carries);
    std::printf("{\"kind\":\"scalar_dot4_preflight\",\"packed_pairs\":1048576,\"sign_policies\":4,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n");
    std::fflush(stdout);
}
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("scalar integer QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"scalar_integer_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
