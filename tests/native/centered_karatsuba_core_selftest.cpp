...
struct Cell { int integer[4]; float candidate[6]; int rounded[2]; int64_t recovered[2]; };
...
void finish() {
    ...
    for (;;) {
        ...
        if (std::chrono::steady_clock::now() >= deadline) throw std::runtime_error("centered core completion deadline");
...
template<class T> struct Guarded {
    ...
    void verify_guards() {
        ...
        if (std::any_of(low.begin(), low.end(), [](unsigned char x) { return x != 0xa5u; }) ||
            ...
            throw std::runtime_error("centered core memory guard changed");
    }
    std::vector<T> read() {
    ...
    void unchanged(const std::vector<T>& expected) {
        ...
        if (std::memcmp(actual.data(), expected.data(), size * sizeof(T))) throw std::runtime_error("centered core input changed");
        verify_guards();
    }
};
...
__global__ void compare(const Row* rows, const core::Prepared* prepared, Cell* output) {
    ...
    const auto inline_parts=core::products(rows[base+lane%16u],rows[base+16u+lane%16u],a,b);
    const auto pa=prepared[base+lane%16u],pb=prepared[base+16u+lane%16u];
    const auto prepared_parts=core::products(rows[base+lane%16u],rows[base+16u+lane%16u],pa,pb);
#pragma unroll
...
#pragma unroll
        ...
        cell.rounded[0]=core::nearest(inline_parts.combined[i]);cell.rounded[1]=core::nearest(prepared_parts.combined[i]);
        cell.recovered[0]=core::reconstruct(inline_parts.high[i],inline_parts.low[i],cell.rounded[0],__shfl(a.sum,2u*i+lane/16u),b.sum);
        cell.recovered[1]=core::reconstruct(prepared_parts.high[i],prepared_parts.low[i],cell.rounded[1],__shfl(pa.sum,2u*i+lane/16u),pb.sum);
...
#pragma unroll
            ...
            const auto parts=core::products(rows[base+lane%16u],rows[base+16u+lane%16u],a,b);
#pragma unroll
            for(unsigned i=0u;i<8u;++i)totals[i]+=uint64_t(core::reconstruct(parts.high[i],parts.low[i],core::nearest(parts.combined[i]),__shfl(a.sum,2u*i+lane/16u),b.sum));
        }
    }
#pragma unroll
    for (unsigned i = 0u; i < 8u; ++i)
...
void run() {
    ...
    for (unsigned row = 0u; row < rows.size(); ++row) {
        ...
        for(unsigned i=0u;i<16u;++i){
            ...
            const int high=int(value>>8u)-((value&32768u)?256:0),low=int(value&255u),combined=high+low-128;
            if(decode_half(expected_prepared[row].combined[i])!=float(combined))throw std::runtime_error("prepared centered digit changed");
            sum+=combined;
        }
        if(sum!=expected_prepared[row].sum)throw std::runtime_error("prepared centered row sum changed");
    }
    ...
    size_t integer_bad=0u,raw_differences[2]{},recovery_bad[2]{},reconstruction_bad[2]{};
    ...
    for(unsigned cell=0u;cell<actual.size();++cell){
        ...
        int integer[4]{},combined=0;int64_t total=0;
        for(unsigned k=0u;k<16u;++k){
            ...
            combined+=(ah+al-128)*(bh+bl-128);total+=int64_t(decoded(a))*decoded(b);
        }
        ...
        for(unsigned variant=0u;variant<2u;++variant){
            ...
            integer_bad+=unsigned(parts[0]!=float(integer[0]) || parts[1]!=float(integer[3]));
            raw_differences[variant]+=unsigned(parts[2]!=float(combined));
            const double error=std::abs(double(parts[2])-combined);
            if(!std::isfinite(parts[2]) || error>85.0/256.0)throw std::runtime_error("centered native error bound failed");
            maximum_error[variant]=std::max(maximum_error[variant],error);
            recovery_bad[variant]+=unsigned(actual[cell].rounded[variant]!=combined);
    ...
    for(unsigned variant=0u;variant<2u;++variant)
        std::printf("{\"kind\":\"centered_karatsuba_core\",\"variant\":%u,\"tiles\":%u,\"cells\":%zu,\"core_encodings\":65536,\"integer_partial_mismatches\":%zu,\"native_combined_raw_differences\":%zu,\"rounded_combined_mismatches\":%zu,\"reconstruction_mismatches\":%zu,\"maximum_native_absolute_error\":%.12g,\"conditional_error_bound\":0.33203125,\"prepared_encoding_ms\":%.6f,\"prepared_bytes\":%zu,\"redzones_pass\":true,\"immutable_inputs\":true,\"integer_rounding_used\":true,\"hardware_premise_proven\":false,\"inference_acceptance\":false}\n",variant+1u,tiles,actual.size(),integer_bad,raw_differences[variant],recovery_bad[variant],reconstruction_bad[variant],maximum_error[variant],encoding_ms,expected_prepared.size()*sizeof(core::Prepared));
    std::fflush(stdout);
    if(integer_bad || recovery_bad[0] || recovery_bad[1] || reconstruction_bad[0] || reconstruction_bad[1])throw std::runtime_error("centered core differs from independent integer oracle");
