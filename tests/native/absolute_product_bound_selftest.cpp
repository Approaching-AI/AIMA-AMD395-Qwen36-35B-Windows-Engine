...
void run(Shape s) {
    ...
    for (unsigned partial = 0u; partial < 2u; ++partial) {
        ...
        const unsigned first_token = unsigned(first / s.rows), last_token = unsigned((first + count - 1u) / s.rows);
        ...
        hipLaunchKernelGGL(qrt_bf16_absolute_product_matrix::window_kernel,
            dim3((s.rows + 127u) / 128u, (last_token - first_token + 64u) / 64u), dim3(256u), 0u, nullptr,
            dw.data(), dx.data(), dwf.data(), dxf.data(), bounds.data(), s.rows, s.tokens, s.k, first, count);
...
int main() try {
    ...
    for (const Shape s : {Shape{3,5,16,0}, {129,65,512,0}, {257,131,2048,1},
                         {129,67,4096,2}, {37,71,2048,3}, {131,17,2048,4}}) run(s);
    return 0;
