#pragma once
#include "../../native/providers/moe_accumulator/sm121_scalar_projection.h"

namespace projection_safety_test {
template<unsigned Variant>
__global__ void scalar_projection_replay_kernel(const uint16_t* weights, const uint16_t* inputs,
    const uint16_t* prepared_weights, const uint16_t* prepared_inputs, const uint16_t* transposed,
    ...
    constexpr unsigned lanes = Variant == 0u || Variant == 3u ? 4u : 1u;
    ...
    if constexpr (Variant == 0u)
        value = eligible ? qrt_sm121_prepared_projection::dot<4u>(prepared_inputs + size_t(token) * width,
            prepared_weights + size_t(row) * width, width) : qrt_sm121_subgroup::dot<4u>(
            inputs + size_t(token) * width, weights + size_t(row) * width, width);
    else if constexpr (Variant == 3u)
        value = qrt_sm121_scalar_projection::cooperative_dot(inputs + size_t(token) * width,
            weights + size_t(row) * width, width, eligible);
    else if constexpr (Variant == 2u)
        value = qrt_sm121_scalar_projection::dot<true>(inputs + size_t(token) * width,
            transposed, rows, row, width, eligible);
    else
        value = qrt_sm121_scalar_projection::dot<false>(inputs + size_t(token) * width,
            weights, rows, row, width, eligible);
    if (!(threadIdx.x & (lanes - 1u))) output[cell] = device_bf16_round_to_float(value);
}

void complete_scalar_projection() {
...
void run_scalar_projection_replays(DeviceBuffer<uint16_t>& dw, DeviceBuffer<uint16_t>& di,
    ...
    std::vector<uint16_t> prepared_w(weights.size(), kBf16Guard), prepared_i(inputs.size(), kBf16Guard);
    ...
    DeviceBuffer<uint16_t> dpw(prepared_w), dpi(prepared_i), dt(prepared_w);
    ...
    for (unsigned variant = 0u; variant < 4u; ++variant) {
        ...
        for (unsigned attempt = 0u; attempt < 2u; ++attempt) {
            ...
            if (variant == 0u) {
                hipLaunchKernelGGL(qrt_sm121_prepared_projection::prepare_rows_kernel, dim3(rows), dim3(256u), 0u, nullptr,
                    dw.data(), dpw.data(), dfw.data(), rows, width);
                hip_ok(hipGetLastError(), "scalar_prepare_weights");
                hipLaunchKernelGGL(qrt_sm121_prepared_projection::prepare_rows_kernel, dim3(tokens), dim3(256u), 0u, nullptr,
                ...
                hip_ok(hipGetLastError(), "scalar_weight_eligibility");
            ...
            if (variant == 2u) {
                hipLaunchKernelGGL(selected_hawkeye_transpose_weights_kernel,
                    dim3((width + 31u) / 32u, (rows + 31u) / 32u), dim3(32u, 8u), 0u, nullptr,
                    dw.data(), dt.data(), rows, width);
                hip_ok(hipGetLastError(), "scalar_weight_transpose");
            }
            const unsigned lanes = variant == 0u || variant == 3u ? 4u : 1u;
            const dim3 grid((unsigned(selected.size()) * lanes + 255u) / 256u);
#define QRT_SCALAR_REPLAY_CASE(v) if (variant == v) hipLaunchKernelGGL(scalar_projection_replay_kernel<v>, grid, dim3(256u), 0u, nullptr, dw.data(), di.data(), dpw.data(), dpi.data(), dt.data(), dfw.data(), dfi.data(), indices.data(), dout.data(), rows, width, unsigned(selected.size()))
...
#undef QRT_SCALAR_REPLAY_CASE
            hip_ok(hipGetLastError(), "scalar_replay_launch"); complete_scalar_projection();
        ...
        const auto check_flags = [&](const std::vector<uint16_t>& source, const std::vector<unsigned>& flags, unsigned count) {
            ...
            for (unsigned row = 0u; row < count; ++row) {
                ...
                for (unsigned k = 0u; k < width; ++k) {
                    ...
                    valid &= !(x & 0x7fffu) || (e >= 64u && e <= (variant == 0u ? 191u : 190u));
                }
        ...
        auto transpose = prepared_w; dt.read(transpose);
        if (variant >= 2u) for (unsigned row = 0u; row < rows; ++row) for (unsigned k = 0u; k < width; ++k)
            require(transpose[kGuard + size_t(k) * rows + row] == weights[kGuard + size_t(row) * width + k], "scalar transpose value changed");
        for (size_t i = 0u; i < kGuard; ++i) {
            ...
            require(transpose[i] == kBf16Guard && transpose[kGuard + size_t(rows) * width + i] == kBf16Guard, "scalar transpose redzone");
        }
        std::cout << "{\"type\":\"scalar_projection_real_replay\",\"variant\":" << variant
        ...
        require(!raw_bad && !bf16_bad, "scalar captured projection differs from control or GB10");
    }
}
} // namespace projection_safety_test
