...
#include "../../native/providers/ck_fmha/two_pass_packed_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
#define QRT_TWO_PASS_CASE(V,W) else if(variant==V)hipLaunchKernelGGL((qrt_two_pass_packed_qk::scores<W,4u>),grid,dim3(kThreads),0u,nullptr,qp,kp,qf,kf,out,start,count,stride,key_stride)
        QRT_TWO_PASS_CASE(3u,32u);
        QRT_TWO_PASS_CASE(4u,64u);
        QRT_TWO_PASS_CASE(5u,128u);
        else throw std::runtime_error("invalid two-pass QK variant");
#undef QRT_TWO_PASS_CASE
        check(hipGetLastError());
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("two-pass packed QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"two_pass_packed_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
