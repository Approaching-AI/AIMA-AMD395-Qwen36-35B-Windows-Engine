#include "../../native/providers/ck_fmha/matrix_consumer_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
template<IntegerRowKind Kind> void prepare(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(qrt_positive_integer_qk::prepare_rows<Kind,true>),
...
void staged_variant(unsigned variant,const Row* q,const Row* k,float* out,
    ...
    if(variant<2u) {
        ...
        if(!variant)
            hipLaunchKernelGGL((qrt_positive_integer_qk::scores<false>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else
            hipLaunchKernelGGL((qrt_positive_integer_qk::scores<true>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
    } else {
        const unsigned columns=variant==2u?16u:variant==4u?64u:32u;
        const dim3 grid((stride+columns-1u)/columns,kQueryHeads,(count+15u)/16u);
        if(variant==2u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<16u,true,true>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==3u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<32u,true,true>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==4u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<64u,true,true>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==5u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<32u,false,true>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==6u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<32u,false,false>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==7u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<32u,true,false>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid matrix/float variant");
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned sample=0u;sample<16u;++sample)for(unsigned side=0u;side<2u;++side) {
        ...
        for(unsigned col=0u;col<16u;++col) {
            ...
            expected.core.original[col]=(side?k:q)[guard+index];
        }
        qrt_positive_integer_qk::prepare(expected);
        expected.core.original[17]=uint16_t(qrt_sm121_matrix_float_fallback::eligible(expected.core.original));
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"matrix_float_qk_safety\",\"variant\":%u,\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",variant,c.tokens,c.start,c.count,c.mode,cells);
        std::fflush(stdout);
    }
...
float classify_sample(const uint16_t* q,const uint16_t* k,unsigned (&paths)[3]) {
    qrt_q1_moe_hawkeye::Value carry{0u,-133,false};
    for(unsigned group=0u;group<16u;++group) {
        Row a{},b{};int32_t parts[4]{};
        for(unsigned i=0u;i<16u;++i){a.core.original[i]=q[group*16u+i];b.core.original[i]=k[group*16u+i];}
        qrt_positive_integer_qk::prepare(a);qrt_positive_integer_qk::prepare(b);
        for(unsigned i=0u;i<16u;++i) {
            const uint16_t x=qrt_sm121_integer_core::encode(a.core.original[i],a.core.unit);
            const uint16_t y=qrt_sm121_integer_core::encode(b.core.original[i],b.core.unit);
            const int ah=int(x>>8u)-((x&0x8000u)?256:0),bh=int(y>>8u)-((y&0x8000u)?256:0);
            const int al=x&255u,bl=y&255u;
            parts[0]+=ah*bh;parts[1]+=ah*bl;parts[2]+=al*bh;parts[3]+=al*bl;
        }
        qrt_sm121_group16::AlignedSum sum;
        const auto path=qrt_sm121_matrix_float_fallback::sum(carry,a.core,b.core,parts,
            qrt_sm121_matrix_float_fallback::eligible(a.core.original)&&qrt_sm121_matrix_float_fallback::eligible(b.core.original),&sum);
        ++paths[unsigned(path)];
        const qrt_q1_moe_hawkeye::Value scaled{sum.value.magnitude,int16_t(sum.max_exponent-2),sum.value.negative};
        carry=qrt_sm121_group16::finish_accumulator(qrt_q1_moe_hawkeye::group_sum<26,-133>(&scaled,1u));
        carry=qrt_q1_moe_hawkeye::value_from_float(qrt_q1_moe_hawkeye::value_to_float(carry),-133);
    }
    return qrt_q1_moe_hawkeye::value_to_float(carry)*kExactScale;
}
...
void captured(const char* qfile,const char* kfile) {
    ...
    const auto keys_before=download<unsigned char>(kp,krows*sizeof(Row));
    ...
    size_t compared=0u;unsigned cpu_dots=0u;unsigned sampled_paths[3]{};
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned mode=0u;mode<variant_count;++mode) {
            ...
            for(unsigned sample=0u;sample<4u;++sample) {
                ...
                if(!mode) {
                    const float classified=classify_sample(q.data()+(size_t(start+row)*kQueryHeads+head)*kHeadDim,
                        k.data()+(size_t(key)*kKvHeads+head/(kQueryHeads/kKvHeads))*kHeadDim,sampled_paths);
                    if(bits(classified)!=bits(expected))throw std::runtime_error("sampled mixed arithmetic differs from wide CPU sum");
                }
                ++cpu_dots;
            }
        }
    ...
    unchanged(dq,q);unchanged(dk,k);unchanged(kp,keys_before);unchanged(dt,transposed_before);
    for(unsigned mode=0u;mode<variant_count;++mode) {
        const unsigned variant=variants[mode],columns=variant<2u?128u:variant==2u?16u:variant==4u?64u:32u;
        std::printf("{\"kind\":\"matrix_float_original_q7169\",\"variant\":%u,\"columns\":%u,\"scalar_chains_per_thread\":%u,\"positive_core\":%s,\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"scalar_float_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"query_and_encoding_ms\":%.6f,\"key_encoding_ms\":%.6f,\"maximum_completed_slab_ms\":%.6f,\"cpu_hybrid_sample_paths\":[%u,%u,%u],\"cpu_hybrid_sample_groups\":3648,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",variant,columns,columns/16u,(variant!=0u&&variant!=5u&&variant!=6u)?"true":"false",compared/variant_count,cpu_dots/variant_count,original_ms,transpose_ms,staged_ms[mode],key_encoding_ms,maximum_stage_ms[mode],sampled_paths[0],sampled_paths[1],sampled_paths[2]);
        std::fflush(stdout);
    }
...
int main(int argc,char** argv) {
    try {
        ...
        const Case cases[]={{17,0,8,0},{17,0,9,2},{33,0,16,0},{33,1,31,1},{1,0,1,0},{9,0,9,1},{35,3,17,2},{67,33,32,3},{67,64,3,4},
            {129,1,128,0},{7169,7041,128,1},{8192,8064,128,0},{8192,8191,1,2},{8193,8191,2,3},{17,0,9,5},{67,33,32,5}};
