...
#define main qrt_lifetime_prior_gdn_main
...
void lifetime_launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,
 ...
 require(variant<=2u,"invalid lifetime GDN variant");
 ...
 for(unsigned offset=0u;offset<count;offset+=1024u){
  ...
  hipLaunchKernelGGL(scalar::wu_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,
   k.data<uint16_t>()+size_t(offset)*2048u,v.data<uint16_t>()+size_t(offset)*4096u,beta.data<uint16_t>()+size_t(offset)*32u,inverse.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,w.data<uint16_t>()+size_t(offset)*4096u,actual_u+size_t(offset)*4096u,n,exp);check(hipGetLastError());
  hipLaunchKernelGGL((lifetime::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,
   k.data<uint16_t>()+size_t(offset)*2048u,actual_u+size_t(offset)*4096u,w.data<uint16_t>()+size_t(offset)*4096u,g.data<float>()+size_t(offset)*32u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,vn.data<uint16_t>()+size_t(offset)*4096u,state.data<float>(),n,exp);check(hipGetLastError());
#define QRT_LIFETIME_OUTPUT_ARGS q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,n,exp
  if(variant==1u){hipLaunchKernelGGL(scalar::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_LIFETIME_OUTPUT_ARGS);}
  else{hipLaunchKernelGGL(lifetime::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_LIFETIME_OUTPUT_ARGS);}
  check(hipGetLastError());
#undef QRT_LIFETIME_OUTPUT_ARGS
 }
}
...
void lifetime_run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table,const char* capture_root=nullptr){
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<6u;++position){
  ...
  const auto begin=std::chrono::steady_clock::now();lifetime_launch(variant,count,dq,dk,dv,db,dinv,dg,ds,du,dw,output,h,vn,state,table,alias);finish();
  if(attempt){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"lifetime GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"lifetime GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"lifetime GDN intermediate or alias differs");
...
int main(int argc,char** argv)try{
 ...
 if(captured)lifetime_run(!std::strcmp(argv[2],"q7169")?7169u:8192u,6u,3u,dt,table,argv[3]);else if(timing){lifetime_run(8192u,1u,3u,dt,table);lifetime_run(8192u,3u,3u,dt,table);}else for(unsigned count:{1u,63u,64u,65u,129u,1023u,1024u,1025u})for(unsigned mode=0u;mode<6u;++mode)lifetime_run(count,mode,0u,dt,table);
