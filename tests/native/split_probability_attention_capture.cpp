...
#include <array>
#define QRT_SPLIT_PROBABILITY_REUSE_HELPERS 1
#define main qrt_split_probability_safety_main
#include "split_probability_selftest.cpp"
#undef main
#undef QRT_SPLIT_PROBABILITY_REUSE_HELPERS

namespace {
namespace split = qrt_split_probability;
const char* probability_names[]={"online","staged_cta","split_global"};
...
void attention(const uint16_t* q,const uint16_t* transposed_key,const uint16_t* v,
    ...
    unsigned variant,float* probability_workspace,size_t capacity_words,const unsigned char* rcp,bool original_scores,Events* probability_events){
    ...
    if(variant==2u){
        check(hipError_t(split::launch(scores,probabilities,scales,probability_workspace,capacity_words,start,queries,stride,exp2,true,nullptr)));
    }else if(variant==1u){
        hipLaunchKernelGGL(blackwell_staged_probability_kernel,dim3(kQueryHeads,queries),dim3(256u),0u,nullptr,
...
__global__ void workspace_check(const uint32_t* workspace,size_t capacity,const float* scales,
    unsigned start,unsigned queries,unsigned stride,bool used,unsigned* bad){
    const size_t i=size_t(blockIdx.x)*blockDim.x+threadIdx.x;if(i>=capacity)return;
    const unsigned rows=queries*kQueryHeads,tiles=(stride+31u)/32u;
    const size_t entries=size_t(rows)*tiles,required=3u*entries;
    if(!used||i>=required){if(workspace[i]!=0xa5a5a5a5u)atomicAdd(bad,1u);return;}
    const unsigned part=unsigned(i/entries),index=unsigned(i%entries);
    const unsigned row=part==0u?index/tiles:index%rows,tile=part==0u?index%tiles:index/rows;
    if(tile>=(start+row/kQueryHeads+32u)/32u){if(workspace[i]!=0xa5a5a5a5u)atomicAdd(bad,1u);return;}
    const float value=__uint_as_float(workspace[i]);
    if(!isfinite(value)||(part&&value<0.0f))atomicAdd(bad,1u);
    if(part==1u&&workspace[i]!=__float_as_uint(scales[size_t(row)*(tiles+1u)+tile]))atomicAdd(bad,1u);
    if(part==0u&&tile&&value<__uint_as_float(workspace[i-1u]))atomicAdd(bad,1u);
}
void compare(AttentionOutputs& expected,AttentionOutputs& actual,Device& bad){
    ...
    finish();
    if(download<unsigned>(bad,1u)[0])throw std::runtime_error("complete split probability attention differs");
    actual.guards();
}
...
void capture(unsigned tokens,const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    const auto exp=read_table(exp_file,exp2_backend::table_bytes),rcp=read_table(rcp_file,qrt_sm121_attention_rcp::table_bytes);
    if(!exp2_backend::valid_layout(exp.data(),exp.size())||!qrt_sm121_attention_rcp::valid_layout(rcp.data(),rcp.size()))throw std::runtime_error("table layout mismatch");
    Guarded dq(q.size()*2u),dk(k.size()*2u),dv(v.size()*2u),dr(reference.size()*2u),de(exp.size()),dc(rcp.size()),workspace(size_t(3u)*query_batch*kQueryHeads*((tokens+31u)/32u)*4u);
    ...
    double samples[3][3]{},probability_ms[3][3]{};unsigned invalid_intervals[3]{},cpu_dots=0u;
    uint64_t score_slots=0u,scale_slots=0u,pv_candidates[3]{};
    for(unsigned start=0;start<tokens;start+=query_batch){
        ...
        expected.reset();
        attention(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,expected,start,queries,tokens,de.data(),0u,nullptr,0u,dc.data(),true,nullptr);finish();
        expected.guards();
        ...
        for(unsigned attempt=0;attempt<4u;++attempt)for(unsigned position=0;position<3u;++position){
            ...
            actual.reset();workspace.reset();finish();const auto begin=std::chrono::steady_clock::now();
            attention(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,actual,start,queries,tokens,de.data(),variant,workspace.as<float>(),workspace.bytes/4u,dc.data(),false,&events);finish();
            ...
            if(attempt){samples[variant][attempt-1u]+=ms;
                if(std::isfinite(probability)&&probability>=0.0f)probability_ms[variant][attempt-1u]+=probability;else {
                    ++invalid_intervals[variant];std::fprintf(stderr,"SPLIT_PROBABILITY_INVALID_INTERVAL variant=%u start=%u attempt=%u probability_ms=%.9g\n",variant,start,attempt,probability);}}
            compare(expected,actual,bad);
            hipLaunchKernelGGL(workspace_check,dim3((workspace.bytes/4u+255u)/256u),dim3(256u),0u,nullptr,
                workspace.as<uint32_t>(),workspace.bytes/4u,actual.tensor.scales.as<float>()+guard,start,queries,stride,variant==2u,bad.as<unsigned>());
            check(hipGetLastError());finish();workspace.guards();
            if(download<unsigned>(bad,1u)[0])throw std::runtime_error("split probability workspace mismatch");
    ...
    if(pv_candidates[0]!=pv_candidates[1]||pv_candidates[0]!=pv_candidates[2])throw std::runtime_error("PV candidate counts differ");
    for(unsigned variant=0;variant<3u;++variant){
        ...
        std::printf("{\"kind\":\"split_probability_attention_capture\",\"tokens\":%u,\"source_capture_tokens\":7169,\"repeated_rows\":%u,\"variant\":\"%s\",\"variant_id\":%u,\"score_probability_slots\":%llu,\"scale_slots\":%llu,\"output_cells\":%u,\"gb10_context_cells\":29364224,\"cpu_dots\":%u,\"pv_candidates\":%llu,\"completed_attention_samples_ms\":[%.9f,%.9f,%.9f],\"median_completed_attention_ms\":%.9f,\"valid_probability_samples_ms\":[%.9f,%.9f,%.9f],\"invalid_probability_intervals\":%u,\"common_preparation_ms\":%.9f,\"probability_workspace_bytes\":%llu,\"preparation_plus_median_ms\":%.9f,\"raw_bit_mismatches\":0,\"gb10_context_mismatches\":0,\"all_attempts_checked\":true,\"warmups_per_slab\":1,\"timed_attempts_per_slab\":3,\"original_qk_and_pv\":true,\"reference_is_compute_input\":false,\"workspace_guards_tails_and_alphas_pass\":true,\"redzones_and_unused_tails_pass\":true,\"immutable_inputs\":true,\"model_loaded\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",tokens,tokens-7169u,probability_names[variant],variant,(unsigned long long)score_slots,(unsigned long long)scale_slots,tokens*4096u,cpu_dots,(unsigned long long)pv_candidates[variant],samples[variant][0],samples[variant][1],samples[variant][2],sorted[1],probability_ms[variant][0],probability_ms[variant][1],probability_ms[variant][2],invalid_intervals[variant],prepared.ms+transpose_ms,(unsigned long long)(variant==2u?workspace.bytes:0u),prepared.ms+transpose_ms+sorted[1]);std::fflush(stdout);
    }
}
} // namespace
int main(int argc,char** argv)try{
    if(argc==3&&!std::strcmp(argv[1],"--selftest")){char* args[]={argv[0],argv[2]};return qrt_split_probability_safety_main(2,args);}
...
}catch(const std::exception& e){std::fprintf(stderr,"split_probability_capture_error=%s\n",e.what());return 2;}
