...
#include <thread>
#include <vector>
namespace scaled=qrt_sm121_pair_projection_products;
namespace projection=qrt_sm121_pair_projection;
namespace cases=qrt_strong_replay_cases;
constexpr unsigned guard=65u;
struct Result { float value;unsigned recovered,fallback; };
...
__global__ void execute(const scaled::Row* a,const scaled::Row* b,Result* out,uint32_t* trace,unsigned rows,unsigned width){
 ...
 if(!(threadIdx.x&(Lanes-1u)))out[row]={value,stats.recovered,stats.fallback};
}
...
void variant(unsigned rows,unsigned width,Device& a,Device& b,Device& pa,Device& pb,Device& out,Device& trace,
 const std::vector<uint16_t>& left,const std::vector<uint16_t>& right,const std::vector<uint32_t>& expected,const std::vector<unsigned>& recovered){
 constexpr unsigned Lanes=4u;
 ...
 for(unsigned row=0u;row<rows;++row){
  ...
  if(values[guard+row].recovered!=recovered[row] || values[guard+row].fallback!=width/2u-recovered[row])throw std::runtime_error("group path count differs from independent original-exponent predicate");
  floating+=values[guard+row].recovered;original+=values[guard+row].fallback;
 }
 ...
 std::printf("{\"kind\":\"pair_projection_safety\",\"lanes\":%u,\"staging_groups\":%u,\"rows\":%u,\"width\":%u,\"ordered_raw_carry_states\":%zu,\"recovered_pairs\":%u,\"fallback_pairs\":%u,\"raw_bit_mismatches\":0,\"unaligned_operands\":true,\"production_diagnostic_parity\":true,\"all_encoded_words_checked\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",Lanes,StagingGroups,rows,width,groups,floating,original);std::fflush(stdout);
 if(!floating || !original || floating+original!=groups*8u)throw std::runtime_error("missing numerical path coverage");
}
void run(unsigned rows,unsigned width){
 const size_t words=size_t(rows)*width,groups=words/16u;std::vector<uint16_t> left(words),right(words);std::vector<uint32_t> expected(groups*3u);std::vector<unsigned> recovered(rows,0u);
 for(unsigned row=0u;row<rows;++row){qrt_q1_moe_hawkeye::Value carry{0u,-133,false};for(unsigned group=0u;group<width/16u;++group){qrt_q1_moe_hawkeye::Value terms[17];terms[0]=carry;const size_t base=size_t(row)*width+group*16u;for(unsigned i=0u;i<16u;++i){const auto input=cases::input(row,group,i);left[base+i]=input.x;right[base+i]=input.y;terms[i+1u]=qrt_q1_moe_hawkeye::multiply_bf16(input.x,input.y,-133);}recovered[row]+=qrt_pair_projection_reference::expected(carry,left.data()+base,right.data()+base).recovered_pairs;carry=qrt_q1_moe_hawkeye::group_sum<26,-133>(terms,17u);const size_t out=base/16u*3u;expected[out]=carry.significand;expected[out+1u]=uint32_t(int32_t(carry.exponent));expected[out+2u]=unsigned(carry.negative);}}
 ...
 variant<1u>(rows,width,a,b,pa,pb,out,trace,left,right,expected,recovered);variant<2u>(rows,width,a,b,pa,pb,out,trace,left,right,expected,recovered);variant<4u>(rows,width,a,b,pa,pb,out,trace,left,right,expected,recovered);variant<8u>(rows,width,a,b,pa,pb,out,trace,left,right,expected,recovered);
}
