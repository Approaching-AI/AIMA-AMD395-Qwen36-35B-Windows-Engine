...
__global__ void prepare(const uint16_t* input, core::Row* output, OriginalRow* control, unsigned count) {
    ...
    core::prepare<Headroom>(result); output[row] = result;
    OriginalRow old{};
...
#pragma unroll
    for (unsigned i = 0u; i < 8u; ++i) {
        ...
        result.accepted=unsigned(core::recover<Headroom>(result.approximate,uint32_t(result.residue),&result.recovered));
...
#pragma unroll
            for(unsigned i=0u;i<8u;++i){int64_t value=0;const bool accepted=core::recover<Headroom>(parts.approximate[i],uint32_t(parts.residue[i]),&value);totals[i]+=accepted?uint64_t(value):0xdeadbeef395u;}
        }
    }
#pragma unroll
...
template<unsigned Headroom> void run() {
    ...
    for (unsigned row = 0u; row < expected_rows.size(); ++row) {
        ...
        core::prepare<Headroom>(expected_rows[row]); eligible_rows += unsigned(unit >= 0);
    ...
    for (size_t cell = 0u; cell < actual.size(); ++cell) {
        ...
        if (value.accepted && value.recovered != expected) {
            if (recovery_bad < 4u) std::printf("{\"kind\":\"byte_residue_core_counterexample\",\"headroom\":%u,\"cell\":%u,\"expected\":%lld,\"actual\":%lld,\"approximate\":%.12g,\"residue\":%d,\"accepted\":%u}\n", Headroom,index, static_cast<long long>(expected), static_cast<long long>(value.recovered), double(value.approximate), value.residue, value.accepted);
            ++recovery_bad;
        }
    }
    ...
    for(unsigned groups:{16u,64u,256u}) {
        ...
        for(unsigned variant=0u;variant<2u;++variant){
            ...
            std::printf("{\"kind\":\"byte_residue_core_sequence\",\"headroom\":%u,\"variant\":%u,\"groups\":%u,\"blocks\":1024,\"median_ms\":%.6f,\"samples_ms\":[%.6f,%.6f,%.6f],\"warmups\":1,\"measured_attempts\":3,\"cpu_integer_sequence_output_cells\":1048576,\"all_attempts_verified\":true,\"mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"canonical_carry_and_exceptions_included\":false,\"preparation_timed_separately\":true,\"performance_acceptance\":false,\"inference_acceptance\":false}\n",Headroom,variant,groups,sorted[1],samples[variant][0],samples[variant][1],samples[variant][2]);
        }
        std::fflush(stdout);
    }
}
