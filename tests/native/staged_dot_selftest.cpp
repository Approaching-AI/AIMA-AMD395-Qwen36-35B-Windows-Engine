...
int main() try {
    ...
    for (unsigned k : {16u, 48u, 80u, 128u, 512u, 2048u}) {
        ...
        for (unsigned groups : {1u, 4u, 8u}) {
            ...
            if (groups == 1u) hipLaunchKernelGGL(HIP_KERNEL_NAME(probe<16u, 1u>), grid, dim3(256u), 0u, nullptr, da, db, dout + 64u, cases, k);
            else if (groups == 4u) hipLaunchKernelGGL(HIP_KERNEL_NAME(probe<16u, 4u>), grid, dim3(256u), 0u, nullptr, da, db, dout + 64u, cases, k);
            else hipLaunchKernelGGL(HIP_KERNEL_NAME(probe<16u, 8u>), grid, dim3(256u), 0u, nullptr, da, db, dout + 64u, cases, k);
            ...
            std::printf("{\"kind\":\"sm121_staged_dot\",\"lanes\":%u,\"staging_groups\":%u,\"k\":%u,\"dots\":%u,\"bit_mismatches\":%u,\"ms\":%.6f,\"dpp_reduction\":%s,\"inference_acceptance\":false}\n", lanes, groups, k, cases, bad, ms, QRT_SM121_DPP_REDUCTION ? "true" : "false");
    ...
    return 0;
} catch (const std::exception& error) { std::fprintf(stderr, "staged_dot_selftest_error=%s\n", error.what()); return 2; }
