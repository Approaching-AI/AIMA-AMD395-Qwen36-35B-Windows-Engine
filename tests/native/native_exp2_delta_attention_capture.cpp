...
void attention(const uint16_t* q,const uint16_t* transposed_key,const uint16_t* v,
    ...
    const unsigned char* packed,const unsigned char* rcp,bool original_scores,Events* probability_events){
...
void compare(AttentionOutputs& expected,AttentionOutputs& actual,Device& bad){
    ...
    finish();
    if(download<unsigned>(bad,1u)[0])throw std::runtime_error("complete native EXP attention differs");
    actual.guards();
}
...
void capture(unsigned tokens,const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    double samples[2][3]{},probability_ms[2][3]{};unsigned invalid_intervals[2]{},cpu_dots=0u;
    uint64_t score_slots=0u,scale_slots=0u,pv_candidates[2]{};
    for(unsigned start=0;start<tokens;start+=query_batch){
        ...
        for(unsigned attempt=0;attempt<4u;++attempt)for(unsigned position=0;position<2u;++position){
            ...
            attention(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,actual,start,queries,tokens,de.data(),variant?dd.data():nullptr,dc.data(),false,&events);finish();
            ...
            if(attempt){samples[variant][attempt-1u]+=ms;
                if(std::isfinite(probability)&&probability>=0.0f)probability_ms[variant][attempt-1u]+=probability;else {
                    ++invalid_intervals[variant];std::fprintf(stderr,"NATIVE_EXP2_INVALID_INTERVAL variant=%u start=%u attempt=%u probability_ms=%.9g\n",variant,start,attempt,probability);}}
    ...
    if(pv_candidates[0]!=pv_candidates[1])throw std::runtime_error("PV candidate counts differ");
    for(unsigned variant=0;variant<2u;++variant){
        ...
        std::printf("{\"kind\":\"native_exp2_delta_attention_capture\",\"tokens\":%u,\"source_capture_tokens\":7169,\"repeated_rows\":%u,\"native_delta\":%s,\"score_probability_slots\":%llu,\"scale_slots\":%llu,\"output_cells\":%u,\"gb10_context_cells\":29364224,\"cpu_dots\":%u,\"pv_candidates\":%llu,\"completed_attention_samples_ms\":[%.9f,%.9f,%.9f],\"median_completed_attention_ms\":%.9f,\"valid_probability_samples_ms\":[%.9f,%.9f,%.9f],\"invalid_probability_intervals\":%u,\"common_preparation_ms\":%.9f,\"candidate_table_build_ms\":%.9f,\"preparation_plus_median_ms\":%.9f,\"raw_bit_mismatches\":0,\"gb10_context_mismatches\":0,\"all_attempts_checked\":true,\"warmups_per_slab\":1,\"timed_attempts_per_slab\":3,\"original_qk_and_pv\":true,\"reference_is_compute_input\":false,\"redzones_and_unused_tails_pass\":true,\"immutable_inputs\":true,\"model_loaded\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",tokens,tokens-7169u,variant?"true":"false",(unsigned long long)score_slots,(unsigned long long)scale_slots,tokens*4096u,cpu_dots,(unsigned long long)pv_candidates[variant],samples[variant][0],samples[variant][1],samples[variant][2],sorted[1],probability_ms[variant][0],probability_ms[variant][1],probability_ms[variant][2],invalid_intervals[variant],prepared.ms+transpose_ms,variant?build_ms:0.0,prepared.ms+transpose_ms+sorted[1]+(variant?build_ms:0.0));std::fflush(stdout);
    }
}
} // namespace
...
}catch(const std::exception& e){std::fprintf(stderr,"native_exp2_delta_capture_error=%s\n",e.what());return 2;}
