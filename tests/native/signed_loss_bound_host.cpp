#include "../../native/providers/moe_accumulator/sm121_signed_loss_bound.h"
...
void dot(const uint16_t* left,const uint16_t* right,unsigned width,unsigned mode,Audit& audit,
    ...
    base::State legacy;loss::State directed;original::Value carry{0u,-133,false};
    for(unsigned start=0u;start<width;start+=64u){
        ...
        const bool old_accept=base::certified(legacy),new_accept=loss::certified(directed);
        check(!old_accept||new_accept,"lost old certificate");
        if(new_accept)check(scalar::bf16(expected)==scalar::bf16(directed.center),"false certificate");
        ...
        if(absolute>0.0f){audit.legacy_width+=double(2.0f*legacy.error)/double(absolute);
            audit.revised_width+=(double(directed.lower_error)+double(directed.upper_error))/double(absolute);}
...
void generated(){
    std::array<uint16_t,512u> left{},right{};
    ...
    Audit audit;
    constexpr unsigned samples=16384u;
    for(unsigned sample=0u;sample<samples;++sample){
        ...
        for(unsigned mode=0u;mode<3u;++mode)dot(left.data(),right.data(),512u,mode,audit);
    }
    ...
    print("signed_loss_bound_generated",audit,samples,512u,3u);
    std::puts("{\"kind\":\"signed_loss_metadata\",\"bf16_encodings\":65536,\"support_masks\":65536,\"all_pass\":true}");
}
...
int main(int argc,char** argv){try{
    ...
    for(unsigned mode=0u;mode<3u;++mode){std::printf("{\"kind\":\"signed_loss_capture_mode\",\"mode\":%u}\n",mode);print("signed_loss_bound_capture",audits[mode],samples,width,1u);}
    return 0;
