...
#include "../../native/providers/ck_fmha/narrow_qk_lookahead.h"
...
struct NarrowDomain {
    ...
    Guarded query,key,statistics,rejections,metadata_query,metadata_key,byte_query,byte_key;
    ...
    NarrowDomain(const uint16_t* q,const uint16_t* k,const std::vector<uint16_t>& hq,
        ...
        query(size_t(tokens)*16u*4u),key(size_t(tokens)*2u*4u),statistics(8u),rejections(4u),
    ...
    qrt_narrow_qk_lookahead::Workspace plan_workspace(Prepared& p,unsigned tokens){
        return {byte_workspace(p,tokens),rejections.as<unsigned>()};
    }
    void verify(){
        rejections.guards();
...
void producer(const uint16_t* q,const uint16_t* kt,const uint16_t* v,const uint16_t* vt,
    ...
    check(hipMemsetAsync(domain.rejections.data(),0,4u,nullptr));
    if(!variant){
        ...
        check(hipError_t(qrt_byte_exponent_qk::launch<4u>(&workspace,q,kt,scores,nullptr,start,count,stride,tokens)));
    }else{
        const auto workspace=domain.plan_workspace(prepared,tokens);
        const auto launch=variant==2u?qrt_narrow_qk_lookahead::launch<2u>:
            variant==3u?qrt_narrow_qk_lookahead::launch<4u>:
            variant==4u?qrt_narrow_qk_lookahead::launch<4u,true>:nullptr;
        if(!launch)throw std::runtime_error("invalid narrow QK lookahead variant");
...
void run_safety(const unsigned char* exp,const unsigned char* packed,const unsigned char* rcp){
    ...
    unsigned cases=0u;uint64_t forced_replayed=0u;
    for(const auto shape:shapes)for(unsigned mode=0u;mode<10u;++mode){
        ...
        for(unsigned variant=0u;variant<5u;++variant){
            ...
            unsigned rejects=0u,counts[2]{};
            check(hipMemcpy(&rejects,domain.rejections.data(),4u,hipMemcpyDeviceToHost));
            check(hipMemcpy(counts,domain.statistics.data(),8u,hipMemcpyDeviceToHost));
            if(variant<2u&&rejects)throw std::runtime_error("control unexpectedly has plan rejections");
            if(variant==4u){
                if(rejects!=counts[1]*1024u)throw std::runtime_error("forced mismatch did not replay every fast score");
                forced_replayed+=rejects;
            }
        }
        ...
        std::fprintf(stderr,"NARROW_QK_LOOKAHEAD_SAFETY tokens=%u start=%u queries=%u mode=%u pass=1\n",n,start,count,mode);
    }
    if(!forced_replayed)throw std::runtime_error("forced mismatch was never exercised");
    std::printf("{\"kind\":\"narrow_qk_lookahead_safety\",\"cases\":%u,\"shapes\":8,\"data_modes\":10,\"retained_control_callback_and_isolated_candidate\":true,\"pre_replay_native_surfaces_and_complete_replay\":true,\"domain_extremes_and_nearby_rejections\":true,\"span31_and_span32_boundary\":true,\"cpu_metadata_checked\":true,\"forced_wrong_predictions_checked\":true,\"forced_replayed_scores\":%llu,\"raw_bit_mismatches\":0,\"guards_pass\":true,\"immutable_inputs\":true}\n",cases,(unsigned long long)forced_replayed);
}
...
void run_capture(unsigned tokens,const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    double samples[4][3]{};uint64_t candidates[4]{},fast_tiles[4]{},slow_tiles[4]{},rejected_scores[4]{},score_cells=0u;unsigned cpu_dots=0u;
    for(unsigned start=0;start<tokens;start+=query_batch){
        ...
        for(unsigned attempt=0;attempt<4u;++attempt)for(unsigned position=0;position<4u;++position){
            ...
            if(!attempt){unsigned n=0;check(hipMemcpy(&n,actual.count.data(),4u,hipMemcpyDeviceToHost));candidates[variant]+=n;
                ...
                unsigned rejected=0u;check(hipMemcpy(&rejected,domain.rejections.data(),4u,hipMemcpyDeviceToHost));
                rejected_scores[variant]+=rejected;}
        }
    ...
    for(unsigned variant=0;variant<4u;++variant){
        ...
        std::printf("{\"kind\":\"narrow_qk_lookahead_capture\",\"tokens\":%u,\"source_capture_tokens\":7169,\"repeated_rows\":%u,\"variant\":%u,\"query_cells\":%u,\"key_cells\":%u,\"narrow_tiles\":%llu,\"original_tiles\":%llu,\"plan_groups\":%u,\"plan_rejected_scores\":%llu,\"domain_classification_ms\":%.9f,\"retained_control_callback_and_isolated_candidate\":true,\"pre_replay_native_surfaces_checked_on_warmup\":true,\"cpu_metadata_checked\":true,\"query_batch\":128,\"score_slots\":%llu,\"output_cells\":%u,\"gb10_context_cells\":29364224,\"cpu_dots\":%u,\"pv_candidates\":%llu,\"completed_attention_samples_ms\":[%.9f,%.9f,%.9f],\"median_completed_attention_ms\":%.9f,\"common_preparation_ms\":%.9f,\"raw_bit_mismatches\":0,\"gb10_context_mismatches\":0,\"all_attempts_checked\":true,\"warmups_per_slab\":1,\"timed_attempts_per_slab\":3,\"original_qk_and_pv\":true,\"reference_is_compute_input\":false,\"redzones_and_unused_tails_pass\":true,\"immutable_inputs\":true,\"model_loaded\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",tokens,tokens-7169u,variant,variant==1u?4u:2u,variant>=2u?2u:4u,(unsigned long long)fast_tiles[variant],(unsigned long long)slow_tiles[variant],variant==2u?2u:variant==3u?4u:0u,(unsigned long long)rejected_scores[variant],variant?domain.byte_ms:domain.ms,(unsigned long long)score_cells,tokens*4096u,cpu_dots,(unsigned long long)candidates[variant],samples[variant][0],samples[variant][1],samples[variant][2],sorted[1],prepared.ms+transpose_ms);
    }
}
} // namespace
...
}catch(const std::exception& e){std::fprintf(stderr,"narrow_qk_lookahead_error=%s\n",e.what());return 2;}
