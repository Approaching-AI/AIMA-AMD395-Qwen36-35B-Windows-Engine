...
#include <thread>
#include <vector>
namespace scaled=qrt_sm121_embedded_half;
namespace projection=qrt_sm121_embedded_half_projection;
...
__global__ void execute(const scaled::Row* a,const scaled::Row* b,const uint16_t* oa,const uint16_t* ob,const unsigned* fa,const unsigned* fb,Result* out,uint32_t* trace,unsigned rows,unsigned width){
 constexpr unsigned Lanes=4u;
 const unsigned row=(blockIdx.x*blockDim.x+threadIdx.x)/Lanes;if(row>=rows)return;
 projection::Stats stats;const float value=projection::dot<StagingGroups,Audit>(a+size_t(row)*(width/16u),b+size_t(row)*(width/16u),oa+size_t(row)*width,ob+size_t(row)*width,fa+size_t(row)*scaled::flag_words(width),fb+size_t(row)*scaled::flag_words(width),width,
  Audit?trace+size_t(row)*(width/16u)*3u:nullptr,Audit?&stats:nullptr);
 if(!(threadIdx.x&(Lanes-1u)))out[row]={value,stats.compact_groups,stats.original_groups};
}
template<unsigned StagingGroups>
void variant(unsigned rows,unsigned width,bool ordinary,Device& a,Device& b,Device& pa,Device& pb,Device& fa,Device& fb,Device& out,Device& trace,
 ...
 hipLaunchKernelGGL((execute<StagingGroups,true>),dim3((rows*Lanes+255u)/256u),dim3(256u),0u,nullptr,pa.data<scaled::Row>(),pb.data<scaled::Row>(),a.data<uint16_t>(),b.data<uint16_t>(),fa.data<unsigned>(),fb.data<unsigned>(),out.data<Result>(),trace.data<uint32_t>(),rows,width);check(hipGetLastError());finish();
 ...
 hipLaunchKernelGGL((execute<StagingGroups,false>),dim3((rows*Lanes+255u)/256u),dim3(256u),0u,nullptr,pa.data<scaled::Row>(),pb.data<scaled::Row>(),a.data<uint16_t>(),b.data<uint16_t>(),fa.data<unsigned>(),fb.data<unsigned>(),out.data<Result>(),trace.data<uint32_t>(),rows,width);check(hipGetLastError());finish();
 ...
 for(unsigned side=0u;side<2u;++side){
  ...
  for(size_t group=0u;group<groups;++group){scaled::Row reference;const bool supported=scaled::prepare(input.data()+group*16u,&reference);if(std::memcmp(&reference,&packed[guard+group],sizeof(reference)))throw std::runtime_error("operand encoding differs or changed");for(unsigned i=0u;i<16u;++i)if(scaled::original(packed[guard+group],i,supported)!=input[group*16u+i])throw std::runtime_error("lossless operand roundtrip failed");}
  const unsigned flag_words=scaled::flag_words(width);const auto flags=read<unsigned>(side?fb:fa,size_t(rows)*flag_words);guards(flags);
  for(unsigned row=0u;row<rows;++row){
   std::vector<unsigned> expected_flags(flag_words,0u);bool all=true;
   for(unsigned group=0u;group<width/16u;++group){const bool valid=eligible(input.data()+size_t(row)*width+group*16u);all&=valid;if(valid)expected_flags[1u+group/32u]|=1u<<(group&31u);}
   expected_flags[0]=all;
   if(std::memcmp(flags.data()+guard+size_t(row)*flag_words,expected_flags.data(),flag_words*4u))throw std::runtime_error("support bitmap or padding differs");
  }

 }
 std::printf("{\"kind\":\"embedded_half_projection_safety\",\"lanes\":%u,\"staging_groups\":%u,\"rows\":%u,\"width\":%u,\"ordinary_inputs\":%s,\"ordered_raw_carry_states\":%zu,\"transformed_groups\":%u,\"original_groups\":%u,\"raw_bit_mismatches\":0,\"unaligned_operands\":true,\"production_diagnostic_parity\":true,\"all_encoded_words_checked\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",Lanes,StagingGroups,rows,width,ordinary?"true":"false",groups,floating,original);std::fflush(stdout);
 if(!floating || (!ordinary && !original) || floating+original!=groups)throw std::runtime_error("missing numerical path coverage");
}
void run(unsigned rows,unsigned width,bool ordinary=false){
 ...
 for(unsigned row=0u;row<rows;++row){qrt_q1_moe_hawkeye::Value carry{0u,-133,false};for(unsigned group=0u;group<width/16u;++group){qrt_q1_moe_hawkeye::Value terms[17];terms[0]=carry;const size_t base=size_t(row)*width+group*16u;for(unsigned i=0u;i<16u;++i){auto input=cases::input(row,group,i);if(ordinary){const unsigned a=cases::random_word(unsigned(base+i)^0x3958192u),b=cases::random_word(a^0x8192395u);input={uint16_t((a&0x807fu)|((120u+a%12u)<<7u)),uint16_t((b&0x807fu)|((119u+b%14u)<<7u))};if((base+i)%37u==0u)input.x=uint16_t(a&0x8000u);}left[base+i]=input.x;right[base+i]=input.y;terms[i+1u]=qrt_q1_moe_hawkeye::multiply_bf16(input.x,input.y,-133);}transformed[row]+=eligible(left.data()+base)&&eligible(right.data()+base);carry=qrt_q1_moe_hawkeye::group_sum<26,-133>(terms,17u);const size_t out=base/16u*3u;expected[out]=carry.significand;expected[out+1u]=uint32_t(int32_t(carry.exponent));expected[out+2u]=unsigned(carry.negative);}}
 const size_t flag_count=size_t(rows)*scaled::flag_words(width);
 Device fa((flag_count+2u*guard)*4u),fb((flag_count+2u*guard)*4u);
 ...
 check(projection::prepare(a.data<uint16_t>(),words,pa.data<scaled::Row>(),groups,fa.data<unsigned>(),flag_count,rows,width,nullptr));
 check(projection::prepare(b.data<uint16_t>(),words,pb.data<scaled::Row>(),groups,fb.data<unsigned>(),flag_count,rows,width,nullptr));finish();
 variant<2u>(rows,width,ordinary,a,b,pa,pb,fa,fb,out,trace,left,right,expected,transformed);
 variant<4u>(rows,width,ordinary,a,b,pa,pb,fa,fb,out,trace,left,right,expected,transformed);
}
