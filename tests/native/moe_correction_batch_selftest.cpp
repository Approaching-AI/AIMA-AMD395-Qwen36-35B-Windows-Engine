...
void compare_routed_compaction(uint32_t tokens, uint32_t mode,
    ...
    for (uint32_t compact = 0u; compact < (expert_order ? 7u : (partition || scaled_fallback || staged_half) ? 6u : prevalidated ? 5u : float_replay ? 4u : prepared ? 3u : 2u); ++compact) {
        ...
        hip_ok(launch_moe_routed_correction<false>(
            ...
            blocks, stream, MoeL2::Input, MoeL2::RoutedGateUp, nullptr, nullptr, nullptr, dn.data(), di.data(), dw.data(), did.data(), da.data(), dl.data(),
        ...
        hip_ok(launch_moe_routed_correction<false>(
            ...
            MoeL2::RoutedActivated, MoeL2::RoutedDown, nullptr, nullptr, nullptr, dd.data(), dt.data(), did.data(), da.data(), ddw.data(), routes, radius, exponent), "compaction down");
    ...
    try {
        ...
        if (argc == 2 && std::strcmp(argv[1], "--shared-prevalidated") == 0) {
            for (bool down : {false, true}) {
                ...
                moe_batch_test::compare_shared_prevalidated(129u, down, 1u);
                moe_batch_test::compare_shared_prevalidated(513u, down, 2u);
                moe_batch_test::compare_shared_prevalidated(8192u, down, 0u);
            }
            return 0;
        }
