...
#include "../../native/providers/ck_fmha/matrix_interval_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
constexpr unsigned variants[] = {0u, 16u, 32u, 64u, 132u};
...
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_positive_integer_qk::prepare_rows<Kind,true>),
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            for(unsigned c=0u;c<16u;++c)
                value.core.original[c]=input[integer_row_input_index(kind,row,c,tokens,start,count)];
            qrt_positive_integer_qk::prepare(value);
            value.core.original[17]=uint16_t(qrt_sm121_matrix_float_fallback::eligible(value.core.original));
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        const unsigned columns=variant==132u?32u:variant;
        const dim3 grid((stride+columns-1u)/columns,kQueryHeads,(count+15u)/16u);
        if(variant==16u)
            hipLaunchKernelGGL((qrt_matrix_interval_qk::scores<16u>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==32u)
            hipLaunchKernelGGL((qrt_matrix_interval_qk::scores<32u>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==64u)
            hipLaunchKernelGGL((qrt_matrix_interval_qk::scores<64u>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==132u)
            hipLaunchKernelGGL((qrt_matrix_interval_qk::scores<32u,false>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid matrix interval QK variant");
    }
    check(hipGetLastError());
}
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("matrix interval QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"matrix_interval_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void classify_sample(const uint16_t* a,const uint16_t* b,unsigned (&counts)[4]) {
    qrt_q1_moe_hawkeye::Value carry{0u,-133,false};
    for(unsigned base=0u;base<kHeadDim;base+=16u) {
        qrt_sm121_integer_core::Row left{},right{};
        qrt_q1_moe_hawkeye::Value values[17];values[0]=carry;
        for(unsigned i=0u;i<16u;++i) {
            left.original[i]=a[base+i];right.original[i]=b[base+i];
            values[i+1u]=qrt_q1_moe_hawkeye::multiply_bf16(a[base+i],b[base+i],-133);
        }
        qrt_sm121_integer_core::prepare(left);qrt_sm121_integer_core::prepare(right);
        int64_t mathematical=0;
        for(unsigned i=0u;i<16u;++i)
            mathematical+=int64_t(qrt_sm121_integer_core::signed_core(a[base+i],left.unit))*
                qrt_sm121_integer_core::signed_core(b[base+i],right.unit);
        qrt_sm121_group16::AlignedSum sum;
        const bool old=left.unit>=0&&right.unit>=0&&!((left.exceptions|right.exceptions)&left.nonzero&right.nonzero)&&
            qrt_sm121_integer_parts::sum_exact_integer_product(carry,mathematical,left.unit,left.maximum,
                right.unit,right.maximum,&sum,left.trailing,right.trailing);
        qrt_sm121_integer_interval::Bounds bounds;
        qrt_q1_moe_hawkeye::Value result;
        const bool eligible=qrt_sm121_integer_interval::bounds(carry,left,right,mathematical,&bounds);
        const bool accepted=qrt_sm121_integer_interval::accumulate(carry,left,right,mathematical,&result);
        ++counts[old?0u:accepted?1u:eligible?2u:3u];
        carry=qrt_q1_moe_hawkeye::group_sum<26,-133>(values,17u);
        if(accepted&&!qrt_sm121_integer_interval::same(qrt_sm121_group16::finish_accumulator(carry),
            qrt_sm121_group16::finish_accumulator(result)))throw std::runtime_error("captured CPU interval is incorrect");
    }
}
void captured(const char* qfile,const char* kfile) {
    ...
    size_t compared=0u;unsigned cpu_dots=0u,classification[4]{};
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned position=0u;position<variant_count;++position) {
            ...
            for(unsigned sample=0u;sample<4u;++sample) {
                ...
                if(!variant)classify_sample(q.data()+guard+(size_t(start+row)*kQueryHeads+head)*kHeadDim,
                    k.data()+guard+(size_t(key)*kKvHeads+head/(kQueryHeads/kKvHeads))*kHeadDim,classification);
                ++cpu_dots;
            }
        }
    }
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"matrix_interval_original_q7169\",\"variant\":%u,\"query_rows\":16,\"key_columns\":%u,\"tokens\":7169,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"cpu_sample_paths_old_added_uncertain_unsupported\":[%u,%u,%u,%u],\"cpu_sample_groups\":3648,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,variant==132u?32u:variant?variant:16u,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
            preparation_ms,sorted[1],sorted[1]+preparation_ms,samples[mode][0],samples[mode][1],samples[mode][2],maximum_stage_ms[mode],classification[0],classification[1],classification[2],classification[3]);
        std::fflush(stdout);
    }
}
