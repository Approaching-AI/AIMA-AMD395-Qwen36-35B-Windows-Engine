...
#include "../../native/providers/gdn/exponent_mask_matrices.h"
#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstdio>
...
namespace compact=qrt_fla_exponent_mask;
...
struct PhaseEvents{
 hipEvent_t events[32]{};bool enabled;unsigned segments;
 explicit PhaseEvents(bool active,unsigned count):enabled(active),segments(count){require(segments && segments<=8u,"phase segment count");if(enabled)for(unsigned i=0u;i<segments*4u;++i)check(hipEventCreate(&events[i]));}
 ...
 float elapsed(unsigned phase){float total=0.0f;if(enabled)for(unsigned i=0u;i<segments;++i){float ms=0.0f;check(hipEventElapsedTime(&ms,events[i*4u+phase],events[i*4u+phase+1u]));require(std::isfinite(ms)&&ms>=0.0f,"invalid phase duration");total+=ms;}return total;}
};
...
void launch(unsigned variant,unsigned total_count,Device& q,Device& k,Device& v,Device& beta,Device& inverse,Device& g,Device& scores,Device& u,Device& w,Device& output,Device& h,Device& vn,Device& state,Device& table,bool alias,PhaseEvents& phases){
 for(unsigned offset=0u;offset<total_count;offset+=1024u){
 const unsigned count=std::min(1024u,total_count-offset),phase_base=(offset/1024u)*4u;
 const unsigned chunks=(count+63u)/64u;const auto* exp=reinterpret_cast<unsigned char*>(table.data<uint32_t>());
 auto* actual_u=alias?v.data<uint16_t>()+size_t(offset)*4096u:u.data<uint16_t>()+size_t(offset)*4096u;
 phases.mark(phase_base+0u);
 if(!variant || variant==3u){
  hipLaunchKernelGGL(scalar::wu_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,k.data<uint16_t>()+size_t(offset)*2048u,v.data<uint16_t>()+size_t(offset)*4096u,beta.data<uint16_t>()+size_t(offset)*32u,inverse.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,w.data<uint16_t>()+size_t(offset)*4096u,actual_u,count,exp);check(hipGetLastError());
  phases.mark(phase_base+1u);
  hipLaunchKernelGGL((scalar::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,k.data<uint16_t>()+size_t(offset)*2048u,actual_u,w.data<uint16_t>()+size_t(offset)*4096u,g.data<float>()+size_t(offset)*32u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,vn.data<uint16_t>()+size_t(offset)*4096u,state.data<float>(),count,exp);check(hipGetLastError());
  phases.mark(phase_base+2u);
  if(!variant){
  hipLaunchKernelGGL(scalar::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,count,exp);check(hipGetLastError());
  }else{
  hipLaunchKernelGGL((compact::output_kernel<false,32u>),dim3(32u,32u,chunks),dim3(256u),0u,nullptr,q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,count,exp);check(hipGetLastError());
  }
  phases.mark(phase_base+3u);
 }else{
  hipLaunchKernelGGL(compact::wu_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,k.data<uint16_t>()+size_t(offset)*2048u,v.data<uint16_t>()+size_t(offset)*4096u,beta.data<uint16_t>()+size_t(offset)*32u,inverse.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,w.data<uint16_t>()+size_t(offset)*4096u,actual_u,count,exp);check(hipGetLastError());
  phases.mark(phase_base+1u);
  hipLaunchKernelGGL((compact::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,k.data<uint16_t>()+size_t(offset)*2048u,actual_u,w.data<uint16_t>()+size_t(offset)*4096u,g.data<float>()+size_t(offset)*32u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,vn.data<uint16_t>()+size_t(offset)*4096u,state.data<float>(),count,exp);check(hipGetLastError());
  phases.mark(phase_base+2u);
  if(variant==1u){
  hipLaunchKernelGGL((compact::output_kernel<true,64u>),dim3(16u,32u,chunks),dim3(256u),0u,nullptr,q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,count,exp);check(hipGetLastError());
  }else{
  hipLaunchKernelGGL((compact::output_kernel<true,32u>),dim3(32u,32u,chunks),dim3(256u),0u,nullptr,q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,count,exp);check(hipGetLastError());
  }
  phases.mark(phase_base+3u);
...
void run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table){
 ...
 double phase_samples[2][4][3][3]{};PhaseEvents phases(measured!=0u,(count+1023u)/1024u);
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<8u;++position){
  ...
  dv.reset();dv.upload(v);du.reset();dw.reset();output.reset();state.reset();state.upload(seed);h.reset();vn.reset();finish();
  const auto begin=std::chrono::steady_clock::now();launch(variant,count,dq,dk,dv,db,dinv,dg,ds,du,dw,output,h,vn,state,table,alias,phases);finish();
  if(attempt){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"exponent-mask GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"exponent-mask GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"exponent-mask GDN intermediate or alias differs");
 ...
 for(unsigned alias=0u;alias<2u;++alias)for(unsigned variant=0u;variant<4u;++variant){
  ...
  std::printf("{\"kind\":\"exponent_mask_gdn_component\",\"tokens\":%u,\"mode\":%u,\"state_segment_tokens\":1024,\"segments\":%u,\"variant\":%u,\"lanes\":%u,\"u_aliases_v\":%s,\"output_cells\":%zu,\"state_cells\":%zu,\"checkpoint_cells\":%zu,\"wu_and_residual_cells\":%zu,\"independent_cpu_dots\":%zu,\"warmups\":1,\"measured_attempts\":%u,\"complete_preparation_wu_state_output_ms\":%.6f,\"samples_ms\":[%.6f,%.6f,%.6f],\"phase_events_enabled\":%s,\"phase_event_ms\":{",count,mode,(count+1023u)/1024u,variant,1u,alias?"true":"false",large,state_cells,checkpoints,large*3u,cpu_dots,measured,sorted[1],samples[alias][variant][0],samples[alias][variant][1],samples[alias][variant][2],measured?"true":"false");
...
int main(int argc,char** argv)try{
 ...
 if(timing)run(8192u,1u,3u,dt,table);else for(unsigned count:{1u,63u,64u,65u,129u,1024u})for(unsigned mode=0u;mode<3u;++mode)run(count,mode,0u,dt,table);
