...
namespace projection=qrt_sm121_dominant_half_projection;
namespace cases=qrt_strong_replay_cases;
constexpr unsigned guard=65u;
struct Result { float value;unsigned transformed,original,dominated; };
...
__global__ void execute(const scaled::Row* a,const scaled::Row* b,Result* out,uint32_t* trace,unsigned rows,unsigned width){
 ...
 if(!(threadIdx.x&(Lanes-1u)))out[row]={value,stats.transformed,stats.original,stats.dominated};
}
...
void variant(unsigned rows,unsigned width,bool ordinary,Device& a,Device& b,Device& pa,Device& pb,Device& out,Device& trace,
 const std::vector<uint16_t>& left,const std::vector<uint16_t>& right,const std::vector<uint32_t>& expected,const std::vector<unsigned>& transformed,const std::vector<unsigned>& dominated){
 constexpr unsigned Lanes=4u;
 ...
 unsigned floating=0u,original=0u,fast=0u;
 for(unsigned row=0u;row<rows;++row){
  ...
  if(values[guard+row].dominated!=dominated[row])throw std::runtime_error("carry dominance differs from original BF16 bound");
  floating+=values[guard+row].transformed;original+=values[guard+row].original;fast+=values[guard+row].dominated;
 }
...
void run(unsigned rows,unsigned width,bool ordinary=false){
 ...
 variant<2u>(rows,width,ordinary,a,b,pa,pb,out,trace,left,right,expected,transformed,dominated);
 variant<4u>(rows,width,ordinary,a,b,pa,pb,out,trace,left,right,expected,transformed,dominated);
}
