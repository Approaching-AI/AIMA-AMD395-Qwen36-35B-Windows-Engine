...
constexpr unsigned rows = 65536u, guard = 64u;
__global__ void compare(cases::Result* output, uint32_t* traces) {
    const unsigned row = blockIdx.x * blockDim.x + threadIdx.x;
    if (row < rows) output[row] = cases::candidate(row, traces + row * 16u);
}
...
int main() try {
    ...
    for (unsigned row = 0u; row < rows; ++row) {
        ...
        const auto result = results[row + guard]; const auto expected = cases::reference(row, expected_groups);
    ...
    std::printf("{\"kind\":\"scalar_float_alignment\",\"dots\":65536,\"ordered_groups\":1048576,\"raw_bit_mismatches\":%u,\"group_mismatches\":%u,\"float_groups\":%u,\"fallback_groups\":%u,\"redzones_pass\":%s,\"immutable_inputs\":true,\"inference_acceptance\":false}\n", mismatches, group_mismatches, accepted, fallback, guards ? "true" : "false");
