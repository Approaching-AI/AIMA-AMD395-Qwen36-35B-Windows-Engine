...
namespace projection=qrt_sm121_scaled_half_projection;
...
template<unsigned Lanes,bool Audit>
__global__ void execute(const scaled::Row* a,const scaled::Row* b,Result* out,uint32_t* trace,unsigned rows,unsigned width){
 ...
 projection::Stats stats;const float value=projection::dot<Lanes,Audit>(a+size_t(row)*(width/16u),b+size_t(row)*(width/16u),width,
...
void variant(unsigned rows,unsigned width,Device& a,Device& b,Device& pa,Device& pb,Device& out,Device& trace,
 ...
 hipLaunchKernelGGL((execute<Lanes,true>),dim3((rows*Lanes+255u)/256u),dim3(256u),0u,nullptr,pa.data<scaled::Row>(),pb.data<scaled::Row>(),out.data<Result>(),trace.data<uint32_t>(),rows,width);check(hipGetLastError());finish();
 ...
 hipLaunchKernelGGL((execute<Lanes,false>),dim3((rows*Lanes+255u)/256u),dim3(256u),0u,nullptr,pa.data<scaled::Row>(),pb.data<scaled::Row>(),out.data<Result>(),trace.data<uint32_t>(),rows,width);check(hipGetLastError());finish();
...
void run(unsigned rows,unsigned width){
 ...
 variant<1u>(rows,width,a,b,pa,pb,out,trace,left,right,expected,transformed);variant<4u>(rows,width,a,b,pa,pb,out,trace,left,right,expected,transformed);
}
