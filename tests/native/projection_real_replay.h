...
void run_real_qkv(const char *input_path, const char *weight_path, const char *reference_path,
    ...
    const bool coarse_interval = coarse_guard_model || (coarse_option && !std::strcmp(coarse_option,"1"));
    ...
    const char* parallel_transfer_option = std::getenv("QRT_PROJECTION_SAFETY_PARALLEL_TRANSFER_REPLAY");
    const bool parallel_transfer_replay = parallel_transfer_option && !std::strcmp(parallel_transfer_option,"1");
    ...
    const char* narrow_half_option = std::getenv("QRT_PROJECTION_SAFETY_NARROW_HALF_REPLAY");
    const bool narrow_half_replay = narrow_half_option && !std::strcmp(narrow_half_option,"1");
    ...
    if (coarse_interval) {
        if(coarse_guard_model)run_coarse_interval_projection<true>(dw,di,dout,weights,inputs,reference,output,selected_indices,rows,tokens,k,ppb);
        else run_coarse_interval_projection<false>(dw,di,dout,weights,inputs,reference,output,selected_indices,rows,tokens,k,ppb);
        return;
    }
    if (exponent_loss) {
    ...
    if (parallel_transfer_replay) {
        run_parallel_transfer_replays(dw,di,dout,weights,inputs,reference,output,
            selected_indices,rows,tokens,k);
        return;
    }
    if (transfer_replay) {
    ...
    if (resident_input_replay) {
        run_resident_input_replays(dw,di,dout,weights,inputs,reference,output,
            selected_indices,rows,tokens,k);
        return;
    }
    if (slab_bucket_replay) {
        run_slab_bucket_replays(dw,di,dout,weights,inputs,reference,output,
            selected_indices,rows,tokens,k);
        return;
    }
    ...
    if (narrow_half_replay) {
        run_narrow_half_replays(dw,di,dout,weights,inputs,reference,output,
            selected_indices,rows,tokens,k);
        return;
    }
