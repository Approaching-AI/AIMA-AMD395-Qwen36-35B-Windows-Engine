...
#include "../../native/providers/ck_fmha/compact_integer_qk.h"
#include "../../native/providers/ck_fmha/deferred_qk_fallback.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
using Row = qrt_compact_integer_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_compact_integer_qk::prepare_rows<Kind>),
...
struct IntegerRows {
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            const Row value=qrt_compact_integer_qk::prepare(original);
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    const dim3 grid((stride+31u)/32u,kQueryHeads,(count+31u)/32u);
    if(!variant) {
        hipLaunchKernelGGL((qrt_microtile_exact_qk::scores<2u,2u>),grid,dim3(kThreads),0u,nullptr,
        ...
        // Every candidate clock includes this slab's complete query encoding.
        encode_rows<IntegerRowKind::Query>(q,matrix.query(),key_stride,start,count);
        if(variant==1u) hipLaunchKernelGGL((qrt_compact_integer_qk::scores<64u>),grid,dim3(kThreads),0u,nullptr,
            matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==2u) hipLaunchKernelGGL((qrt_compact_integer_qk::scores<128u>),grid,dim3(kThreads),0u,nullptr,
            matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid compact four-score QK variant");
    }
    check(hipGetLastError());
    if(!variant) {
        hipLaunchKernelGGL(qrt_deferred_qk_fallback::replay_scan,
            dim3((size_t(count)*kQueryHeads*stride+255u)/256u),dim3(kThreads),0u,nullptr,
...
__global__ void paired_group_values(const Row* left,const Row* right,const uint32_t* carry,
    ...
    unsigned path=0u;
    const auto value=qrt_sm121_compact_integer_dot4::accumulate(input,left[i],right[i],&path);
...
void paired_preflight() {
    ...
    for(unsigned group=0u;group<count;++group) {
        ...
        left[guard+group]=qrt_sm121_compact_integer_dot4::prepare(a);right[guard+group]=qrt_sm121_compact_integer_dot4::prepare(b);
    ...
    upload(a,left);upload(b,right);upload(c,carry);
    for(unsigned variant:{1u}) {
        check(hipMemset(out.pointer,0xa5,expected.size()*4u));check(hipMemset(paths.pointer,0,16u));
        ...
        if(counts[0]+counts[1]+counts[2]+counts[3]!=count || counts[0]<65536u || !counts[1] || !counts[2] || counts[3])
            throw std::runtime_error("compact integer preflight lacks arithmetic path coverage");
        std::printf("{\"kind\":\"compact_integer_preflight\",\"variant\":%u,\"all_bf16_encodings\":65536,\"controls\":16,\"original_products\":2097152,\"raw_carry_states\":131072,\"general_encoding_groups\":65536,\"scaled_normal_groups\":65536,\"native_path_fallback_exact_remainder_unused\":[%u,%u,%u,%u],\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",variant,counts[0],counts[1],counts[2],counts[3]);
        std::fflush(stdout);
    }
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("compact integer QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"compact_integer_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    constexpr unsigned source_tokens=7169u,batch=128u,attempts=4u;
    ...
    if(tokens != source_tokens) {
        if(tokens != 8192u) throw std::runtime_error("invalid captured shape");
    ...
    uint64_t supported[2]{},row_counts[2]{};
    for(unsigned operand=0u;operand<2u;++operand) {
        const auto& words=operand?k:q;
        for(size_t offset=guard;offset+16u<=words.size()-guard;offset+=16u) {
            const auto row=qrt_sm121_compact_integer_dot4::prepare(words.data()+offset);
            supported[operand]+=qrt_sm121_compact_integer_dot4::unit(row)!=0u;
            ++row_counts[operand];
        }
    }
    // Diagnostic CPU sample, outside all candidate clocks. Original carries
    // and products remain an independent check on every sampled K16 state.
    unsigned sampled_paths[3]{};
    for(unsigned sample=0u;sample<4096u;++sample) {
        const unsigned query=(sample*797u)%tokens, key=(sample*149u)%(query+1u), head=sample%16u;
        const uint16_t* left=q.data()+guard+(size_t(query)*16u+head)*256u;
        const uint16_t* right=k.data()+guard+(size_t(key)*2u+head/8u)*256u;
        ...
        for(unsigned group=0u;group<16u;++group) {
            ...
            for(unsigned i=0u;i<16u;++i)values[i+1u]=qrt_q1_moe_hawkeye::multiply_bf16(left[group*16u+i],right[group*16u+i],-133);
            const auto expected=qrt_q1_moe_hawkeye::group_sum<26,-133>(values,17u);
            unsigned path=99u;
            carry=qrt_sm121_compact_integer_dot4::accumulate(carry,
                qrt_sm121_compact_integer_dot4::prepare(left+group*16u),
                qrt_sm121_compact_integer_dot4::prepare(right+group*16u),&path);
            if(path>2u || carry.significand!=expected.significand || carry.exponent!=expected.exponent || carry.negative!=expected.negative)
                throw std::runtime_error("captured compact integer raw carry differs");
            ++sampled_paths[path];
        }
    }
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"compact_integer_qk_capture\",\"variant\":%u,\"query_rows\":32,\"key_columns\":%u,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"supported_query_groups\":%llu,\"all_query_groups\":%llu,\"supported_key_groups\":%llu,\"all_key_groups\":%llu,\"sampled_host_paths_fallback_exact_remainder\":[%u,%u,%u],\"sampled_host_raw_carries\":65536,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,32u,tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
            preparation_ms,sorted[1],sorted[1]+preparation_ms,samples[mode][0],samples[mode][1],samples[mode][2],maximum_stage_ms[mode],
            (unsigned long long)supported[0],(unsigned long long)row_counts[0],(unsigned long long)supported[1],(unsigned long long)row_counts[1],
            sampled_paths[0],sampled_paths[1],sampled_paths[2]);
        std::fflush(stdout);
    }
}
...
int main(int argc,char** argv) {
    try {
        ...
        if(argc==4 && !std::strcmp(argv[1],"--q7169")){captured(argv[2],argv[3],7169u);return 0;}
        if(argc==4 && !std::strcmp(argv[1],"--q8192")){captured(argv[2],argv[3],8192u);return 0;}
        if(argc!=2 || std::strcmp(argv[1],"--selftest"))throw std::runtime_error("use --selftest or --q7169/--q8192 Q K");
        paired_preflight();
