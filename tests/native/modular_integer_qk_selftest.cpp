...
#include "../../native/providers/ck_fmha/modular_integer_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
using Row = qrt_modular_integer_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_modular_integer_qk::prepare_rows<Kind>),
...
struct MatrixRows {
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            qrt_modular_integer_qk::prepare(value);
...
#define MODULAR_QK_CASE(v) if(variant==v) hipLaunchKernelGGL((qrt_modular_integer_qk::scores<v>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        MODULAR_QK_CASE(16u);
        else MODULAR_QK_CASE(32u);
        else MODULAR_QK_CASE(64u);
        else throw std::runtime_error("invalid modular QK variant");
#undef MODULAR_QK_CASE
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"modular_integer_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
#include "modular_integer_core_native_probe.h"
int main(int argc,char** argv) {
    try {
        ...
        if(argc==2 && !std::strcmp(argv[1],"--core")){modular_probe::run();return 0;}
        ...
        if(argc!=2 || std::strcmp(argv[1],"--selftest"))throw std::runtime_error("use --core, --selftest, --q7169 Q K or --q8192 Q K");
