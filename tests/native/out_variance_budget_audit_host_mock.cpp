...
int hipMemcpy(void* dst,const void* src,size_t size,int kind){assert(!pending&&kind==hipMemcpyDeviceToHost);range(src,size);if(fail())return error;
 if(size==8192u*20u*4u){auto* p=static_cast<unsigned*>(dst);std::fill_n(p,8192u*20u,0u);for(unsigned t=0;t<8192u;++t){auto* r=p+t*20u;r[0]=2;r[1]=1;r[3]=1;r[9]=1;r[13]=7;r[14]=1;r[16]=1;r[17]=1;}}
...
#include "out_variance_budget_actual_owner.h"
...
void reset(unsigned failed=0){assert(allocations.empty()&&!pending);operation=allocations_seen=frees=launches=matrices=drains=0;fail_operation=failed;
 setting("QRT_QWEN36_Q8192_OUT_VARIANCE_BUDGET_AUDIT","1");setting("QRT_QWEN36_Q8192_MATRIX_PRODUCER_ALGORITHM","4");setting("QRT_QWEN36_Q8192_MATRIX_PRODUCER_SCOPE","qkv");setting("QRT_QWEN36_Q8192_OUT_L1_BOUND","0");}
int run(unsigned tokens=8192u,bool compatible=true,bool null_source=false){const auto* bf=reinterpret_cast<const uint16_t*>(1);const auto* fp=reinterpret_cast<const float*>(1);const auto* table=reinterpret_cast<const uint8_t*>(1);
 return qrt_out_variance_budget_audit::run(null_source?nullptr:bf,bf,bf,fp,bf,table,tokens,512u,1000u,nullptr,"host-audit",compatible);}
int main(){
 ...
 for(const char* off:{static_cast<const char*>(nullptr),"","0"}){reset();setting("QRT_QWEN36_Q8192_OUT_VARIANCE_BUDGET_AUDIT",off);assert(run(8192u,false,true)==hipSuccess&&!operation);}
 for(const char* bad:{"2"," 1","1x"}){reset();setting("QRT_QWEN36_Q8192_OUT_VARIANCE_BUDGET_AUDIT",bad);assert(run()==hipErrorInvalidValue&&!operation);}
 ...
 std::printf("out_variance_budget_host_pass intervals=%u enumerated_bf16_values=%u injected_failures=%u residual_filter_certified=%u\n",intervals,enumerated,operations,residual_certified);
}
