...
using Row = qrt_byte_residue32_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_byte_residue32_qk::prepare_rows<Kind>),
...
struct MatrixRows {
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            qrt_byte_residue32_qk::prepare(value);
...
#define RESIDUE32_QK_CASE(v) if(variant==v) hipLaunchKernelGGL((qrt_byte_residue32_qk::scores<v>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        ...
        else RESIDUE32_QK_CASE(32u);
        else RESIDUE32_QK_CASE(64u);
        else throw std::runtime_error("invalid byte-residue32 QK variant");
#undef RESIDUE32_QK_CASE
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"byte_residue32_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
