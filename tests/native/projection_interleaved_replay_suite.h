...
__global__ void interleaved_projection_replay_kernel(const uint16_t* weights, const uint16_t* inputs,
    ...
    constexpr unsigned outputs = Variant ? Variant : 1u;
    const unsigned first = ((blockIdx.x * blockDim.x + threadIdx.x) / 4u) * outputs;
    if (first >= count) return;
    const unsigned active = count - first < outputs ? count - first : outputs;
    const uint16_t* left[outputs]; const uint16_t* right[outputs];
    unsigned cells[outputs]; bool eligible[outputs]; float values[outputs];
#pragma unroll
    for (unsigned slot = 0u; slot < outputs; ++slot) if (slot < active) {
        cells[slot] = indices[first + slot];
        const unsigned row = cells[slot] % rows, token = cells[slot] / rows;
        left[slot] = inputs + size_t(token) * width;
        right[slot] = weights + size_t(row) * width;
        eligible[slot] = ((weight_flags[row] & input_flags[token]) & 1u) != 0u;
    }
    if constexpr (Variant == 0u)
        values[0] = qrt_sm121_scalar_projection::validated_dot<4u>(left[0], right[0], width, eligible[0]);
    else
        qrt_sm121_interleaved_projection::dot<outputs>(left, right, eligible, active, width, values);
    if (!(threadIdx.x & 3u)) {
#pragma unroll
        for (unsigned slot = 0u; slot < outputs; ++slot) if (slot < active) {
            unrounded[first + slot] = values[slot];
            output[cells[slot]] = device_bf16_round_to_float(values[slot]);
        }
    }
}

void run_interleaved_projection_replays(DeviceBuffer<uint16_t>& dw, DeviceBuffer<uint16_t>& di,
    ...
    for (unsigned variant : {0u, 2u, 4u}) {
        double ms = 0.0; std::array<double,3> times{};
        for (unsigned attempt = 0u; attempt < 4u; ++attempt) {
            ...
            hipLaunchKernelGGL(classify_strong_projection_rows, dim3(rows), dim3(256u), 0u, nullptr,
                dw.data(), dfw.data(), rows, width);
            hip_ok(hipGetLastError(), "range_weight_eligibility");
            hipLaunchKernelGGL(classify_strong_projection_rows, dim3(tokens), dim3(256u), 0u, nullptr,
                di.data(), dfi.data(), tokens, width);
            hip_ok(hipGetLastError(), "scalar_input_preparation");
            const unsigned outputs = variant ? variant : 1u;
            const dim3 grid((unsigned(selected.size()) + outputs * 64u - 1u) / (outputs * 64u));
#define QRT_SCALAR_REPLAY_CASE(v) if (variant == v) hipLaunchKernelGGL(interleaved_projection_replay_kernel<v>, grid, dim3(256u), 0u, nullptr, dw.data(), di.data(), dfw.data(), dfi.data(), indices.data(), dout.data(), raw.data(), rows, width, unsigned(selected.size()))
            QRT_SCALAR_REPLAY_CASE(0u); QRT_SCALAR_REPLAY_CASE(2u); QRT_SCALAR_REPLAY_CASE(4u);
#undef QRT_SCALAR_REPLAY_CASE
            hip_ok(hipGetLastError(), "scalar_replay_launch"); complete_strong_projection();
            if (attempt) times[attempt-1u] = std::chrono::duration<double, std::milli>(std::chrono::steady_clock::now() - start).count();
        }
        auto ordered=times;std::sort(ordered.begin(),ordered.end());ms=ordered[1];
        ...
        const auto check_flags = [&](const std::vector<uint16_t>& source, const std::vector<unsigned>& flags, unsigned count, unsigned& certified) {
            ...
            for (unsigned row = 0u; row < count; ++row) {
                bool valid = true; bool bounded = width > 0u && width <= 16384u && width % 16u == 0u; bool strong=width>0u && width<=4096u && width%16u==0u;
                for (unsigned k = 0u; k < width; ++k) {
                    ...
                    strong &= !(x & 0x7fffu) || (e >= 84u && e <= 174u);
                }
                require(flags[kGuard + row] == (unsigned(valid) | (unsigned(bounded) << 1u) | (unsigned(strong) << 2u)), "real row eligibility mismatch"); eligible += valid; certified += strong;
            }
            return eligible;
        };
        ...
        size_t certified_candidates = 0u, range_candidates=0u;
        for (unsigned cell : selected) { const unsigned f=wf[kGuard+cell%rows]&inf[kGuard+cell/rows];certified_candidates+=(f&4u)!=0u;range_candidates+=(f&2u)!=0u; }
        ...
        std::cout << "{\"type\":\"interleaved_projection_real_replay\",\"variant\":" << variant
            << ",\"lanes\":4,\"outputs_per_subgroup\":" << (variant ? variant : 1u)
            ...
            << ",\"range_certified_candidates\":" << range_candidates
            ...
            << ",\"warmup_sequences\":1,\"timed_sequences\":3,\"sorted_common_candidate_order\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}" << std::endl;
