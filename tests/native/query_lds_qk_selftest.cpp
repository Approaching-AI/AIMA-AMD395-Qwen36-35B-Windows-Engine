...
#include "../../native/providers/ck_fmha/query_lds_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
#define QRT_LAYOUT_ARGS qp,kp,qf,kf,out,start,count,stride,key_stride
        ...
        else if(variant==2u)hipLaunchKernelGGL((qrt_query_lds_qk::scores<1u>),grid,dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS);
        else if(variant==3u)hipLaunchKernelGGL((qrt_query_lds_qk::scores<2u>),grid,dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS);
        else if(variant==4u)hipLaunchKernelGGL((qrt_query_lds_qk::scores<3u>),grid,dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS);
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("query LDS QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"query_lds_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
