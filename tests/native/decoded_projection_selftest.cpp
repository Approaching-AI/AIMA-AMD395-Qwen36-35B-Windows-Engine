...
template<bool PackedWeights, bool Trace>
__global__ void execute(const uint16_t* left, const uint16_t* right, const unsigned* flags, const uint32_t* packed_left, const uint32_t* packed_right,
    Result* output, uint32_t* traces, unsigned rows, unsigned width) {
    const unsigned row = (blockIdx.x * blockDim.x + threadIdx.x) / 4u;
    if (row >= rows) return;
    qrt_sm121_decoded_projection::Stats stats;
    const float value = qrt_sm121_decoded_projection::dot<PackedWeights, Trace>(left + size_t(row) * width,
        right + size_t(row) * width, packed_left + size_t(row) * width,
        PackedWeights ? packed_right + size_t(row) * width : nullptr, width, flags[row] != 0u,
        Trace ? traces + size_t(row) * (width / 16u) : nullptr, Trace ? &stats : nullptr);
    if (!(threadIdx.x & 3u)) output[row] = {value, stats.floating, stats.integer};
}
...
template<bool PackedWeights>
void variant(unsigned rows, unsigned width, Device& left, Device& right,
    Device& output, Device& traces, Device& flags, Device& packed_left, Device& packed_right, const std::vector<uint16_t>& a, const std::vector<uint16_t>& b,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(execute<PackedWeights,true>), dim3((rows * 4u + 255u) / 256u),
        dim3(256u), 0u, nullptr, left.data<uint16_t>(), right.data<uint16_t>(), flags.data<unsigned>(), packed_left.data<uint32_t>(), packed_right.data<uint32_t>(), output.data<Result>(),
    ...
    check(hipMemset(output.value, 0xa5, (rows + 2u * guard) * sizeof(Result)));
    hipLaunchKernelGGL(HIP_KERNEL_NAME(execute<PackedWeights,false>), dim3((rows * 4u + 255u) / 256u),
        dim3(256u), 0u, nullptr, left.data<uint16_t>(), right.data<uint16_t>(), flags.data<unsigned>(), packed_left.data<uint32_t>(), packed_right.data<uint32_t>(), output.data<Result>(),
        traces.data<uint32_t>(), rows, width);
    check(hipGetLastError()); check(hipDeviceSynchronize());
    ...
    const auto pa=read<uint32_t>(packed_left,size_t(rows)*width),pb=read<uint32_t>(packed_right,size_t(rows)*width);
    guards(pa);guards(pb);
    for(size_t i=0;i<a.size();++i) {
        const auto pack=[](uint16_t x) { const int e=(x&0x7fffu)?int((x>>7u)&255u)-127:-512;return (uint32_t(x)<<16u)|uint16_t(e); };
        if(pa[guard+i]!=pack(a[i]) || pb[guard+i]!=pack(b[i])) throw std::runtime_error("packed operand differs from CPU or changed during replay");
    }
    ...
    std::printf("{\"kind\":\"decoded_projection_safety\",\"lanes\":4,\"packed_weights\":%u,\"rows\":%u,\"width\":%u,\"ordered_groups\":%zu,\"raw_bit_mismatches\":%u,\"floating_groups\":%u,\"integer_groups\":%u,\"unaligned_operands\":true,\"production_diagnostic_parity\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",
        unsigned(PackedWeights), rows, width, groups, mismatches, floating, integer);
    std::fflush(stdout);
    if (mismatches || !floating || !integer || floating + integer != groups)
        throw std::runtime_error("decoded projection arithmetic or coverage failure");
}
void run(unsigned rows, unsigned width) {
    ...
    for (unsigned row = 0u; row < rows; ++row) {
        for (unsigned group = 0u; group < width / 16u; ++group) for (unsigned i = 0u; i < 16u; ++i) {
            const auto pair = cases::input(row, group, i);
    ...
    Device packed_left((words+2u*guard)*4u),packed_right((words+2u*guard)*4u),left_flags((rows+2u*guard)*4u),right_flags((rows+2u*guard)*4u);
    ...
    check(hipGetLastError());check(hipDeviceSynchronize());
    hipLaunchKernelGGL(qrt_sm121_decoded_projection::prepare_rows,dim3(rows+1u),dim3(256u),0u,nullptr,
        left.data<uint16_t>(),packed_left.data<uint32_t>(),left_flags.data<unsigned>(),rows,width);
    check(hipGetLastError());
    hipLaunchKernelGGL(qrt_sm121_decoded_projection::prepare_rows,dim3(rows+1u),dim3(256u),0u,nullptr,
        right.data<uint16_t>(),packed_right.data<uint32_t>(),right_flags.data<unsigned>(),rows,width);
    check(hipGetLastError());check(hipDeviceSynchronize());
    const auto lf=read<unsigned>(left_flags,rows),rf=read<unsigned>(right_flags,rows);guards(lf);guards(rf);
    for(unsigned row=0;row<rows;++row) {
        unsigned av=1u,bv=1u;
        for(unsigned k=0;k<width;++k) { av &= cases::alignment::eligible(a[size_t(row)*width+k]);bv &= cases::alignment::eligible(b[size_t(row)*width+k]); }
        if(lf[guard+row]!=av || rf[guard+row]!=bv || (av&&bv)!=bool(expected_flags[row])) throw std::runtime_error("decoded row flags differ");
    }
    variant<false>(rows,width,left,right,output,traces,flags,packed_left,packed_right,a,b,expected,expected_flags,expected_floating);
    variant<true>(rows,width,left,right,output,traces,flags,packed_left,packed_right,a,b,expected,expected_flags,expected_floating);
    if(read<unsigned>(left_flags,rows)!=lf || read<unsigned>(right_flags,rows)!=rf) throw std::runtime_error("decoded flags changed during replay");
}
int main() try {
    ...
    run(257u,16u); run(4096u, 272u); run(2048u, 2048u); run(1024u, 4096u); run(129u,4112u); run(129u,8192u);
    return 0;
} catch (const std::exception& error) { std::fprintf(stderr, "decoded_projection_error=%s\n", error.what()); return 1; }
