...
#include "../../native/providers/ck_fmha/compact_streamed_attention.h"
...
void compact(const uint16_t* q,const uint16_t* kt,const uint16_t* v,const uint16_t* vt,
    Prepared& prepared,AttentionOutputs& out,unsigned start,unsigned count,unsigned tokens,
    const unsigned char* exp,const unsigned char* packed,const unsigned char* rcp,bool diagnostic){
    const unsigned stride=start+count;
    auto* p=out.tensor.probability.as<uint16_t>()+guard;
    auto* s=out.tensor.scales.as<float>()+guard;
    hipLaunchKernelGGL((qrt_compact_streamed_attention::produce<KeyWindow>),
        dim3(16u,(count+15u)/16u),dim3(256u),0u,nullptr,
        ...
        p,s,out.output.as<float>(),out.error.as<float>(),out.accumulator.as<float>(),out.denominator.as<float>(),
        diagnostic?out.tensor.scores.as<float>()+guard:nullptr,start,count,stride,tokens,exp,packed,rcp);
    check(hipGetLastError());
    check(hipError_t(launch_compacted_pv_replay(v,p,s,out.output.as<float>(),start,count,0u,stride,
        rcp,out.accumulator.as<float>(),out.denominator.as<float>(),out.error.as<float>(),
        out.indices.as<unsigned>(),out.count.as<unsigned>(),nullptr,nullptr,vt,tokens,0u,nullptr,true)));
...
void run_safety(const unsigned char* exp,const unsigned char* packed,const unsigned char* rcp){
    ...
    for(const auto shape:shapes)for(unsigned mode=0u;mode<5u;++mode){
        ...
        actual.reset();streamed(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
            prepared,actual,start,count,n,exp,packed,rcp,false);finish();compare(expected,actual,bad);++cases;
        for(unsigned variant:{1u,2u})for(bool diagnostic:{true,false}){
            actual.reset();
            if(variant==1u)compact<64u>(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
                prepared,actual,start,count,n,exp,packed,rcp,diagnostic);
            else compact<32u>(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
                prepared,actual,start,count,n,exp,packed,rcp,diagnostic);
            finish();compare_stream(expected,actual,bad,diagnostic);++cases;
        }
        ...
        std::fprintf(stderr,"COMPACT_STREAMED_SAFETY tokens=%u start=%u queries=%u mode=%u pass=1\n",n,start,count,mode);
    }
    std::printf("{\"kind\":\"compact_streamed_attention_safety\",\"cases\":%u,\"shapes\":8,\"data_modes\":5,\"candidate_diagnostic_and_score_free\":true,\"raw_bit_mismatches\":0,\"guards_pass\":true,\"immutable_inputs\":true}\n",cases);
}
...
void run_capture(unsigned tokens,const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    double samples[3][3]{};uint64_t candidates[3]{},score_cells=0u;unsigned cpu_dots=0u;
    for(unsigned start=0;start<tokens;start+=query_batch){
        ...
        for(unsigned attempt=0;attempt<4u;++attempt)for(unsigned position=0;position<3u;++position){
            ...
            if(!variant)streamed(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,
                actual,start,count,tokens,exp,packed,rcp,false);
            else if(variant==1u)compact<64u>(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,
                actual,start,count,tokens,exp,packed,rcp,!attempt);
            else compact<32u>(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,
                actual,start,count,tokens,exp,packed,rcp,!attempt);
    ...
    if(candidates[0]!=candidates[1] || candidates[0]!=candidates[2])throw std::runtime_error("original PV selection differs");
    for(unsigned variant=0;variant<3u;++variant){
        ...
        std::printf("{\"kind\":\"compact_streamed_attention_capture\",\"tokens\":%u,\"source_capture_tokens\":7169,\"repeated_rows\":%u,\"variant\":%u,\"score_free_timed\":%s,\"resident_queries\":%u,\"key_window\":%u,\"query_batch\":128,\"score_slots\":%llu,\"output_cells\":%u,\"gb10_context_cells\":29364224,\"cpu_dots\":%u,\"pv_candidates\":%llu,\"completed_attention_samples_ms\":[%.9f,%.9f,%.9f],\"median_completed_attention_ms\":%.9f,\"common_preparation_ms\":%.9f,\"raw_bit_mismatches\":0,\"gb10_context_mismatches\":0,\"all_attempts_checked\":true,\"warmups_per_slab\":1,\"timed_attempts_per_slab\":3,\"original_qk_and_pv\":true,\"reference_is_compute_input\":false,\"redzones_and_unused_tails_pass\":true,\"immutable_inputs\":true,\"model_loaded\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",tokens,tokens-7169u,variant,variant?"true":"false",variant?16u:32u,variant==1u?64u:variant==2u?32u:0u,(unsigned long long)score_cells,tokens*4096u,cpu_dots,(unsigned long long)candidates[variant],samples[variant][0],samples[variant][1],samples[variant][2],sorted[1],prepared.ms+transpose_ms);
    }
}
} // namespace
...
}catch(const std::exception& e){std::fprintf(stderr,"compact_streamed_attention_error=%s\n",e.what());return 2;}
