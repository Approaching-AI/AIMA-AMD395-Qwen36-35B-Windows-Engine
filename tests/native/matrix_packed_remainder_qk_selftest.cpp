...
#include "../../native/providers/ck_fmha/matrix_packed_remainder_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
constexpr unsigned variants[] = {0u, 32u, 132u, 232u, 332u, 432u};
...
using Row = qrt_matrix_packed_remainder_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_matrix_packed_remainder_qk::prepare_rows<Kind>),
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            for(unsigned c=0u;c<16u;++c)
                value.matrix.core.original[c]=input[integer_row_input_index(kind,row,c,tokens,start,count)];
            qrt_matrix_packed_remainder_qk::prepare(value);
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        const dim3 grid((stride+31u)/32u,kQueryHeads,(count+15u)/16u);
#define MATRIX_REMAINDER_CASE(v,p) if(variant==v) hipLaunchKernelGGL((qrt_matrix_packed_remainder_qk::scores<32u,p>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        MATRIX_REMAINDER_CASE(32u,0u);
        else MATRIX_REMAINDER_CASE(132u,1u);
        else MATRIX_REMAINDER_CASE(232u,2u);
        else MATRIX_REMAINDER_CASE(332u,3u);
        else MATRIX_REMAINDER_CASE(432u,4u);
        else throw std::runtime_error("invalid matrix packed remainder QK variant");
#undef MATRIX_REMAINDER_CASE
    }
    check(hipGetLastError());
}
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("matrix packed remainder QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"matrix_packed_remainder_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
