...
#include "../../native/providers/ck_fmha/wave_qk_probe.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
static_assert(kQueryHeads==qrt_wave_qk_probe::query_heads && kKvHeads==qrt_wave_qk_probe::kv_heads);
static_assert(kHeadDim==qrt_wave_qk_probe::head_dim && kThreads==qrt_wave_qk_probe::threads && kExactScale==qrt_wave_qk_probe::scale);
...
void verify_wave_execution() {
    Device d((2u+2u*guard)*4u);
    for(unsigned width:{32u,64u}) {
        check(hipMemset(d.pointer,0xa5,(2u+2u*guard)*4u));
        check((width==32u?qrt_wave_qk_probe_32:qrt_wave_qk_probe_64)(d.as<unsigned>()+guard));
        finish();const auto actual=download<unsigned>(d,2u+2u*guard);
        if(actual[guard]!=width || actual[guard+1u]!=width)throw std::runtime_error("wave width or ballot differs");
        for(size_t i=0u;i<actual.size();++i)if((i<guard || i>=guard+2u) && actual[i]!=0xa5a5a5a5u)throw std::runtime_error("wave probe guard differs");
    }
}
...
#define QRT_LAYOUT_ARGS qp,kp,qf,kf,out,start,count,stride,key_stride
        ...
        else if(variant==2u)check(qrt_wave_qk_32(QRT_LAYOUT_ARGS));
        else if(variant==3u)check(qrt_wave_qk_64(QRT_LAYOUT_ARGS));
        else throw std::runtime_error("invalid query LDS layout variant");
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("wave QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"wave_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"device_wave32_and_wave64_ballots_verified\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"wave_qk_capture\",\"variant\":%u,\"query_rows\":16,\"key_columns\":16,\"verified_flagged_tiles_after_last_attempt\":%zu,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_replay_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"tile_flag_guards_and_tail_pass\":true,\"complete_replay_timing_included\":true,\"device_wave32_and_wave64_ballots_verified\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
