...
#define main qrt_deferred_prior_gdn_main
#include "tiled_scalar_gdn_selftest.cpp"
#undef main
#include "../../native/providers/gdn/deferred_state_matrices.h"
#include <array>
...
void deferred_launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,
    ...
    require(variant<=2u,"invalid deferred variant");
...
#define QRT_DEFERRED_RESIDUAL_ARGS actual_u+size_t(offset)*4096u,w.data<uint16_t>()+size_t(offset)*4096u,hw,gates,updated,archive,n,step,exp
            if(variant==1u){hipLaunchKernelGGL((deferred::residuals<false>),dim3(8u,32u),dim3(256u),0u,nullptr,QRT_DEFERRED_RESIDUAL_ARGS);}
            else{hipLaunchKernelGGL((deferred::residuals<true>),dim3(8u,32u),dim3(256u),0u,nullptr,QRT_DEFERRED_RESIDUAL_ARGS);}
#undef QRT_DEFERRED_RESIDUAL_ARGS
            check(hipGetLastError());
            hipLaunchKernelGGL(deferred::update,dim3(4u,32u,2u),dim3(256u),0u,nullptr,
                keys,archive,n,step,workspace.lower.data<float>(),workspace.upper.data<float>(),
                workspace.flags.data<unsigned char>(),workspace.coefficients.data<float>(),audit_lo,audit_hi);check(hipGetLastError());
        }
        const unsigned step=steps;
        hipLaunchKernelGGL((deferred::checkpoint<true>),dim3(state_cells/256u),dim3(256u),0u,nullptr,
            QRT_DEFERRED_CHECKPOINT_ARGS);check(hipGetLastError());
#undef QRT_DEFERRED_CHECKPOINT_ARGS
        hipLaunchKernelGGL(scalar::output_kernel,dim3(16u,32u,steps),dim3(256u),0u,nullptr,
...
void deferred_run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table){
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<6u;++position){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"deferred-state GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"deferred-state GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"deferred-state GDN intermediate or alias differs");
  if(variant){
