...
#include "../../native/providers/ck_fmha/exponent_mask_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
struct Prepared {
    ...
    Prepared(const uint16_t* q, const uint16_t* k, uint16_t* transposed,
        const uint16_t* hq, const uint16_t* hk, unsigned n, bool use_masks)
        : tokens(n), masked(use_masks), qpacked(size_t(n)*kQueryHeads*kHeadDim+2u*guard,0xa5a5a5a5u),
        ...
        if(masked) {
            hipLaunchKernelGGL((qrt_exponent_mask_qk::prepare<false>),dim3(n*kQueryHeads),dim3(kHeadDim),0u,nullptr,
                q,qp.as<uint32_t>()+guard,qf.as<unsigned>()+guard,nullptr,n);
            check(hipGetLastError());
            hipLaunchKernelGGL((qrt_exponent_mask_qk::prepare<true>),dim3(n*kKvHeads),dim3(kHeadDim),0u,nullptr,
        ...
        for(unsigned key=0u;key<2u;++key) {
            ...
            for(unsigned row=0u;row<n*heads;++row) {
                ...
                for(unsigned c=0u;c<kHeadDim;++c) {
                    ...
                    uint16_t low=uint16_t(exponent);
                    if(masked && c%16u<4u) {
                        int maximum=-512;uint16_t masks[3]{};
                        const auto* block=source+size_t(row)*kHeadDim+(c/16u)*16u;
                        for(unsigned j=0u;j<16u;++j) {
                            const int e=(block[j]&0x7fffu)?int((block[j]>>7u)&255u)-127:-512;
                            maximum=std::max(maximum,e);
                        }
                        for(unsigned j=0u;j<16u;++j) {
                            const int e=(block[j]&0x7fffu)?int((block[j]>>7u)&255u)-127:-512;
                            const int rank=maximum-e;
                            if((block[j]&0x7fffu) && rank>=0 && rank<3)masks[rank]|=uint16_t(1u<<j);
                        }
                        low=c%16u==0u?uint16_t(maximum):masks[c%16u-1u];
                    }
                    packed[guard+index]=(uint32_t(x)<<16u)|low;
                }
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    unsigned start,unsigned count,unsigned stride,unsigned key_stride,Prepared& original,Prepared& masked) {
    auto& p=variant?masked:original;
    ...
    if(!variant)
        hipLaunchKernelGGL((qrt_prepared_decoded_qk::scores<128u,true,16u,16u>),grid,dim3(kThreads),0u,nullptr,
            q,k,p.qp.as<uint32_t>()+guard,p.kp.as<uint32_t>()+guard,p.qf.as<unsigned>()+guard,p.kf.as<unsigned>()+guard,out,start,count,stride,key_stride);
    else if(variant==1u)
        hipLaunchKernelGGL((qrt_exponent_mask_qk::scores<128u,false>),grid,dim3(kThreads),0u,nullptr,
            q,k,p.qp.as<uint32_t>()+guard,p.kp.as<uint32_t>()+guard,p.qf.as<unsigned>()+guard,p.kf.as<unsigned>()+guard,out,start,count,stride,key_stride);
    else if(variant==2u)
        hipLaunchKernelGGL((qrt_exponent_mask_qk::scores<128u,true>),grid,dim3(kThreads),0u,nullptr,
            q,k,p.qp.as<uint32_t>()+guard,p.kp.as<uint32_t>()+guard,p.qf.as<unsigned>()+guard,p.kf.as<unsigned>()+guard,out,start,count,stride,key_stride);
    else throw std::runtime_error("invalid exponent mask QK variant");
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    if(c.mode==8u) {
        // Wide eligible exponent spreads exercise maximum certificates and
        // the original accumulator's supported-range checks.
        for(size_t i=guard;i+guard<q.size();++i)
            q[i]=uint16_t(((64u+8u*(i%16u))<<7u)|37u|((i&1u)?0x8000u:0u));
        for(size_t i=guard;i+guard<k.size();++i)
            k[i]=uint16_t(((64u+8u*(i%16u))<<7u)|53u);
    }
    if(c.mode==9u) {
        // A nonzero subnormal in every K16 forces the original integer
        // fallback, including the original carry beyond the fast domain.
        for(size_t i=guard;i+guard<q.size();++i)
            q[i]=(i%16u)?uint16_t((126u<<7u)|37u):uint16_t(1u);
        for(size_t i=guard;i+guard<k.size();++i)
            k[i]=(i%16u)?uint16_t((127u<<7u)|53u):uint16_t(1u);
    }
    ...
    Prepared prepared(dq.as<uint16_t>()+guard,dk.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,
        q.data()+guard,k.data()+guard,c.tokens,false);
    Prepared masked(dq.as<uint16_t>()+guard,dk.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,
        q.data()+guard,k.data()+guard,c.tokens,true);
    ...
    for(unsigned variant:variants) {
        ...
        staged_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,candidate.as<float>()+guard,c.start,c.count,stride,c.tokens,prepared,masked);
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("exponent mask QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"exponent_mask_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
    unchanged(dq,q);unchanged(dk,k);unchanged(dt,transposed);prepared.verify();masked.verify();
}
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    Prepared prepared(dq.as<uint16_t>()+guard,dk.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,
        q.data()+guard,k.data()+guard,tokens,false);
    Prepared masked(dq.as<uint16_t>()+guard,dk.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,
        q.data()+guard,k.data()+guard,tokens,true);
    std::vector<uint16_t> transposed(k.size(),0x5a5au);
        ...
        for(unsigned position=0u;position<variant_count;++position) {
            ...
            for(unsigned attempt=0u;attempt<attempts;++attempt) {
                ...
                staged_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,candidate.as<float>()+guard,
                    start,count,stride,tokens,prepared,masked);
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        const double preparation_ms=variant?masked.ms:prepared.ms;
        std::printf("{\"kind\":\"exponent_mask_qk_capture\",\"variant\":%u,\"query_rows\":16,\"key_columns\":16,\"carry_certificate\":%s,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,variant==2u?"true":"false",tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
