...
void run(Case c) {
    ...
    Device original((cells+2u*guard)*4u), paired((cells+2u*guard)*4u);
    ...
    check(hipMemset(paired.pointer,0xa5,(cells+2u*guard)*4u));
    ...
    hipLaunchKernelGGL(blackwell_paired_query_scores_kernel,
        dim3((stride+31u)/32u,kQueryHeads,(c.count+15u)/16u),dim3(kThreads),0u,nullptr,
        dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,paired.as<float>()+guard,c.start,c.count,stride,c.tokens);
    check(hipGetLastError()); finish();
    const auto a = download<uint32_t>(original,cells+2u*guard);
    const auto b = download<uint32_t>(paired,cells+2u*guard);
    for (size_t i=0u;i<a.size();++i) {
        if (i<guard || i>=cells+guard) {
            if (a[i]!=0xa5a5a5a5u || b[i]!=0xa5a5a5a5u) throw std::runtime_error("QK score redzone changed");
        } else if (a[i]!=b[i]) throw std::runtime_error("Paired-query QK differs from original scores");
    }
    ...
    std::printf("{\"kind\":\"paired_tiled_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",
