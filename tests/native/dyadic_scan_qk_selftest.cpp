...
#include "../../native/providers/ck_fmha/dyadic_scan_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
#define QRT_DYADIC_CASE(V,G) else if(variant==V)hipLaunchKernelGGL((qrt_dyadic_scan_qk::scores<G>),dim3((stride+(64u/G)-1u)/(64u/G),kQueryHeads,(count+3u)/4u),dim3(kThreads),0u,nullptr,qp,kp,qf,kf,out,start,count,stride,key_stride)
        QRT_DYADIC_CASE(3u,4u);
        QRT_DYADIC_CASE(4u,8u);
        QRT_DYADIC_CASE(5u,16u);
        else throw std::runtime_error("invalid dyadic scan QK variant");
#undef QRT_DYADIC_CASE
        check(hipGetLastError());
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("dyadic scan QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"dyadic_scan_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"dyadic_scan_qk_capture\",\"variant\":%u,\"parallel_groups\":%u,\"query_rows\":%u,\"key_columns\":%u,\"unused_flag_tiles\":%zu,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_replay_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"unused_auxiliary_guards_pass\":true,\"complete_fallback_timing_included\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,variant>=3u?(4u<<(variant-3u)):1u,variant>=3u?4u:16u,variant>=3u?(16u>>(variant-3u)):16u,flagged_tiles[mode],tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
