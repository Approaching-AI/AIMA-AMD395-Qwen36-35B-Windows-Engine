#include "sm121_matrix_predicted_group.h"
...
struct Group {
    ...
    qrt_sm121_matrix_predicted_group::Row left{},right{};
...
int main(){
    ...
    size_t prefix_hits=0u,prefix_fallbacks=0u,coefficient_equalities=0u,matrix_plans=0u,replayed_pairs=0u;
    ...
    for(unsigned row=0u;row<8192u;++row){
        ...
        for(unsigned g=0u;g<16u;++g){auto& input=inputs[g];
            for(unsigned i=0u;i<16u;++i){
                ...
                input.left.original[i]=p.left;input.right.original[i]=p.right;
                input.scalar.set(i,p.left,p.right);
                input.approximate+=input.scalar.products[i];
            }
            qrt_sm121_integer_core::prepare(input.left);qrt_sm121_integer_core::prepare(input.right);
            int32_t parts[4]{};
            for(unsigned i=0u;i<16u;++i){
                const int a=qrt_sm121_integer_core::signed_core(input.left.original[i],input.left.unit);
                const int b=qrt_sm121_integer_core::signed_core(input.right.original[i],input.right.unit);
                input.mathematical+=int64_t(a)*b;
                const int ah=(uint16_t(a)>>8u)-(a<0?256:0),bh=(uint16_t(b)>>8u)-(b<0?256:0);
                const int al=uint16_t(a)&255u,bl=uint16_t(b)&255u;
                parts[0]+=ah*bh;parts[1]+=ah*bl;parts[2]+=al*bh;parts[3]+=al*bl;
            }
            assert(input.mathematical==int64_t(parts[0])*65536+(int64_t(parts[1])+parts[2])*256+parts[3]);
        ...
        for(unsigned g=0u;g<16u;++g){auto& input=inputs[g];input.values[0]=carry;
            ...
            for(int exponent:{int(carry.exponent),guessed,int(carry.exponent)+1,int(carry.exponent)-1,int(carry.exponent)+37,-500,512}){
                ...
                if(input.eligible){
                    ...
                    const auto matrix=qrt_sm121_matrix_predicted_group::prepare(input.left,input.right,input.mathematical,exponent,&repaired);
                    if(matrix.modulo!=fp.modulo||matrix.control!=fp.control){
                        std::fprintf(stderr,"matrix_plan_mismatch row=%u group=%u guess=%d mathematical=%lld units=%d,%d maxima=%d,%d nonzero=%x,%x expected=%08x/%08x actual=%08x/%08x\n",
                            row,g,exponent,(long long)input.mathematical,input.left.unit,input.right.unit,input.left.maximum,input.right.maximum,input.left.nonzero,input.right.nonzero,fp.modulo,fp.control,matrix.modulo,matrix.control);
                        std::abort();
                    }
                    ++matrix_plans;replayed_pairs+=repaired;
                    Value matrix_actual=marker;
                    const bool matrix_used=plan::apply(carry,matrix,&matrix_actual);
                    assert(matrix_used==plan::apply(carry,fp,&actual)&&same(matrix_actual,actual));
                    assert(!plan::valid(qrt_sm121_matrix_predicted_group::prepare(input.left,input.right,INT64_MAX,exponent)));
                    assert(!plan::valid(qrt_sm121_matrix_predicted_group::prepare(input.left,input.right,INT64_MIN,exponent)));
    ...
    std::printf("{\"kind\":\"matrix_predicted_k16_alignment_host\",\"matrix_plans\":%zu,\"replayed_pairs\":%zu,\"integer_core_matrix_products\":4,\"hardware_error_bound_required\":false,\"sequences\":8192,\"canonical_groups\":%zu,\"packed_accept\":%zu,\"packed_reject\":%zu,\"float_accept\":%zu,\"float_reject\":%zu,\"identical_float_coefficients\":%zu,\"parallel_prefix_hits\":%zu,\"parallel_prefix_fallbacks\":%zu,\"raw_mismatches\":0,\"rejected_outputs_unchanged\":true,\"plan_bytes\":8,\"inference_acceptance\":false}\n",matrix_plans,replayed_pairs,groups,packed_accept,packed_reject,float_accept,float_reject,coefficient_equalities,prefix_hits,prefix_fallbacks);
}
