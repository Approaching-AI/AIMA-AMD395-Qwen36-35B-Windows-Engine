...
template<unsigned Headroom> void run(){
    ...
    for(unsigned pattern=0u;pattern<65536u;++pattern){
        ...
        const auto before=row;core::prepare(row);
    ...
    for(unsigned low=0u;low<256u;++low)for(int64_t high:{int64_t(-65536),int64_t(-256),int64_t(0),int64_t(256),int64_t(16777216),maximum_dot-512,-maximum_dot}){
        ...
        for(double delta:{-128.0,-127.5,-64.0,-0.5,0.0,0.5,64.0,127.5,128.0}){
            ...
            int32_t result=0x3958192;assert(core::recover(approximate,uint32_t(expected),&result) && result==expected);++recoveries;
        }
    }
    for(float x:{std::numeric_limits<float>::quiet_NaN(),std::numeric_limits<float>::infinity(),-std::numeric_limits<float>::infinity(),float(maximum_dot+1024),-float(maximum_dot+1024),128.0f,-128.0f}){int32_t result=123;assert(!core::recover(x,0u,&result) && result==123);}
    ...
    for(unsigned sample=0u;sample<262144u;++sample){
        ...
        for(unsigned shift=1u;shift<30u;++shift){
            ...
            assert(core::original::remainder_mask(a,b,shift)==expected);++remainders;
        }
        ...
        assert(mathematical>=-core::maximum_dot && mathematical<=core::maximum_dot);
        if(core::sum(terms[0],a,b,int32_t(mathematical),&actual)){
    ...
    std::printf("{\"kind\":\"byte_residue32_core_host\",\"headroom\":%u,\"exact_encodings\":%zu,\"conditional_recovery_checks\":%zu,\"remainder_mask_checks\":%zu,\"canonical_groups\":%zu,\"accepted_groups\":%zu,\"declined_groups\":%zu,\"mismatches\":0,\"row_bytes\":116,\"immutable_inputs\":true,\"hardware_error_bound_proven\":false}\n",Headroom,encodings,recoveries,remainders,groups,accepted,declined);
}
int main(){run<5u>();}
