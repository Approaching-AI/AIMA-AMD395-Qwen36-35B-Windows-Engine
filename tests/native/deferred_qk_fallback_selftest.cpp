...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        if(variant==2u)check(hipMemset(scratch.data(),0,scratch.live(stride,count)*4u));
        ...
        else if(variant==2u)hipLaunchKernelGGL((qrt_deferred_qk_fallback::scores<true>),grid,dim3(kThreads),0u,nullptr,
            qp,kp,qf,kf,out,start,count,stride,key_stride,scratch.data());
        else throw std::runtime_error("invalid deferred QK variant");
        check(hipGetLastError());
        if(variant==1u)hipLaunchKernelGGL(qrt_deferred_qk_fallback::replay_scan,
            dim3((size_t(count)*kQueryHeads*stride+255u)/256u),dim3(256u),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else hipLaunchKernelGGL(qrt_deferred_qk_fallback::replay_tiles,grid,dim3(kThreads),0u,nullptr,
            q,k,out,scratch.data(),start,count,stride,key_stride);
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"deferred_qk_fallback_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
