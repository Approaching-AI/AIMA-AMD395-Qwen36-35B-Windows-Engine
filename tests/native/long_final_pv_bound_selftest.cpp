...
#endif
#include "../../native/providers/moe_accumulator/sm121_pv_long_final_bound.h"

namespace b = qrt_sm121_pv_bound;
namespace f = qrt_sm121_pv_long_final_bound;
struct Result { unsigned checks, underestimated, false_admissions, finite_checks, short_checks; float worst_ratio; };
BOUND_TEST_INLINE unsigned length(unsigned trial){
    constexpr unsigned values[]={2u,4u,512u,514u,1024u,1026u,2048u,4096u,8192u,8194u,16384u,16544u,16546u};
    return values[trial%13u];
}
...
BOUND_TEST_INLINE Result sequence(unsigned trial) {
    ...
    Result result{};
    const unsigned groups = length(trial);
    ...
    for (unsigned g = 0; g < groups; ++g) {
        ...
        if (g % 2u == 1u) {
            ...
            ++result.checks;
            if(g+1u<=512u){++result.short_checks;
                if(b::bits(enlarged)!=b::bits(qrt_sm121_pv_final_bound::finalize(state,g+1u)))++result.underestimated;}
            if (!(enlarged >= original)) ++result.underestimated;
            if (b::finite(enlarged) && original > 0.0f) {
                ++result.finite_checks;const float ratio = original / enlarged;
...
#ifdef QRT_LONG_BOUND_LIBRARY
int qrt_long_final_bound_selftest() {
#else
int main() {
#endif
    constexpr unsigned trials = 4096u;
...
#endif
    unsigned checks = 0, underestimated = 0, false_admissions = 0, finite_checks = 0, short_checks=0, expected_checks=0;
    for(unsigned i=0u;i<trials;++i)expected_checks+=length(i)/2u;
    float worst_ratio = 0.0f;
    for (const auto& r : results) {
        checks += r.checks;finite_checks+=r.finite_checks;short_checks+=r.short_checks; underestimated += r.underestimated; false_admissions += r.false_admissions;
        if (r.worst_ratio > worst_ratio) worst_ratio = r.worst_ratio;
    }
    assert(checks == expected_checks && finite_checks && short_checks && underestimated == 0u && false_admissions == 0u);
    ...
    assert(!b::finite(f::finalize(1.0f, 16548u)));
    ...
    std::printf("{\"kind\":\"long_final_pv_bound_scope\",\"maximum_groups\":16546,\"finite_envelope_checks\":%u,\"short_raw_identity_checks\":%u,\"shapes\":13,\"all_pass\":true}\n",finite_checks,short_checks);
    return 0;
}
