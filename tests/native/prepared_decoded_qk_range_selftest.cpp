#include "../../native/providers/ck_fmha/prepared_decoded_qk_range.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
constexpr unsigned variants[] = {0u, 4u};
...
struct Prepared {
    ...
    Device memory;
    qrt_prepared_decoded_qk_range::Workspace workspace;
    double ms=0.0;
    Prepared(const uint16_t* q,const uint16_t* k,uint16_t* transposed,
        const uint16_t* hq,const uint16_t* hk,unsigned n,unsigned origin=0u,unsigned count=0u)
        : arena(qrt_prepared_decoded_qk_range::workspace_words(n)+2u*guard,0xa5a5a5a5u),
          memory(arena.size()*4u),workspace{memory.as<uint32_t>()+guard,arena.size()-2u*guard,n,origin,count?count:n,n} {
        namespace range=qrt_prepared_decoded_qk_range;
        if(!range::valid(workspace))throw std::runtime_error("invalid range fixture");
        upload(memory,arena);
        const auto begin=std::chrono::steady_clock::now();
        check(hipError_t(range::prepare_workspace(q,k,transposed,workspace,nullptr)));
        finish();ms=elapsed(begin);
        for(unsigned key=0u;key<2u;++key) {
            const unsigned heads=key?kKvHeads:kQueryHeads;
            const unsigned rows=(key?n:workspace.query_count)*heads;
            const auto* source=key?hk:hq+size_t(origin)*kQueryHeads*kHeadDim;
            const size_t values=key?range::query_words:0u;
            ...
            for(unsigned row=0u;row<rows;++row) {
                ...
                for(unsigned c=0u;c<kHeadDim;++c) {
                    const uint16_t x=source[size_t(row)*kHeadDim+c];const unsigned e=(x>>7u)&255u;
                    ...
                    arena[guard+values+index]=(uint32_t(x)<<16u)|uint16_t(exponent);
                }
                arena[guard+flags+row]=unsigned(eligible);
            }
        }
        verify();
    }
    void verify() { unchanged(memory,arena); }
};
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        check(hipError_t(qrt_prepared_decoded_qk_range::launch_workspace(&prepared.workspace,
            q,k,out,nullptr,start,count,stride,key_stride)));
    }
}
...
void run(Case c) {
    ...
    Prepared prepared(dq.as<uint16_t>()+guard,dk.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,
        q.data()+guard,k.data()+guard,c.tokens,c.start,c.count);
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("decoded QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"prepared_decoded_range_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    const auto prepared_keys=download<uint16_t>(dt,k.size());
    ...
    double samples[variant_count][3]{};
    ...
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned mode=0u;mode<variant_count;++mode) {
            for(unsigned attempt=0u;attempt<4u;++attempt) {
                // Independent output reset prevents a no-op launch from passing.
                check(hipMemset(candidate.pointer,0xa5,(capacity+2u*guard)*4u));
                finish();
                begin=std::chrono::steady_clock::now();
                staged_variant(variants[mode],dq.as<uint16_t>(),dt.as<uint16_t>(),candidate.as<float>()+guard,start,count,stride,tokens,prepared);
    ...
    unchanged(dq,q);unchanged(dk,k);unchanged(dt,prepared_keys);prepared.verify();
...
int main(int argc,char** argv) {
    try {
        ...
        const Case cases[]={{17,0,8,0},{33,1,31,1},{67,33,32,5},{129,1,128,0},
            {8192,8064,128,2},{8193,8191,2,3},{16384,8192,65,0},{16384,16383,1,6},
            {65568,65536,32,0},{65568,65536,32,5},{131104,131072,32,0},
            {263200,263168,32,1},{264736,264735,1,2}};
