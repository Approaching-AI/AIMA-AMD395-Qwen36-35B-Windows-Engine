...
template<IntegerRowKind Kind> void prepare(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL(HIP_KERNEL_NAME(qrt_positive_integer_qk::prepare_rows<Kind>),
...
void staged_variant(unsigned variant,const Row* q,const Row* k,float* out,
    ...
    if(variant<2u) {
        ...
        if(variant==2u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<16u>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==3u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<32u>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==4u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<64u>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if(variant==5u)
            hipLaunchKernelGGL((qrt_matrix_consumer_qk::scores<32u,false>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid matrix consumer variant");
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"matrix_consumer_qk_safety\",\"variant\":%u,\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",variant,c.tokens,c.start,c.count,c.mode,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"matrix_consumer_original_q7169\",\"variant\":%u,\"columns\":%u,\"scalar_chains_per_thread\":%u,\"positive_core\":%s,\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"scalar_float_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"query_and_encoding_ms\":%.6f,\"key_encoding_ms\":%.6f,\"maximum_completed_slab_ms\":%.6f,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",variant,columns,columns/16u,(variant!=0u&&variant!=5u)?"true":"false",compared/variant_count,cpu_dots/variant_count,original_ms,transpose_ms,staged_ms[mode],key_encoding_ms,maximum_stage_ms[mode]);
        std::fflush(stdout);
    }
