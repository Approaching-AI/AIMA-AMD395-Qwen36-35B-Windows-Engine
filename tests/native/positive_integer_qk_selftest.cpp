#include "../../native/providers/ck_fmha/positive_integer_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
void staged_variant(unsigned variant, const Row* q, const Row* k, float* out,
    ...
    if (variant == 0u)
        hipLaunchKernelGGL(HIP_KERNEL_NAME(qrt_positive_integer_qk::scores<false>),
        ...
        hipLaunchKernelGGL(HIP_KERNEL_NAME(qrt_positive_integer_qk::scores<true>),
            dim3((stride+kIntegerMatrixColumns-1u)/kIntegerMatrixColumns,kQueryHeads,(count+15u)/16u),
            dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
    else throw std::runtime_error("invalid positive integer variant");
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"positive_integer_qk_safety\",\"positive_core\":%s,\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",variant==1u?"true":"false",c.tokens,c.start,c.count,c.mode,cells);
        std::fflush(stdout);
    }
