...
#include "../../native/providers/ck_fmha/inline_probability_repair.h"

namespace {
namespace joint=qrt_joint_context_qk;
namespace inline_repair=qrt_inline_probability_repair;
struct JointWorkspace {
    size_t cells,rows,scale_cells;
    Guarded score_error,unit_scales,center_denominator,denominator_intervals,pending,certified,statistics;
    explicit JointWorkspace(unsigned tokens):cells(size_t(query_batch)*16u*tokens),rows(query_batch*16u),
        scale_cells(rows*((tokens+31u)/32u+1u)),score_error(cells*4u),unit_scales(scale_cells*4u),
        center_denominator(rows*4u),denominator_intervals(rows*8u),pending(rows*4u),certified(4u),statistics(8u){}
    void reset(){for(auto* p:{&score_error,&unit_scales,&center_denominator,&denominator_intervals,&pending,&certified,&statistics})p->reset();}
    void guards(){for(auto* p:{&score_error,&unit_scales,&center_denominator,&denominator_intervals,&pending,&certified,&statistics})p->guards();}
};

void initial_probability(const uint16_t* q,const uint16_t* kt,const uint16_t* v,const uint16_t* vt,
    ...
    check(hipMemsetAsync(w.statistics.data(),0,8u,nullptr));
    hipLaunchKernelGGL((inline_repair::probabilities<true>),dim3(16u,count),dim3(32u),0u,nullptr,
        q,kt,pq,pk,qflags,kflags,scores,w.score_error.as<float>(),probabilities,scales,w.unit_scales.as<float>(),
        w.center_denominator.as<float>(),w.denominator_intervals.as<float>(),w.pending.as<unsigned>(),
        w.statistics.as<unsigned>(),start,stride,tokens,exp,packed);check(hipGetLastError());
}
...
void joint_attention(const uint16_t* q,const uint16_t* kt,const uint16_t* v,const uint16_t* vt,
    ...
    auto* pq=prepared.qp.as<uint32_t>()+guard;auto* pk=prepared.kp.as<uint32_t>()+guard;
    auto* qflags=prepared.qf.as<unsigned>()+guard;auto* kflags=prepared.kf.as<unsigned>()+guard;
    initial_probability(q,kt,v,vt,prepared,out,w,start,count,tokens,exp,packed,rcp);
    ...
    const auto certify=[&](bool exact_point){
        hipLaunchKernelGGL(inline_repair::certify,dim3(16u,count),dim3(32u),0u,nullptr,
            out.accumulator.as<float>(),out.error.as<float>(),w.denominator_intervals.as<float>(),
            out.output.as<float>(),w.pending.as<unsigned>(),rcp,exact_point,w.certified.as<unsigned>());
    ...
    certify(false);repair_numerators(false);certify(false);
    hipLaunchKernelGGL((qrt_pending_row_qk::scores<2u,2u>),
        dim3((stride+31u)/32u,16u,(count+31u)/32u),dim3(256u),0u,nullptr,
        pq,pk,qflags,kflags,scores,w.score_error.as<float>(),w.pending.as<unsigned>(),start,count,stride,tokens);
    check(hipGetLastError());
    ...
    check(hipGetLastError());
    hipLaunchKernelGGL((inline_repair::probabilities<false>),dim3(16u,count),dim3(32u),0u,nullptr,
        q,kt,pq,pk,qflags,kflags,scores,w.score_error.as<float>(),probabilities,scales,w.unit_scales.as<float>(),
        w.center_denominator.as<float>(),w.denominator_intervals.as<float>(),w.pending.as<unsigned>(),
        w.statistics.as<unsigned>(),start,stride,tokens,exp,packed);check(hipGetLastError());
    certify(true);repair_numerators(true);certify(true);
}

// This observer runs independently before native PV or any full-row fallback.
// It prevents later score/P recomputation from hiding a first-pass repair error.
__global__ void inspect_initial_probability(const float* expected_scores,
    const uint16_t* expected_probability,const float* expected_scales,const float* scores,
    const float* errors,const uint16_t* probability,const float* scales,const float* denominators,
    unsigned start,unsigned count,unsigned stride,unsigned* bad){
    ...
    if(cell<rows*stride){
        ...
        if(key<=start+row/16u){
            const auto interval=qrt_selective_qk::score_interval(scores[cell],errors[cell]);
            if(!(expected_scores[cell]>=interval.low && expected_scores[cell]<=interval.high) ||
                (errors[cell]==0.0f && __float_as_uint(expected_scores[cell])!=__float_as_uint(scores[cell])) ||
                expected_probability[cell]!=probability[cell])atomicAdd(bad,1u);
        }
    }
    if(cell<rows){
        for(unsigned tile=0u;tile<(start+unsigned(cell)/16u+32u)/32u;++tile)
            if(__float_as_uint(expected_scales[cell*(tiles+1u)+tile])!=__float_as_uint(scales[cell*(tiles+1u)+tile]))atomicAdd(bad,1u);
        const float expected=expected_scales[cell*(tiles+1u)+tiles];
        if(!(expected>=denominators[cell*2u] && expected<=denominators[cell*2u+1u]))atomicAdd(bad,1u);
    }
}
void verify_initial_probability(AttentionOutputs& expected,AttentionOutputs& actual,
    JointWorkspace& w,unsigned start,unsigned count,Device& bad){
    check(hipMemset(bad.pointer,0,4u));const unsigned stride=start+count;
    hipLaunchKernelGGL(inspect_initial_probability,dim3((size_t(count)*16u*stride+255u)/256u),dim3(256u),0u,nullptr,
        expected.tensor.scores.as<float>()+guard,expected.tensor.probability.as<uint16_t>()+guard,
        expected.tensor.scales.as<float>()+guard,actual.tensor.scores.as<float>()+guard,w.score_error.as<float>(),
        actual.tensor.probability.as<uint16_t>()+guard,actual.tensor.scales.as<float>()+guard,
        w.denominator_intervals.as<float>(),start,count,stride,bad.as<unsigned>());
    check(hipGetLastError());finish();
    if(download<unsigned>(bad,1u)[0])throw std::runtime_error("initial inline probability boundary differs");
    actual.guards();w.guards();
}
...
JointMetrics verify_joint(AttentionOutputs& expected,Guarded& canonical,Guarded& raw,AttentionOutputs& actual,
    ...
    tail(w.score_error,cells*4u,bad);
    for(auto* data:{&w.center_denominator,&w.pending})tail(*data,size_t(rows)*4u,bad);
    tail(w.unit_scales,size_t(rows)*(tiles+1u)*4u,bad);
    ...
    if(errors || certified!=rows || download<unsigned>(bad,1u)[0]){
        std::fprintf(stderr,"INLINE_PROBABILITY_MISMATCH start=%u count=%u score=%u probability=%u alpha=%u denominator=%u numerator=%u context=%u pending=%u certified=%u tail_or_gb10=%u\n",
...
void joint_safety(const unsigned char* exp,const unsigned char* packed,const unsigned char* rcp){
    ...
    for(const auto shape:shapes)for(unsigned mode=0u;mode<5u;++mode){
        ...
        actual.reset();workspace.reset();initial_probability(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
            prepared,actual,workspace,start,count,n,exp,packed,rcp);finish();
        verify_initial_probability(expected,actual,workspace,start,count,bad);
        ...
        std::fprintf(stderr,"INLINE_PROBABILITY_SAFETY tokens=%u start=%u queries=%u mode=%u pass=1\n",n,start,count,mode);
    }
    std::printf("{\"kind\":\"inline_probability_repair_safety\",\"cases\":%u,\"shapes\":8,\"data_modes\":5,\"interval_bound_failures\":0,\"probability_and_alpha_differences\":0,\"context_differences\":0,\"unresolved_rows\":0,\"full_original_pv_observer\":true,\"initial_probability_independently_checked\":true,\"guards_pass\":true,\"unused_tails_pass\":true,\"immutable_inputs\":true}\n",cases);
}
...
void joint_capture(unsigned tokens,const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    uint64_t score_cells=0u,initial_exact=0u,fallback_rows=0u;unsigned cpu_dots=0u;
    for(unsigned start=0;start<tokens;start+=query_batch){
        ...
        actual.reset();workspace.reset();initial_probability(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
            prepared,actual,workspace,start,count,tokens,exp,packed,rcp);finish();
        verify_initial_probability(expected,actual,workspace,start,count,bad);
        for(unsigned attempt=0u;attempt<4u;++attempt)for(unsigned position=0u;position<2u;++position){
            ...
            if(!variant){
                ...
                if(!attempt){exact_scores+=result.exact_scores;exact_numerators+=result.exact_numerators;
                    unsigned statistics[2]{};check(hipMemcpy(statistics,workspace.statistics.data(),8u,hipMemcpyDeviceToHost));
                    initial_exact+=statistics[0];fallback_rows+=statistics[1];}
            }
        }
    ...
    for(unsigned variant=0u;variant<2u;++variant){
        ...
        std::printf("{\"kind\":\"inline_probability_repair_capture\",\"tokens\":%u,\"source_capture_tokens\":7169,\"repeated_rows\":%u,\"variant\":%u,\"inline_probability_repair\":%s,\"initial_original_scores\":%llu,\"complete_qk_fallback_rows\":%llu,\"query_batch\":128,\"causal_score_cells\":%llu,\"output_cells\":%u,\"gb10_context_cells\":29364224,\"cpu_dots\":%u,\"original_score_replays\":%llu,\"original_pv_replays\":%llu,\"completed_attention_samples_ms\":[%.9f,%.9f,%.9f],\"median_completed_attention_ms\":%.9f,\"common_preparation_ms\":%.9f,\"common_exp_build_ms\":%.9f,\"common_exp_verify_ms\":%.9f,\"interval_bound_failures\":0,\"probability_and_alpha_differences\":0,\"context_differences\":0,\"gb10_context_mismatches\":0,\"unresolved_rows\":0,\"all_attempts_checked\":true,\"warmups_per_slab\":1,\"timed_attempts_per_slab\":3,\"full_original_pv_observer\":true,\"initial_probability_independently_checked\":true,\"reference_is_compute_input\":false,\"full_fallback_in_timing\":true,\"redzones_and_unused_tails_pass\":true,\"immutable_inputs\":true,\"model_loaded\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            tokens,tokens-7169u,variant,variant?"true":"false",(unsigned long long)(variant?initial_exact:0u),(unsigned long long)(variant?fallback_rows:0u),(unsigned long long)score_cells,tokens*4096u,cpu_dots,
...
}catch(const std::exception& e){std::fprintf(stderr,"inline_probability_repair_error=%s\n",e.what());return 2;}
