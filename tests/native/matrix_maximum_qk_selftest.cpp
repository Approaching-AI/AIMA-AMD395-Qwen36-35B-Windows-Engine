...
#include "../../native/providers/ck_fmha/matrix_maximum_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
struct MatrixRows {
    unsigned tokens;
    size_t qbytes, kbytes, qflag_bytes, kflag_bytes;
    Device qp, kp, qf, kf;
    std::vector<unsigned char> expected_keys, expected_key_flags;
    double key_ms = 0.0;
    MatrixRows(unsigned n, unsigned count, const uint16_t* device_key, const uint16_t* host_key)
        : tokens(n), qbytes(size_t(count) * kQueryHeads * kHeadDim * 4u),
          kbytes(size_t(n) * kKvHeads * kHeadDim * 4u), qflag_bytes(size_t(count) * kQueryHeads * 4u),
          kflag_bytes(size_t(n) * kKvHeads * 4u), qp(qbytes + 2u * guard), kp(kbytes + 2u * guard),
          qf(qflag_bytes + 2u * guard), kf(kflag_bytes + 2u * guard) {
        check(hipMemset(kp.pointer, 0xa5, kbytes + 2u * guard));
        check(hipMemset(kf.pointer, 0xa5, kflag_bytes + 2u * guard));
        const auto begin = std::chrono::steady_clock::now();
        check(hipMemset(key_flags(), 0, kflag_bytes));
        hipLaunchKernelGGL((qrt_matrix_maximum_qk::prepare<true>),
            dim3((size_t(n) * kKvHeads * kHeadDim + 255u) / 256u), dim3(256u), 0u, nullptr,
            device_key, key(), key_flags(), n);
        check(hipGetLastError()); finish(); key_ms = elapsed(begin);
        expected_keys = cpu_encoding(true, host_key, 0u, n, kbytes);
        expected_key_flags = cpu_flags(true, host_key, 0u, n, kflag_bytes);
        verify_keys();
    }
    uint32_t* query() { return reinterpret_cast<uint32_t*>(qp.as<unsigned char>() + guard); }
    uint32_t* key() { return reinterpret_cast<uint32_t*>(kp.as<unsigned char>() + guard); }
    unsigned* query_flags() { return reinterpret_cast<unsigned*>(qf.as<unsigned char>() + guard); }
    unsigned* key_flags() { return reinterpret_cast<unsigned*>(kf.as<unsigned char>() + guard); }
    void reset_query() {
        check(hipMemset(qp.pointer, 0xa5, qbytes + 2u * guard));
        check(hipMemset(qf.pointer, 0xa5, qflag_bytes + 2u * guard));
    }
    std::vector<unsigned char> cpu_encoding(bool key, const uint16_t* input, unsigned start, unsigned count, size_t capacity) const {
        std::vector<unsigned char> expected(capacity + 2u * guard, 0xa5u);
        const unsigned heads = key ? kKvHeads : kQueryHeads;
        if (size_t(count) * heads * kHeadDim * 4u > capacity) throw std::runtime_error("matrix maximum capacity");
        for (size_t i = 0u; i < size_t(count) * heads * kHeadDim; ++i) {
            const unsigned row = unsigned(i / kHeadDim), feature = unsigned(i % kHeadDim);
            const uint16_t x = input[size_t(start) * heads * kHeadDim + i];
            const int exponent = int((x >> 7u) & 255u) - 127;
            const uint16_t encoded = !(x & 0x7fffu) ? 0u : exponent >= -12 && exponent <= 12
                ? uint16_t((127 + 5 * exponent) << 7u) : 0x8000u;
            const uint32_t packed = (uint32_t(x) << 16u) | encoded;
            const size_t destination = key ? (size_t(row % heads) * kHeadDim + feature) * tokens + row / heads : i;
            std::memcpy(expected.data() + guard + destination * 4u, &packed, 4u);
        }
        return expected;
    }
    std::vector<unsigned char> cpu_flags(bool key, const uint16_t* input, unsigned start, unsigned count, size_t capacity) const {
        std::vector<unsigned char> expected(capacity + 2u * guard, 0xa5u);
        const unsigned heads = key ? kKvHeads : kQueryHeads;
        for (unsigned row = 0u; row < count * heads; ++row) {
            unsigned invalid = 0u;
            for (unsigned i = 0u; i < kHeadDim; ++i) {
                const uint16_t x = input[(size_t(start) * heads + row) * kHeadDim + i];
                const unsigned e = (x >> 7u) & 255u;
                invalid |= unsigned((x & 0x7fffu) && (e < 64u || e > 190u));
            }
            std::memcpy(expected.data() + guard + row * 4u, &invalid, 4u);
        }
        return expected;
    }
    void verify_query(const uint16_t* input, unsigned start, unsigned count) {
        unchanged(qp, cpu_encoding(false, input, start, count, qbytes));
        unchanged(qf, cpu_flags(false, input, start, count, qflag_bytes));
    }
    void verify_keys() { unchanged(kp, expected_keys); unchanged(kf, expected_key_flags); }
};
void staged_variant(unsigned variant, const uint16_t* q, const uint16_t* k, float* out,
    ...
    if (!variant) {
        ...
        // The candidate owns and times its query encoding and original-domain flags.
        check(hipMemset(matrix.query_flags(), 0, size_t(count) * kQueryHeads * 4u));
        hipLaunchKernelGGL((qrt_matrix_maximum_qk::prepare<false>),
            dim3((size_t(count) * kQueryHeads * kHeadDim + 255u) / 256u), dim3(256u), 0u, nullptr,
            q + size_t(start) * kQueryHeads * kHeadDim, matrix.query(), matrix.query_flags(), count);
        check(hipGetLastError());
#define MATRIX_MAX_QK_CASE(w) if (variant == w) hipLaunchKernelGGL((qrt_matrix_maximum_qk::scores<w>), grid, dim3(kThreads), 0u, nullptr, q, k, matrix.query(), matrix.key(), matrix.query_flags(), matrix.key_flags(), out, start, count, stride, key_stride)
        MATRIX_MAX_QK_CASE(64u); else MATRIX_MAX_QK_CASE(128u); else MATRIX_MAX_QK_CASE(256u);
        else throw std::runtime_error("invalid matrix maximum QK window");
#undef MATRIX_MAX_QK_CASE
    }
    check(hipGetLastError());
}
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("byte-residue32 QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"matrix_maximum_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"hardware_error_bound_proven\":false,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"matrix_maximum_qk_capture\",\"variant\":%u,\"query_rows\":16,\"key_columns\":16,\"window\":%u,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"hardware_error_bound_proven\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,variant?variant:128u,tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
