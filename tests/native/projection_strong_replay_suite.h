...
__global__ void classify_strong_projection_rows(const uint16_t* source, unsigned* flags,
    unsigned rows, unsigned width) {
    const unsigned row=blockIdx.x;if(row>=rows)return;
    __shared__ unsigned invalid;if(!threadIdx.x)invalid=0u;__syncthreads();
    unsigned bad=qrt_sm121_range::bounded_shape(width)?0u:2u;
    if(!width || width>4096u || (width&15u))bad|=4u;
    for(unsigned k=threadIdx.x;k<width;k+=blockDim.x) {
        const uint16_t value=source[size_t(row)*width+k];
        bad|=unsigned(!qrt_sm121_float_alignment::eligible(value));
        bad|=unsigned(!qrt_sm121_range::bounded_operand(value))<<1u;
        bad|=unsigned(!qrt_sm121_strong_float::eligible(value))<<2u;
    }
    if(bad)atomicOr(&invalid,bad);__syncthreads();
    if(!threadIdx.x)flags[row]=(~invalid)&7u;
}
template<unsigned Variant>
__global__ void strong_projection_replay_kernel(const uint16_t* weights, const uint16_t* inputs,
    ...
    else if ((common_flags & 4u) && width <= 4096u)
        value = qrt_sm121_strong_float::dot<lanes, staging>(inputs + size_t(token) * width,
            weights + size_t(row) * width, width, true);
    else
        value = qrt_sm121_scalar_projection::validated_dot<lanes, staging>(inputs + size_t(token) * width,
            weights + size_t(row) * width, width, eligible);
...
void complete_strong_projection() {
...
void run_strong_projection_replays(DeviceBuffer<uint16_t>& dw, DeviceBuffer<uint16_t>& di,
...
#define QRT_SCALAR_REPLAY_CASE(v) if (variant == v) hipLaunchKernelGGL(strong_projection_replay_kernel<v>, grid, dim3(256u), 0u, nullptr, dw.data(), di.data(), dfw.data(), dfi.data(), indices.data(), dout.data(), raw.data(), rows, width, unsigned(selected.size()))
            QRT_SCALAR_REPLAY_CASE(0u); QRT_SCALAR_REPLAY_CASE(1u); QRT_SCALAR_REPLAY_CASE(2u);
#undef QRT_SCALAR_REPLAY_CASE
        ...
        std::cout << "{\"type\":\"strong_projection_real_replay\",\"variant\":" << variant
