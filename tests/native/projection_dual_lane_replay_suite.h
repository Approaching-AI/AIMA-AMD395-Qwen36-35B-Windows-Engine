...
#include "../../native/providers/moe_accumulator/sm121_dual_lane_projection.h"

namespace projection_safety_test {

template<unsigned Variant>
__global__ void dual_lane_projection_replay_kernel(const uint16_t* weights, const uint16_t* inputs,
    ...
    constexpr unsigned lanes = Variant == 0u ? 4u : 2u;
    ...
    if constexpr (Variant == 0u)
        ...
        value = qrt_sm121_dual_lane::dot(inputs + size_t(token) * width,
            weights + size_t(row) * width, width, eligible);
...
void run_dual_lane_projection_replays(DeviceBuffer<uint16_t>& dw, DeviceBuffer<uint16_t>& di,
    ...
    for (unsigned variant = 0u; variant < 2u; ++variant) {
        ...
        for (unsigned attempt = 0u; attempt < 4u; ++attempt) {
            ...
            const unsigned lanes = variant == 0u ? 4u : 2u;
            const dim3 grid((unsigned(selected.size()) * lanes + 255u) / 256u);
#define QRT_SCALAR_REPLAY_CASE(v) if (variant == v) hipLaunchKernelGGL(dual_lane_projection_replay_kernel<v>, grid, dim3(256u), 0u, nullptr, dw.data(), di.data(), dfw.data(), dfi.data(), indices.data(), dout.data(), raw.data(), rows, width, unsigned(selected.size()))
            QRT_SCALAR_REPLAY_CASE(0u); QRT_SCALAR_REPLAY_CASE(1u);
#undef QRT_SCALAR_REPLAY_CASE
        ...
        std::cout << "{\"type\":\"dual_lane_projection_real_replay\",\"variant\":" << variant
            << ",\"lanes\":" << (variant == 0u ? 4u : 2u)
