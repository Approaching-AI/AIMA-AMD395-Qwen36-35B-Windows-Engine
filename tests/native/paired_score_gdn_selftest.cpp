...
void paired_launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,
 ...
 require(variant<=2u,"invalid paired score GDN variant");
...
#define QRT_LIFETIME_OUTPUT_ARGS q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,n,exp
  if(variant!=2u){hipLaunchKernelGGL(scalar::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_LIFETIME_OUTPUT_ARGS);}
...
void paired_run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table,const char* capture_root=nullptr){
...

 {
  ...
  if(mode<2u || mode==3u || mode==4u || capture_root)for(unsigned offset=0u;offset<count;offset+=64u)for(unsigned head:{0u,7u,31u}){
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<6u;++position){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"paired score GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"paired score GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"paired score GDN intermediate or alias differs");
