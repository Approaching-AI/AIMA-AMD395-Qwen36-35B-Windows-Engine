...
#include "../../native/providers/ck_fmha/register_matrix_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
constexpr unsigned variants[] = {0u, 1u, 2u, 3u, 4u, 5u};
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        const unsigned columns = variant == 1u ? 32u : (variant == 3u || variant == 5u) ? 128u : 64u;
        const dim3 grid((stride+columns-1u)/columns,kQueryHeads,(count+15u)/16u);
        if(variant==1u)
            hipLaunchKernelGGL((qrt_matrix_wave_queue_qk::scores<32u,true>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==2u)
            hipLaunchKernelGGL((qrt_register_matrix_qk::scores<64u,false>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==3u)
            hipLaunchKernelGGL((qrt_register_matrix_qk::scores<128u,false>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==4u)
            hipLaunchKernelGGL((qrt_register_matrix_qk::scores<64u,true>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==5u)
            hipLaunchKernelGGL((qrt_register_matrix_qk::scores<128u,true>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid register matrix QK variant");
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    if(c.mode==8u) {
        // Every K16 has a wide normal exponent spread: integer packing has
        // nonzero exceptions, while the scalar-float fallback stays eligible.
        // Fully live tiles therefore exercise the complete per-wave queue.
        for(size_t i=guard;i+guard<q.size();++i)
    ...
    if(c.mode==9u) {
        ...
        // fallback. This also fills queues without using float conversion.
        for(size_t i=guard;i+guard<q.size();++i)
    ...
    for(unsigned variant:variants) {
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"register_matrix_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"register_matrix_qk_capture\",\"variant\":%u,\"query_rows\":16,\"key_columns\":%u,\"wave_queue\":%s,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,!variant?16u:variant==1u?32u:(variant==3u || variant==5u)?128u:64u,(variant==1u || variant>=4u)?"true":"false",tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
...
int main(int argc,char** argv) {
    try {
        ...
        const Case cases[]={{17,0,8,0},{17,0,9,2},{33,0,16,0},{33,1,31,1},{1,0,1,0},{9,0,9,1},{35,3,17,2},{67,33,32,3},{67,64,3,4},
            ...
            {257,129,128,8},{257,129,128,9},{257,127,129,8},{257,128,129,9}};
