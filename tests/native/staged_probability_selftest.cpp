...
void run(unsigned start, unsigned queries, unsigned mode, bool vllm, const unsigned char* table) {
    ...
    hipLaunchKernelGGL(blackwell_staged_probability_kernel, dim3(kQueryHeads, queries), dim3(kThreads), 0u, nullptr,
        ds.as<float>() + guard, dp.as<uint16_t>() + guard, da.as<float>() + guard, start, stride, table, vllm);
    check(hipGetLastError()); finish();
    ...
    std::printf("{\"kind\":\"staged_probability\",\"query_start\":%u,\"queries\":%u,\"mode\":%u,\"vllm_sum\":%s,\"sm121_exp2_table\":%s,\"probability_cells\":%llu,\"scale_cells\":%llu,\"probability_bit_mismatches\":%llu,\"scale_bit_mismatches\":%llu,\"denominator_bit_mismatches\":%llu,\"zero_alphas\":%llu,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",
...
int main(int argc, char** argv) try {
    ...
    return 0;
} catch (const std::exception& e) { std::fprintf(stderr, "staged_probability_selftest_error=%s\n", e.what()); return 2; }
