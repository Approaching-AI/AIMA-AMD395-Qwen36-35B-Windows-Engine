...
#define main qrt_sparse_prior_gdn_main
#include "tiled_scalar_gdn_selftest.cpp"
#undef main
#include "../../native/providers/gdn/sparse_group_matrices.h"
#include <array>
#include <string>
namespace sparse=qrt_fla_sparse_group;
size_t sparse_records(unsigned count){return (2u*((count+63u)/64u)+(count+1023u)/1024u)*512u;}
void sparse_launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,
...
#define QRT_SPARSE_WU_ARGS k.data<uint16_t>()+size_t(offset)*2048u,v.data<uint16_t>()+size_t(offset)*4096u,beta.data<uint16_t>()+size_t(offset)*32u,inverse.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,w.data<uint16_t>()+size_t(offset)*4096u,actual_u+size_t(offset)*4096u,n,exp,stats+size_t(offset/64u)*512u
#define QRT_SPARSE_STATE_ARGS k.data<uint16_t>()+size_t(offset)*2048u,actual_u+size_t(offset)*4096u,w.data<uint16_t>()+size_t(offset)*4096u,g.data<float>()+size_t(offset)*32u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,vn.data<uint16_t>()+size_t(offset)*4096u,state.data<float>(),n,exp,stats+size_t(chunks_total+offset/1024u)*512u
#define QRT_SPARSE_OUTPUT_ARGS q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,n,exp,stats+size_t(chunks_total+segments_total+offset/64u)*512u
#define QRT_SPARSE_RUN(shorts,observe) \
  hipLaunchKernelGGL((sparse::wu_kernel<shorts,observe>),dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_SPARSE_WU_ARGS);check(hipGetLastError()); \
  hipLaunchKernelGGL((sparse::state_kernel<8u,shorts,observe>),dim3(16u,32u),dim3(256u),0u,nullptr,QRT_SPARSE_STATE_ARGS);check(hipGetLastError()); \
  hipLaunchKernelGGL((sparse::output_kernel<shorts,observe>),dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_SPARSE_OUTPUT_ARGS);check(hipGetLastError())
  if(variant==1u){if(audit){QRT_SPARSE_RUN(false,true);}else{QRT_SPARSE_RUN(false,false);}}
  else{if(audit){QRT_SPARSE_RUN(true,true);}else{QRT_SPARSE_RUN(true,false);}}
#undef QRT_SPARSE_RUN
#undef QRT_SPARSE_WU_ARGS
#undef QRT_SPARSE_STATE_ARGS
#undef QRT_SPARSE_OUTPUT_ARGS
 }
}
std::array<uint64_t,4> sparse_counts(Device& d,unsigned count,bool written){
 const auto words=read<unsigned>(d,sparse_records(count)*4u);guards(words);std::array<uint64_t,4> total{};
 for(size_t i=0u;i<sparse_records(count)*4u;++i){
 ...
 if(written){
  ...
  require(total[0]==expected && total[1]+total[2]<=total[0] && total[3]>=total[2] && total[3]<=4u*total[2],"incomplete sparse group accounting");
 }
 return total;
}
...
void sparse_run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table,const char* capture_root=nullptr){
 ...
 Device statistics((sparse_records(count)*4u+2u*guard)*4u);std::array<std::array<std::array<uint64_t,4>,3>,2> counts{};
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<6u;++position){
  ...
  const auto begin=std::chrono::steady_clock::now();sparse_launch(variant,count,dq,dk,dv,db,dinv,dg,ds,du,dw,output,h,vn,state,table,alias,statistics,attempt==0u);finish();
  if(attempt){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"sparse-group GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"sparse-group GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"sparse-group GDN intermediate or alias differs");
  const auto work=sparse_counts(statistics,count,variant && !attempt);if(variant && !attempt)counts[unsigned(alias)][variant]=work;
...
int main(int argc,char** argv)try{
 ...
 if(captured)sparse_run(!std::strcmp(argv[2],"q7169")?7169u:8192u,6u,3u,dt,table,argv[3]);else if(timing){sparse_run(8192u,1u,3u,dt,table);sparse_run(8192u,3u,3u,dt,table);}else for(unsigned count:{1u,63u,64u,65u,129u,1023u,1024u,1025u})for(unsigned mode=0u;mode<6u;++mode)sparse_run(count,mode,0u,dt,table);
