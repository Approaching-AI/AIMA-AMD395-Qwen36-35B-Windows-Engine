...
#include "../../native/providers/ck_fmha/redistributed_probability_pv.h"
...
void producer(const uint16_t* q,const uint16_t* kt,const uint16_t* v,const uint16_t* vt,
    ...
    if (!variant) {
        ...
        hipLaunchKernelGGL((qrt_redistributed_probability_pv::produce<16u,256u>),
            dim3(16u,(count+15u)/16u),dim3(256u),0u,nullptr,
            scores,v,p,s,out.output.as<float>(),out.error.as<float>(),out.accumulator.as<float>(),
            out.denominator.as<float>(),start,count,stride,exp,packed,rcp);
        check(hipGetLastError());
    } else if (variant == 2u) {
        hipLaunchKernelGGL((qrt_redistributed_probability_pv::produce<32u,512u>),
            dim3(16u,(count+31u)/32u),dim3(512u),0u,nullptr,
            scores,v,p,s,out.output.as<float>(),out.error.as<float>(),out.accumulator.as<float>(),
            out.denominator.as<float>(),start,count,stride,exp,packed,rcp);
        check(hipGetLastError());
    } else throw std::runtime_error("invalid consumer variant");
}
...
void run_safety(const unsigned char* exp,const unsigned char* packed,const unsigned char* rcp){
    ...
    for(const auto shape:shapes)for(unsigned mode=0u;mode<5u;++mode){
        ...
        producer(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
            prepared,native_control,start,count,n,exp,packed,rcp,0u);finish();
        for(unsigned variant=0u;variant<3u;++variant){
            actual.reset();producer(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
                prepared,actual,start,count,n,exp,packed,rcp,variant);finish();
        ...
        std::fprintf(stderr,"REDISTRIBUTED_PV_SAFETY tokens=%u start=%u queries=%u mode=%u pass=1\n",n,start,count,mode);
    }
    std::printf("{\"kind\":\"redistributed_probability_pv_safety\",\"cases\":%u,\"shapes\":8,\"data_modes\":5,\"pre_replay_native_surfaces_and_complete_replay\":true,\"raw_bit_mismatches\":0,\"guards_pass\":true,\"immutable_inputs\":true}\n",cases);
}
...
void run_capture(unsigned tokens,const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    for(unsigned start=0;start<tokens;start+=query_batch){
        ...
        native_control.reset();producer(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),
            prepared,native_control,start,count,tokens,exp,packed,rcp,0u);finish();
        for(unsigned attempt=0;attempt<4u;++attempt)for(unsigned position=0;position<3u;++position){
            ...
            producer(dq.as<uint16_t>(),dt.as<uint16_t>(),dv.as<uint16_t>(),vt.as<uint16_t>(),prepared,
    ...
    for(unsigned variant=1u;variant<3u;++variant)if(candidates[0]!=candidates[variant])throw std::runtime_error("original PV selection differs");
...
}catch(const std::exception& e){std::fprintf(stderr,"redistributed_probability_pv_error=%s\n",e.what());return 2;}
