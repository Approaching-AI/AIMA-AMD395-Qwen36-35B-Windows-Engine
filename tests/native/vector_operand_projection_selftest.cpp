...
struct Test {
    ...
    Test(unsigned r,unsigned n,unsigned k,std::vector<uint16_t> hw,std::vector<uint16_t> hx,
        ...
        : rows(r),tokens(n),width(k),cells(r*n),captured(!ref.empty()),weights(std::move(hw)),inputs(std::move(hx)),reference(std::move(ref)),
          w(weights.size(),weight_offset),x(inputs.size(),input_offset),pw(size_t(r)*(k/16u)),px(size_t(n)*(k/16u)),wf(r),xf(n),ids(cells),counter(1u),finished(cells),
    ...
    template<unsigned Parts,bool VectorLoads>void produce(){
        hipLaunchKernelGGL((whole::produce<Parts,VectorLoads>),dim3((rows+127u)/128u,(tokens+15u)/16u),dim3(256u),0u,nullptr,
    ...
    void producer(unsigned variant,bool vector_loads,bool audit){
        ...
        if(!variant){
            if(vector_loads)hipLaunchKernelGGL((matrix::produce<64u,1u,true>),dim3((rows+127u)/128u,(tokens+15u)/16u),dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width);
            else hipLaunchKernelGGL((matrix::produce<64u,1u,false>),dim3((rows+127u)/128u,(tokens+15u)/16u),dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width);
            check(hipGetLastError());
        }else if(variant==1u){if(vector_loads)produce<1u,true>();else produce<1u,false>();}
        else if(variant==3u){if(vector_loads)produce<2u,true>();else produce<2u,false>();}
        else{if(vector_loads)produce<4u,true>();else produce<4u,false>();}
    }
    ...
    Measurement run(unsigned variant,bool vector_loads,bool audit){
        ...
        finish();const auto begin=std::chrono::steady_clock::now();prepare();producer(variant,vector_loads,audit);
    ...
    void execute(unsigned mode){
        double samples[10][3]{},prefix_samples[10][3]{};Measurement first[10];
        const unsigned attempts=captured?4u:1u;
        for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<10u;++position){
            const unsigned config=(attempt*3u+position)%10u,variant=config/2u;const bool vector_loads=config%2u;
            const auto result=run(variant,vector_loads,false);
            if(!attempt)first[config]=result;
            else{require(result.selected==first[config].selected&&result.phases==first[config].phases&&result.groups==first[config].groups,"phase work changed between attempts");samples[config][attempt-1u]=result.ms;prefix_samples[config][attempt-1u]=result.prefix_ms;}
        }
        for(unsigned config=0u;config<10u;++config){const auto audited=run(config/2u,bool(config%2u),true);
            require(audited.selected==first[config].selected&&audited.phases==first[config].phases&&audited.groups==first[config].groups,"production/audit work parity");}
        for(unsigned variant=0u;variant<5u;++variant){const auto& a=first[variant*2u];const auto& b=first[variant*2u+1u];
            require(a.selected==b.selected&&a.phases==b.phases&&a.groups==b.groups,"vector loads changed replay work");}
        immutable();
        for(unsigned config=0u;config<10u;++config){const unsigned variant=config/2u;
            std::array<double,3> sorted{samples[config][0],samples[config][1],samples[config][2]};std::sort(sorted.begin(),sorted.end());
            std::array<double,3> prefix{prefix_samples[config][0],prefix_samples[config][1],prefix_samples[config][2]};std::sort(prefix.begin(),prefix.end());
            const auto& result=first[config];const size_t total=size_t(result.selected)*(width/16u);
            std::printf("{\"kind\":\"vector_operand_projection\",\"vector_loads\":%s,\"weight_alignment_offset\":%zu,\"input_alignment_offset\":%zu,\"preparation_producer_selection_ms\":%.6f,\"preparation_producer_selection_samples_ms\":[%.6f,%.6f,%.6f],",config%2u?"true":"false",size_t(reinterpret_cast<uintptr_t>(w.data())%16u),size_t(reinterpret_cast<uintptr_t>(x.data())%16u),captured?prefix[1]:result.prefix_ms,prefix_samples[config][0],prefix_samples[config][1],prefix_samples[config][2]);
            std::printf("\"captured\":%s,\"rows\":%u,\"tokens\":%u,\"width\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%u,\"candidates\":%u,\"original_groups\":%zu,\"executed_groups\":%zu,\"skipped_groups\":%zu,\"phase_input_counts\":[",captured?"true":"false",rows,tokens,width,mode,variant,cells,result.selected,total,result.groups,total-result.groups);
            for(unsigned i=0u;i<result.phases.size();++i)std::printf("%s%u",i?",":"",result.phases[i]);
            std::printf("],\"completed_total_ms\":%.6f,\"completed_samples_ms\":[%.6f,%.6f,%.6f],\"warmups\":%u,\"measured_attempts\":%u,\"cpu_full_dots\":%u,\"cpu_prefix_checks\":%u,\"bf16_mismatches\":0,\"full_replay_raw_mismatches\":0,\"interval_undercoverage\":0,\"whole_centers_and_bounds_unchanged_across_replay_variants\":true,\"raw_centers_intervals_and_replay_work_unchanged_by_loads\":true,\"all_metadata_bounds_checked\":true,\"metadata_immutable_in_audit\":true,\"all_attempts_verified\":true,\"complete_candidate_permutation_checked\":true,\"compacted_carry_membership_checked\":true,\"production_audit_work_parity\":true,\"all_prepared_words_checked\":true,\"redzones_pass\":true,\"unused_work_tail_pass\":true,\"immutable_inputs\":true,\"hardware_error_bound_proven\":false,\"real_model_prompt\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
                captured?sorted[1]:result.ms,samples[config][0],samples[config][1],samples[config][2],unsigned(captured),captured?3u:1u,cpu_dots,cpu_prefixes);
            std::fflush(stdout);
        }
    }
...
int main(int argc,char** argv)try{
    ...
    return 0;
}catch(const std::exception& e){std::fprintf(stderr,"vector_operand_error=%s\n",e.what());return 1;}
