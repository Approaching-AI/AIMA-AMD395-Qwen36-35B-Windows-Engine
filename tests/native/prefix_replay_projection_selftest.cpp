...
struct Test {
    ...
    void producer(unsigned variant){
        ...
        else if(variant==2u)hipLaunchKernelGGL((kernel::produce<2u>),grid,dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width,snapshots.data());
        else hipLaunchKernelGGL((kernel::produce<4u>),grid,dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width,snapshots.data());
        check(hipGetLastError());
    }
    ...
    void verify(unsigned variant,const Measurement& measurement,bool audit){
        ...
        else require(!std::memcmp(c.data(),baseline_centers.data(),size_t(cells)*4u)&&!std::memcmp(e.data(),baseline_errors.data(),size_t(cells)*4u),"checkpoint stores changed coarse producer");
    ...
    void execute(unsigned mode){
        ...
        for(unsigned variant=0u;variant<4u;++variant){
            ...
            std::printf("{\"kind\":\"prefix_replay_projection\",\"captured\":%s,\"rows\":%u,\"tokens\":%u,\"width\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%u,\"candidates\":%u,\"original_groups\":%zu,\"executed_groups\":%zu,\"skipped_groups\":%zu,\"phase_input_counts\":[",captured?"true":"false",rows,tokens,width,mode,variant,cells,result.selected,total,result.groups,total-result.groups);
            for(unsigned i=0u;i<result.phases.size();++i)std::printf("%s%u",i?",":"",result.phases[i]);
            std::printf("],\"completed_total_ms\":%.6f,\"completed_samples_ms\":[%.6f,%.6f,%.6f],\"warmups\":%u,\"measured_attempts\":%u,\"cpu_full_dots\":%u,\"cpu_prefix_checks\":%u,\"bf16_mismatches\":0,\"full_replay_raw_mismatches\":0,\"prefix_and_suffix_undercoverage\":0,\"coarse_producer_raw_unchanged\":true,\"all_attempts_verified\":true,\"complete_candidate_permutation_checked\":true,\"compacted_carry_membership_checked\":true,\"production_audit_work_parity\":true,\"all_prepared_words_checked\":true,\"redzones_pass\":true,\"unused_work_tail_pass\":true,\"immutable_inputs\":true,\"hardware_error_bound_proven\":false,\"real_model_prompt\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
