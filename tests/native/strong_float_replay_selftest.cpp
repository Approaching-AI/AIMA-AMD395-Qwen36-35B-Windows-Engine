...
struct Result { float value; unsigned certified_groups; };
...
__global__ void execute(const uint16_t* left, const uint16_t* right, const unsigned* flags,
    ...
    const float value = qrt_sm121_strong_float::dot<Lanes, Staging>(left + size_t(row) * width,
        right + size_t(row) * width, width, flags[row] != 0u, traces + size_t(row) * (width / 16u));
...
void variant(unsigned rows, unsigned width, Device& left, Device& right,
    ...
    std::printf("{\"kind\":\"strong_float_replay_safety\",\"lanes\":%u,\"staging_groups\":%u,\"rows\":%u,\"width\":%u,\"ordered_groups\":%zu,\"raw_bit_mismatches\":%u,\"certified_groups\":%u,\"fallback_groups\":%zu,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",
...
void run(unsigned rows, unsigned width) {
    ...
    variant<8u, 1u>(rows, width, left, right, output, traces, flags, a, b, expected);
    variant<16u, 1u>(rows, width, left, right, output, traces, flags, a, b, expected);
    variant<16u, 4u>(rows, width, left, right, output, traces, flags, a, b, expected);
    variant<16u, 8u>(rows, width, left, right, output, traces, flags, a, b, expected);
}
int main() try {
