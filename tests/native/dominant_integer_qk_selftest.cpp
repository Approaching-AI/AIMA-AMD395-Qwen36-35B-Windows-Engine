...
#include "../../native/providers/ck_fmha/dominant_integer_qk.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdio>
...
using Row = qrt_dominant_integer_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_dominant_integer_qk::prepare_rows<Kind>),
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            for(unsigned c=0u;c<16u;++c)
                value.original[c]=input[integer_row_input_index(kind,row,c,tokens,start,count)];
            qrt_sm121_integer_core::prepare(value);
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        if(variant==4u)hipLaunchKernelGGL((qrt_dominant_integer_qk::scores<4u>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==8u)hipLaunchKernelGGL((qrt_dominant_integer_qk::scores<8u>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid dominant integer QK variant");
    }
    check(hipGetLastError());
}
std::array<unsigned long long,3> audit_variant(unsigned variant,float* out,unsigned start,unsigned count,
    unsigned stride,unsigned key_stride,MatrixRows& matrix) {
    constexpr unsigned long long marker=0xa5a5a5a5a5a5a5a5ull;
    std::vector<unsigned long long> expected(3u+2u*guard,marker);
    std::fill_n(expected.begin()+guard,3u,0ull);Device counters(expected.size()*8u);upload(counters,expected);
    const dim3 grid((stride+15u)/16u,kQueryHeads,(count+15u)/16u);
    if(variant==4u)hipLaunchKernelGGL((qrt_dominant_integer_qk::scores<4u,true>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride,counters.as<unsigned long long>()+guard);
    else if(variant==8u)hipLaunchKernelGGL((qrt_dominant_integer_qk::scores<8u,true>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride,counters.as<unsigned long long>()+guard);
    else throw std::runtime_error("invalid dominant audit variant");
    check(hipGetLastError());finish();const auto actual=download<unsigned long long>(counters,expected.size());
    for(unsigned i=0u;i<guard;++i)if(actual[i]!=marker || actual[guard+3u+i]!=marker)throw std::runtime_error("dominant audit counter guard");
    const unsigned long long groups=(static_cast<unsigned long long>(count)*(2u*start+count+1u)/2u)*kQueryHeads*16u;
    if(actual[guard]+actual[guard+1u]!=groups || actual[guard+2u]>16u*actual[guard])throw std::runtime_error("dominant audit coverage");
    return {actual[guard],actual[guard+1u],actual[guard+2u]};
}
...
void run(Case c) {
    ...
    std::array<unsigned long long,3> control_stats{};
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("dominant integer QK differs from original scores");
            }
        }
        ...
        std::array<unsigned long long,3> stats{};
        if(variant) {
            Device audit((cells+2u*guard)*4u);check(hipMemset(audit.pointer,0xa5,(cells+2u*guard)*4u));
            stats=audit_variant(variant,audit.as<float>()+guard,c.start,c.count,stride,c.tokens,matrix);
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"dominant_integer_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"audit_invoked\":%u,\"audit_fast_groups\":%llu,\"audit_original_groups\":%llu,\"audit_compensated_pairs\":%llu,\"production_diagnostic_parity\":true,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells,unsigned(variant!=0u),stats[0],stats[1],stats[2]);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    unsigned long long totals[variant_count][3]{};
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned position=0u;position<variant_count;++position) {
            ...
            if(variant) {
                ...
                const auto stats=audit_variant(variant,audit.as<float>()+guard,start,count,stride,tokens,matrix);
                ...
                for(unsigned i=0u;i<3u;++i)totals[mode][i]+=stats[i];
    ...
    for(unsigned i=0u;i<3u;++i)if(totals[1][i]!=totals[2][i])throw std::runtime_error("matrix windows changed arithmetic path counts");
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"dominant_integer_qk_capture\",\"variant\":%u,\"query_rows\":16,\"key_columns\":16,\"matrix_window_groups\":%u,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"audit_invoked\":%u,\"audit_fast_groups\":%llu,\"audit_original_groups\":%llu,\"audit_compensated_pairs\":%llu,\"production_diagnostic_parity\":true,\"window_arithmetic_counts_equal\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,variant,tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
            preparation_ms,sorted[1],sorted[1]+preparation_ms,samples[mode][0],samples[mode][1],samples[mode][2],maximum_stage_ms[mode],unsigned(variant!=0u),totals[mode][0],totals[mode][1],totals[mode][2]);
        std::fflush(stdout);
    }
}
