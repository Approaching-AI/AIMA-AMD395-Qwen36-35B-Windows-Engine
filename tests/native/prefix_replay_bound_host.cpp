...
template<unsigned Groups, unsigned Width> void run(){
 ...
 std::printf("{\"kind\":\"prefix_replay_bound_host\",\"groups_per_block\":%u,\"width\":%u,\"cases\":%u,\"native_error_modes\":3,\"original_prefix_checkpoints\":%zu,\"suffix_interval_checks\":%u,\"certified_suffixes\":%zu,\"initial_selected\":%zu,\"selected_stop_counts\":[%zu,%zu,%zu,%zu],\"selected_original_groups\":%zu,\"skipped_original_groups\":%zu,\"undercoverage\":0,\"false_certificates\":0,\"native_matrix_executed\":false,\"hardware_error_bound_proven\":false}\n",Groups,width,cases,checkpoints,cases*9u,certificates,selected,stopped[0],stopped[1],stopped[2],stopped[3],selected*(width/16u),saved_groups);
...
int main(){
 ...
 for(float invalid:{-1.0f,std::numeric_limits<float>::infinity(),-std::numeric_limits<float>::infinity(),std::numeric_limits<float>::quiet_NaN()}){
  float out=123.0f;
  assert(!qrt_sm121_prefix_replay_bound::certificate({0.0f,invalid},{0.0f,1.0f},0.0f,&out)&&out==123.0f);
  assert(!qrt_sm121_prefix_replay_bound::certificate({0.0f,0.0f},{0.0f,invalid},0.0f,&out)&&out==123.0f);
 }
 float unchanged=123.0f;
 assert(!qrt_sm121_prefix_replay_bound::certificate({0.0f,2.0f},{0.0f,1.0f},0.0f,&unchanged)&&unchanged==123.0f);
 run<4u,1024u>();run<4u,4096u>();run<8u,4096u>();run<16u,4096u>();
}
