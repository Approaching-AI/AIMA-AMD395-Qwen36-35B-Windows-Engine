#include "../../native/providers/ck_fmha/f32_carry_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
void staged_variant(unsigned variant, const uint16_t* q, const uint16_t* k, float* out,
    ...
    const dim3 grid((stride+kTiledExactKeys-1u)/kTiledExactKeys,kQueryHeads,(count+kTiledExactQueries-1u)/kTiledExactQueries);
    if (variant == 0u)
        hipLaunchKernelGGL(qrt_blackwell_attention::blackwell_float_alignment_scores_kernel,
            grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
    else if (variant == 1u)
        hipLaunchKernelGGL((qrt_f32_carry_qk::scores<0u>),
            grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
    else if (variant == 2u)
        hipLaunchKernelGGL((qrt_f32_carry_qk::scores<1u>),
            grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
    else throw std::runtime_error("invalid f32 carry variant");
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"f32_carry_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    std::printf("{\"kind\":\"f32_carry_original_q7169\",\"ineligible_q_values\":%zu,\"ineligible_k_values\":%zu,\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"original_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"scalar_float_query_ms\":%.6f,\"f32_bits_query_ms\":%.6f,\"f32_rtz_query_ms\":%.6f,\"maximum_completed_slab_ms\":[%.6f,%.6f,%.6f],\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",size_t(ineligible_q),size_t(ineligible_k),compared,cpu_dots,original_ms,transpose_ms,staged_ms[0],staged_ms[1],staged_ms[2],maximum_stage_ms[0],maximum_stage_ms[1],maximum_stage_ms[2]);
}
}
