...
constexpr unsigned variants[] = {18u};
...
void staged_variant(unsigned variant, const Row* q, const Row* k, float* out,
    ...
    if (variant != 18u) throw std::runtime_error("invalid wide integer variant");
    hipLaunchKernelGGL(qrt_wide_integer_qk::scores,
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"wide_integer_qk_safety\",\"core_bits\":18,\"row_bytes\":132,\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    std::printf("{\"kind\":\"wide_integer_original_q7169\",\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"original_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"wide_query_and_encoding_ms\":%.6f,\"key_encoding_ms\":%.6f,\"maximum_completed_slab_ms\":[%.6f],\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",compared,cpu_dots,original_ms,transpose_ms,staged_ms[0],key_encoding_ms,maximum_stage_ms[0]);
}
}
