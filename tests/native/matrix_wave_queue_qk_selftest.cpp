...
#include "../../native/providers/ck_fmha/matrix_wave_queue_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        const unsigned columns = variant == 3u ? 32u : 64u;
        const dim3 grid((stride+columns-1u)/columns,kQueryHeads,(count+15u)/16u);
        if(variant==1u)
            hipLaunchKernelGGL((qrt_matrix_wave_queue_qk::scores<64u,false>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==2u)
            hipLaunchKernelGGL((qrt_matrix_wave_queue_qk::scores<64u,true>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==3u)
            hipLaunchKernelGGL((qrt_matrix_wave_queue_qk::scores<32u,true>),grid,dim3(kThreads),0u,nullptr,
                matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid matrix wave queue QK variant");
    }
    check(hipGetLastError());
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"matrix_wave_queue_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
