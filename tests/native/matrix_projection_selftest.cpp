...
#include <thread>
#include <vector>
namespace coop=qrt_sm121_matrix_projection;
...
void run(unsigned rows,unsigned tokens,unsigned width){
 const size_t cells=size_t(rows)*tokens,groups=width/16u,mw=(cells+31u)/32u,tw=cells*groups*3u;
 ...
 Device w((weights.size()+2u*guard)*2u),in((inputs.size()+2u*guard)*2u);
 ...
 for(unsigned pattern=0u;pattern<4u;++pattern){
  ...
  for(unsigned variant=0u;variant<2u;++variant){
   ...
   check(coop::launch(w.data<uint16_t>(),weights.size(),in.data<uint16_t>(),inputs.size(),mask.data<unsigned>(),mw,out.data<float>(),cells,rows,tokens,width,variant,nullptr,trace.data<uint32_t>(),tw));finish();
   const auto actual=read<unsigned>(out,cells),states=read<unsigned>(trace,tw);guards(actual);guards(states);
   if(std::memcmp(actual.data()+guard,expected_output.data(),cells*4u))for(size_t c=0u;c<cells;++c)if(actual[guard+c]!=expected_output[c]){std::fprintf(stderr,"variant=%u rows=%u tokens=%u k=%u pattern=%u cell=%zu actual=%08x expected=%08x\n",variant,rows,tokens,width,pattern,c,actual[guard+c],expected_output[c]);break;}
   ...
   out.reset();check(coop::launch(w.data<uint16_t>(),weights.size(),in.data<uint16_t>(),inputs.size(),mask.data<unsigned>(),mw,out.data<float>(),cells,rows,tokens,width,variant,nullptr));finish();
   ...
   std::printf("{\"kind\":\"matrix_projection\",\"rows\":%u,\"tokens\":%u,\"k\":%u,\"variant\":%u,\"pattern\":%u,\"candidates\":%zu,\"k16_states\":%zu,\"raw_bit_mismatches\":0,\"production_trace_parity\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",rows,tokens,width,variant,pattern,selected.size(),selected.size()*groups);
  }
 }
 for(unsigned side=0u;side<2u;++side){const auto& input=side?inputs:weights;const auto raw=read<uint16_t>(side?in:w,input.size());guards(raw);require(!std::memcmp(raw.data()+guard,input.data(),input.size()*2u),"operand changed");}
}
int main(){try{hipDeviceProp_t p{};check(hipGetDeviceProperties(&p,0));require(!std::strncmp(p.gcnArchName,"gfx1151",7u),"requires gfx1151");const unsigned shapes[][3]={{1u,1u,16u},{17u,19u,272u},{33u,17u,2048u},{35u,33u,4096u},{17u,17u,8192u},{65u,3u,4112u}};for(const auto& s:shapes)run(s[0],s[1],s[2]);return 0;}catch(const std::exception& e){std::fprintf(stderr,"%s\n",e.what());return 1;}}
