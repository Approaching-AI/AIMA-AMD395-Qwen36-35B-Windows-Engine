...
#include "../../native/providers/ck_fmha/microtile_half_qk.h"
#include "../../native/providers/ck_fmha/half_f32_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        if(variant==1u) hipLaunchKernelGGL((qrt_microtile_half_qk::scores<true>),grid,dim3(kThreads),0u,nullptr,
            matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else if(variant==2u) hipLaunchKernelGGL((qrt_microtile_half_qk::scores<false>),grid,dim3(kThreads),0u,nullptr,
            matrix.query(),matrix.key(),out,start,count,stride,key_stride);
...
__global__ void paired_group_values(const Row* left,const Row* right,const uint32_t* carry,
    ...
    const bool normal_input = !input.significand || (input.significand >= 0x800000u &&
        input.significand <= 0xffffffu && input.exponent >= -126 && input.exponent <= 127);
    float output_value;
    if constexpr(FullCarry) value=qrt_sm121_scaled_half_products::accumulate<true>(input,left[i],right[i],&path);
    else if (normal_input && qrt_sm121_half_f32_carry::accumulate(
            qrt_q1_moe_hawkeye::value_to_float(input), left[i], right[i], &output_value)) {
        const uint32_t bits = qrt_sm121_f32_carry::bits(output_value);
        const unsigned biased = (bits >> 23u) & 255u;
        value = biased ? qrt_q1_moe_hawkeye::Value{0x800000u | (bits & 0x7fffffu),
            int16_t(int(biased)-127), bool(bits >> 31u)} : qrt_q1_moe_hawkeye::Value{0u,-133,false};
        const unsigned active = (left[i].control & right[i].control) >> 16u;
        path = !active ? 1u : active == 65535u ? 2u : 3u;
    } else value = qrt_sm121_scaled_half_products::accumulate(input,left[i],right[i]);
...
void paired_preflight() {
    ...
    for(unsigned variant:{1u,2u}) {
        ...
        if(variant==1u)hipLaunchKernelGGL((paired_group_values<true>),dim3((count+255u)/256u),dim3(256u),0u,nullptr,
            a.as<Row>()+guard,b.as<Row>()+guard,c.as<uint32_t>()+guard,out.as<uint32_t>()+guard,paths.as<unsigned>(),count);
        else hipLaunchKernelGGL((paired_group_values<false>),dim3((count+255u)/256u),dim3(256u),0u,nullptr,
            a.as<Row>()+guard,b.as<Row>()+guard,c.as<uint32_t>()+guard,out.as<uint32_t>()+guard,paths.as<unsigned>(),count);
        ...
        std::printf("{\"kind\":\"microtile_half_preflight\",\"variant\":%u,\"all_bf16_encodings\":65536,\"controls\":16,\"original_products\":2097152,\"raw_carry_states\":131072,\"general_encoding_groups\":65536,\"scaled_normal_groups\":65536,\"native_path_fallback_zero_full_partial\":[%u,%u,%u,%u],\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",variant,counts[0],counts[1],counts[2],counts[3]);
        std::fflush(stdout);
    }
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"microtile_half_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
