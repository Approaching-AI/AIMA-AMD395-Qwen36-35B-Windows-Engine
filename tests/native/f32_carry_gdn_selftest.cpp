...
#include "../../native/providers/gdn/f32_carry_matrices.h"
#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstdio>
...
namespace compact=qrt_fla_f32_carry;
...
void launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,Device& inverse,Device& g,Device& scores,Device& u,Device& w,Device& output,Device& h,Device& vn,Device& state,Device& table,bool alias,PhaseEvents& phases){
 ...
 if(!variant){
  ...
  hipLaunchKernelGGL(compact::wu_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,k.data<uint16_t>(),v.data<uint16_t>(),beta.data<uint16_t>(),inverse.data<uint16_t>(),g.data<float>(),w.data<uint16_t>(),actual_u,count,exp);check(hipGetLastError());
  phases.mark(1u);
  hipLaunchKernelGGL((compact::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,k.data<uint16_t>(),actual_u,w.data<uint16_t>(),g.data<float>(),h.data<uint16_t>(),vn.data<uint16_t>(),state.data<float>(),count,exp);check(hipGetLastError());
  phases.mark(2u);
  hipLaunchKernelGGL(compact::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,q.data<uint16_t>(),vn.data<uint16_t>(),h.data<uint16_t>(),g.data<float>(),scores.data<uint16_t>(),output.data<float>(),count,exp);check(hipGetLastError());
  phases.mark(3u);
...
void run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table){
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<4u;++position){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"compact-carry GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"compact-carry GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"compact-carry GDN intermediate or alias differs");
