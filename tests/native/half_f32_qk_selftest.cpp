...
constexpr unsigned variants[] = {0u, 1u, 64u, 128u, 256u};
...
#define HALF_F32_CASE(w) if(variant==w) hipLaunchKernelGGL((qrt_half_f32_qk::scores<w>),grid,dim3(kThreads),0u,nullptr,q,k,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        if(variant==1u) hipLaunchKernelGGL((qrt_scaled_half_qk::scores<128u>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride);
        else HALF_F32_CASE(64u);
        else HALF_F32_CASE(128u);
        else HALF_F32_CASE(256u);
        else throw std::runtime_error("invalid half FP32 QK variant");
#undef HALF_F32_CASE
    }
    check(hipGetLastError());
}
...
__global__ void paired_group_values(const Row* left,const Row* right,const uint32_t* carry,
    ...
    float output_value;
    if (normal_input && qrt_sm121_half_f32_carry::accumulate(
...
void paired_preflight() {
    ...
    std::printf("{\"kind\":\"half_f32_preflight\",\"all_bf16_encodings\":65536,\"controls\":16,\"original_products\":2097152,\"raw_carry_states\":131072,\"general_encoding_groups\":65536,\"scaled_normal_groups\":65536,\"native_path_fallback_zero_full_partial\":[%u,%u,%u,%u],\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",counts[0],counts[1],counts[2],counts[3]);
    std::fflush(stdout);
}
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"half_f32_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"half_f32_captured_qk\",\"variant\":%u,\"query_rows\":16,\"key_columns\":%u,\"tokens\":%u,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,16u,tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
