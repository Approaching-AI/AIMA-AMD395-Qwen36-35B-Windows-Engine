...
__global__ void packed_tiles_projection_replay_kernel(const uint16_t* weights, const uint16_t* inputs,
    ...
    if constexpr (Variant == 0u)
        ...
        value = qrt_sm121_packed_tiles::dot<lanes, Variant == 1u ? 1u : Variant == 2u ? 4u : 8u, (Variant > 1u)>(
            inputs + size_t(token) * width, weights + size_t(row) * width, width, true);
    else
        value = qrt_sm121_scalar_projection::validated_dot<lanes, staging>(inputs + size_t(token) * width,
            weights + size_t(row) * width, width, eligible);
...
void run_packed_tiles_projection_replays(DeviceBuffer<uint16_t>& dw, DeviceBuffer<uint16_t>& di,
...
#define QRT_SCALAR_REPLAY_CASE(v) if (variant == v) hipLaunchKernelGGL(packed_tiles_projection_replay_kernel<v>, grid, dim3(256u), 0u, nullptr, dw.data(), di.data(), dfw.data(), dfi.data(), indices.data(), dout.data(), raw.data(), rows, width, unsigned(selected.size()))
...
#undef QRT_SCALAR_REPLAY_CASE
        ...
        std::cout << "{\"type\":\"packed_tiles_projection_real_replay\",\"variant\":" << variant
