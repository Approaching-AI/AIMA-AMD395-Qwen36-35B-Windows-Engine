...
#include "../../native/providers/moe_accumulator/sm121_signed_loss_projection.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdio>
...
struct Test {
    ...
    Test(unsigned r,unsigned n,unsigned k,std::vector<uint16_t> hw,std::vector<uint16_t> hx,
        ...
        :rows(r),tokens(n),width(k),cells(r*n),captured(!ref.empty()),weights(std::move(hw)),inputs(std::move(hx)),reference(std::move(ref)),
         ...
         wf(r),xf(n),ids(cells),counter(1u),centers(cells),errors(cells),upper_errors(cells),output(cells),dc(cells) {
    ...
    void verify(unsigned variant,const Measurement& measurement){
        ...
        for(unsigned cell=0u;cell<cells;++cell){
            ...
            require(!bound::certified({baseline_centers[cell],baseline_errors[cell]})||accepted,"lost original certificate");
...
int main(int argc,char** argv)try{
    ...
    return 0;
}catch(const std::exception& e){std::fprintf(stderr,"signed_loss_projection_error=%s\n",e.what());return 1;}
