...
int main() {
    ...
    for(unsigned raw=0u;raw<65536u;++raw) {
        ...
        const uint32_t packed=decoded::pack(x);
        if((packed>>16u)!=raw || int16_t(packed)!=decoded::exponent(x))return 5;
        ...
        for(unsigned mode=0u;mode<16u;++mode) {
            ...
            for(unsigned i=0u;i<16u;++i) {
                ...
                decoded::set_packed(c,i,decoded::pack(left),decoded::pack(right));
                ++products;
            }
            if(std::memcmp(a.products,b.products,sizeof(a.products)) ||
                a.maximum!=b.maximum || a.first_negative!=b.first_negative ||
                std::memcmp(a.products,c.products,sizeof(a.products)) ||
                a.maximum!=c.maximum || a.first_negative!=c.first_negative)return 3;
    ...
    std::printf("{\"packed_bf16_encodings\":65536,\"eligible_bf16_encodings\":%u,\"groups\":%llu,\"products\":%llu,\"carry_cases_per_group\":7,\"raw_mismatches\":0}\n",
