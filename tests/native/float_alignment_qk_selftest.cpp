#include "../../native/providers/ck_fmha/float_alignment_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
constexpr unsigned variants[] = {0u};
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"float_alignment_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    std::printf("{\"kind\":\"float_alignment_original_q7169\",\"ineligible_q_values\":%zu,\"ineligible_k_values\":%zu,\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"original_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"float_alignment_query_ms\":%.6f,\"maximum_completed_slab_ms\":[%.6f],\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",size_t(ineligible_q),size_t(ineligible_k),compared,cpu_dots,original_ms,transpose_ms,staged_ms[0],maximum_stage_ms[0]);
}
}
