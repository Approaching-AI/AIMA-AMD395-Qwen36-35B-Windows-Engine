...
#include "../../native/providers/moe_accumulator/sm121_observed_prefix_projection.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdio>
...
namespace observed=qrt_sm121_observed_prefix_projection;
...
struct Measurement {double ms=0.0;unsigned selected=0u;std::vector<unsigned> phases;size_t groups=0u;};
struct Test {
    unsigned rows,tokens,width,cells;bool captured;
    std::vector<uint16_t> weights,inputs,reference;
    std::vector<float> canonical,baseline_centers,baseline_errors,observed_errors;
    Buffer<uint16_t> w,x;
    Buffer<Row> pw,px;
    ...
    Test(unsigned r,unsigned n,unsigned k,std::vector<uint16_t> hw,std::vector<uint16_t> hx,
        ...
        : rows(r),tokens(n),width(k),cells(r*n),captured(!ref.empty()),weights(std::move(hw)),inputs(std::move(hx)),reference(std::move(ref)),
          ...
          centers(cells),errors(cells),output(cells),dc(cells),snapshots(size_t(cells)*3u),work_a(cells),work_b(cells) {
        require(rows && tokens && width%256u==0u && width<=8192u,"test shape");w.upload(weights);x.upload(inputs);
    ...
    void producer(unsigned variant){
        ...
        const dim3 grid((rows+127u)/128u,(tokens+15u)/16u);
        if(!variant)hipLaunchKernelGGL((matrix::produce<64u>),grid,dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width);
        else if(variant==1u)hipLaunchKernelGGL((observed::produce<1u>),grid,dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width,snapshots.data());
        else if(variant==2u)hipLaunchKernelGGL((observed::produce<2u>),grid,dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width,snapshots.data());
        else hipLaunchKernelGGL((observed::produce<4u>),grid,dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width,snapshots.data());
        check(hipGetLastError());
    }
    ...
    template<unsigned Parts,bool Audit>void launch(unsigned stage,unsigned n,Work* previous,Work* next){
        for(unsigned offset=0u;offset<n;offset+=window){const unsigned size=std::min(window,n-offset);
            hipLaunchKernelGGL((kernel::phase<Parts,Audit>),dim3((size+63u)/64u),dim3(256u),0u,nullptr,pw.data(),px.data(),ids.data(),previous,
                centers.data(),errors.data(),snapshots.data(),output.data(),next,counter.data(),rows,tokens,width,stage,offset,size,Audit?finished.data():nullptr);
            check(hipGetLastError());}
    }
    Measurement run(unsigned variant,bool audit){
        ids.reset();snapshots.reset();output.reset();check(hipMemset(counter.data(),0,4u));
        if(audit)check(hipMemset(finished.data(),0,size_t(cells)*4u));
        finish();const auto begin=std::chrono::steady_clock::now();prepare();producer(variant);
        ...
        const unsigned parts=variant==2u?2u:4u;unsigned current=result.selected;
        std::vector<unsigned> alive,old_finished;
        if(audit && variant>=2u){const auto selected=ids.read();alive.assign(selected.begin(),selected.begin()+current);old_finished.assign(cells,0u);}
        if(variant<2u){full(true,output.data(),current);finish();result.groups=size_t(current)*(width/16u);}
        else for(unsigned stage=0u;stage<parts;++stage){
            result.phases.push_back(current);result.groups+=size_t(current)*(width/16u/parts);
            ...
            if(audit){
                ...
                for(unsigned i=0u;i<remaining;++i){const auto item=records[i];require(item.cell<cells&&before[item.cell]&&!seen[item.cell]&&!(item.state&0xfffe0000u),"compacted continuation identity or state");
                    ...
                    if(i%step==0u){const auto expected=cpu_prefix(weights.data()+size_t(item.cell%rows)*width,inputs.data()+size_t(item.cell/rows)*width,(stage+1u)*(width/parts));
        ...
        if(variant>=2u)require(!current,"unfinished final continuation");
    ...
    void verify(unsigned variant,const Measurement& measurement,bool audit){
        ...
        else require(!std::memcmp(c.data(),baseline_centers.data(),size_t(cells)*4u),"observed bounds changed raw coarse center");
        if(variant){
            if(observed_errors.empty())observed_errors=e;
            else require(!std::memcmp(e.data(),observed_errors.data(),size_t(cells)*4u),"snapshots changed observed envelope");
        }else require(!std::memcmp(e.data(),baseline_errors.data(),size_t(cells)*4u),"original envelope changed");
        ...
        const auto stages=audit&&variant>=2u?finished.read():std::vector<unsigned>{};
        for(unsigned cell=0u;cell<cells;++cell){
            ...
            if(seen[cell]&&(variant<2u || (audit&&stages[cell]==(variant==2u?2u:4u))))
                require(bound::scalar::bits(out[cell])==bound::scalar::bits(canonical[cell]),"full replay changed raw endpoint");
            if(audit&&variant>=2u)require(seen[cell]?(stages[cell]>=1u&&stages[cell]<=(variant==2u?2u:4u)):stages[cell]==0u,"completion coverage");
        }
        if(variant>=2u){
            const unsigned parts=variant==2u?2u:4u;const auto snap=snapshots.read();marker_tail(snap,size_t(parts-1u)*cells);
            const unsigned checks=captured?256u:cells;
            for(unsigned sample=0u;sample<checks;++sample){const unsigned cell=captured?unsigned((uint64_t(sample)*2654435761ull+1013904223ull)%cells):sample;
                for(unsigned stage=0u;stage<parts-1u;++stage){const auto state=snap[size_t(stage)*cells+cell];
                    const float exact=as_float(cpu_prefix(weights.data()+size_t(cell%rows)*width,inputs.data()+size_t(cell/rows)*width,(stage+1u)*(width/parts)));
                    require(std::abs(double(exact)-double(state.center))<=double(state.error),"native prefix interval undercoverage");
                    if(bound::scalar::finite(state.error)&&bound::scalar::finite(e[cell])){
                        const auto suffix=qrt_sm121_prefix_replay_bound::suffix(state,{c[cell],e[cell]},exact);
        ...
        }else marker_tail(snapshots.read(),0u);
    }
    void immutable(){
    ...
    void execute(unsigned mode){
        double samples[4][3]{};Measurement first[4];
        ...
        for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<4u;++position){
            const unsigned variant=(attempt+position)%4u;const auto result=run(variant,false);
            if(!attempt)first[variant]=result;
            else{require(result.selected==first[variant].selected&&result.phases==first[variant].phases&&result.groups==first[variant].groups,"phase work changed between attempts");samples[variant][attempt-1u]=result.ms;}
        }
        for(unsigned variant=2u;variant<4u;++variant){const auto audited=run(variant,true);
        ...
        for(unsigned variant=0u;variant<4u;++variant){
            ...
            std::printf("{\"kind\":\"observed_prefix_projection\",\"captured\":%s,\"rows\":%u,\"tokens\":%u,\"width\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%u,\"candidates\":%u,\"original_groups\":%zu,\"executed_groups\":%zu,\"skipped_groups\":%zu,\"phase_input_counts\":[",captured?"true":"false",rows,tokens,width,mode,variant,cells,result.selected,total,result.groups,total-result.groups);
            for(unsigned i=0u;i<result.phases.size();++i)std::printf("%s%u",i?",":"",result.phases[i]);
            std::printf("],\"completed_total_ms\":%.6f,\"completed_samples_ms\":[%.6f,%.6f,%.6f],\"warmups\":%u,\"measured_attempts\":%u,\"cpu_full_dots\":%u,\"cpu_prefix_checks\":%u,\"bf16_mismatches\":0,\"full_replay_raw_mismatches\":0,\"prefix_and_suffix_undercoverage\":0,\"coarse_centers_raw_unchanged\":true,\"observed_bounds_unchanged_across_replay_variants\":true,\"all_attempts_verified\":true,\"complete_candidate_permutation_checked\":true,\"compacted_carry_membership_checked\":true,\"production_audit_work_parity\":true,\"all_prepared_words_checked\":true,\"redzones_pass\":true,\"unused_work_tail_pass\":true,\"immutable_inputs\":true,\"hardware_error_bound_proven\":false,\"real_model_prompt\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
...
int main(int argc,char** argv)try{
    ...
    generated(1u,1u,256u,0u);generated(17u,19u,512u,1u);generated(33u,17u,2048u,2u);
    generated(19u,17u,4096u,3u);generated(17u,3u,8192u,4u);generated(67u,19u,4096u,0u);
    return 0;
}catch(const std::exception& e){std::fprintf(stderr,"prefix_replay_error=%s\n",e.what());return 1;}
