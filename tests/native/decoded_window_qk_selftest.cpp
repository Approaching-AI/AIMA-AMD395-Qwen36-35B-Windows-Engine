#include "../../native/providers/ck_fmha/decoded_window_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
void staged_variant(unsigned variant, const uint16_t* q, const uint16_t* k, float* out,
    ...
    if (!variant) {
        ...
        const unsigned rows=variant==4u ? 8u : 16u, keys=variant==4u ? 32u : 16u;
        const dim3 grid((stride+keys-1u)/keys,kQueryHeads,(count+rows-1u)/rows);
        if (variant==1u)
            hipLaunchKernelGGL((qrt_decoded_window_qk::scores<32u>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if (variant==2u)
            hipLaunchKernelGGL((qrt_decoded_window_qk::scores<64u>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if (variant==3u)
            hipLaunchKernelGGL((qrt_decoded_window_qk::scores<128u>),grid,dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        else if (variant==4u)
        ...
        else throw std::runtime_error("invalid decoded QK variant");
    }
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"decoded_window_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    unchanged(dq,q);unchanged(dk,k);unchanged(dt,prepared_keys);
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        const unsigned window=variant==1u?32u:variant==3u?128u:variant?64u:256u;
        const unsigned rows=variant==0u||variant==4u?8u:16u, keys=256u/rows;
        std::printf("{\"kind\":\"decoded_window_original_q7169\",\"variant\":%u,\"window\":%u,\"query_rows\":%u,\"key_columns\":%u,\"ineligible_q_values\":%zu,\"ineligible_k_values\":%zu,\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"original_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"completed_query_ms\":%.6f,\"maximum_completed_slab_ms\":%.6f,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",variant,window,rows,keys,size_t(ineligible_q),size_t(ineligible_k),compared/variant_count,cpu_dots/variant_count,original_ms,transpose_ms,staged_ms[mode],maximum_stage_ms[mode]);
        std::fflush(stdout);
    }
