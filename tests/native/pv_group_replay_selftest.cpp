...
#include "../../native/providers/ck_fmha/pv_group_replay.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
struct Plan {
    ...
    Device pp,vp;
    std::vector<uint32_t> expected_value;
    double value_ms=0.0;
    Plan(unsigned n,unsigned count,const uint16_t* tv,const uint16_t* host_tv)
        :tokens(n),capacity(count),pp_words(size_t(count)*kQueryHeads*((n+15u)/16u)),
         vp_words(size_t(kKvHeads)*kHeadDim*((n+15u)/16u)),pp((pp_words+2u*guard)*4u),vp((vp_words+2u*guard)*4u) {
        check(hipMemset(vp.pointer,0xa5,(vp_words+2u*guard)*4u));finish();
        const auto begin=std::chrono::steady_clock::now();
        hipLaunchKernelGGL((qrt_pv_group_replay::prepare<false>),dim3((vp_words+255u)/256u),dim3(256u),0u,nullptr,
            tv,n,0u,0u,vp.as<uint32_t>()+guard);
        check(hipGetLastError());finish();value_ms=elapsed(begin);
        expected_value=cpu(host_tv,n,0u,0u,false,vp_words);unchanged(vp,expected_value);
    }
    static std::vector<uint32_t> cpu(const uint16_t* source,unsigned stride,unsigned start,
        unsigned count,bool probability,size_t allocation) {
        std::vector<uint32_t> expected(allocation+2u*guard,0xa5a5a5a5u);
        ...
        for(unsigned row=0u;row<rows;++row)for(unsigned group=0u;group<groups;++group) {
            ...
            for(unsigned i=0u;i<16u;++i) {
                const unsigned key=group*16u+i;const uint16_t word=key<extent?source[size_t(row)*stride+key]:0u;
                const unsigned magnitude=word&0x7fffu,exponent=(word>>7u)&255u;
                maximum=std::max(maximum,magnitude);eligible&=!magnitude||(exponent>=64u&&exponent<=190u);
            }
            expected[guard+size_t(row)*groups+group]=maximum|(eligible?0x80000000u:0u);
        }
        return expected;
    }
    void reset(){check(hipMemset(pp.pointer,0xa5,(pp_words+2u*guard)*4u));}
    void prepare(const uint16_t* probability,unsigned start,unsigned count,unsigned stride) {
        ...
        hipLaunchKernelGGL((qrt_pv_group_replay::prepare<true>),dim3((words+255u)/256u),dim3(256u),0u,nullptr,
            probability,stride,start,count,pp.as<uint32_t>()+guard);check(hipGetLastError());
    }
...
#define PV_GROUP_CASE(v,s,f) if(variant==v) hipLaunchKernelGGL((qrt_pv_group_replay::replay<s,f,Audit>),dim3(blocks),dim3(256u),0u,nullptr,probability,tv,plan.pp.as<uint32_t>()+guard,plan.vp.as<uint32_t>()+guard,scales,out,start,queries,output_start,stride,value_stride,rcp,acc,den,indices,count,stats)
        ...
        else PV_GROUP_CASE(2u,false,true);
        else PV_GROUP_CASE(3u,true,true);
        else throw std::runtime_error("PV variant");
#undef PV_GROUP_CASE
    }
    check(hipGetLastError());
}
...
void generated(unsigned start,unsigned queries,unsigned input_mode,unsigned selection_mode) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"pv_group_safety\",\"query_start\":%u,\"query_count\":%u,\"input_mode\":%u,\"selection_mode\":%u,\"variant\":%u,\"cells\":%u,\"selected_cells\":%u,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"unique_candidates\":true,\"complete_cpu_metadata_check\":true,\"inference_acceptance\":false}\n",
...
void captured(const char* qfile,const char* kfile,const char* vfile,const char* reference_file,
    ...
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned position=0u;position<3u;++position) {
            const unsigned variant=1u+(position+start/batch)%3u;
            for(unsigned attempt=0u;attempt<attempts;++attempt) {
    ...
    for(unsigned variant:variants) {
        ...
        std::printf("{\"kind\":\"pv_group_original_q7169\",\"variant\":%u,\"tokens\":7169,\"query_batch\":128,\"external_bf16_cells\":%zu,\"external_bf16_mismatches\":0,\"selected_cells\":%llu,\"cpu_selected_dots\":%u,\"raw_compared_output_cells\":%zu,\"raw_bit_mismatches\":0,\"common_attention_ms\":%.6f,\"query_key_preparation_ms\":%.6f,\"value_transpose_ms\":%.6f,\"value_metadata_ms\":%.6f,\"replay_including_probability_metadata_ms\":%.6f,\"replay_with_all_metadata_ms\":%.6f,\"replay_samples_ms\":[%.6f,%.6f,%.6f],\"maximum_completed_slab_ms\":%.6f,\"warmup_per_slab\":1,\"samples_per_slab\":3,\"audit_groups_total_zero_bounded_float_integer\":[%llu,%llu,%llu,%llu,%llu],\"audit_excluded_from_timing\":true,\"all_attempts_verified\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"unique_candidates\":true,\"complete_cpu_metadata_check\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
...
int main(int argc,char** argv)try {
    ...
    return 0;
}catch(const std::exception& e){std::fprintf(stderr,"pv_group_error=%s\n",e.what());return 1;}
