...
#include "../../native/providers/moe_accumulator/sm121_deferred_loss_projection.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdio>
...
namespace directed=qrt_sm121_signed_loss_projection;
namespace deferred=qrt_sm121_deferred_loss_projection;
...
void finish(){
    ...
    for(;;){const auto s=hipEventQuery(event);if(s==hipSuccess)break;if(s!=hipErrorNotReady)check(s);
        require(std::chrono::steady_clock::now()<deadline,"directional C64 completion deadline");std::this_thread::yield();}
...
const char* names[variants]={"c64_domain_vector","c64_domain_shared64x64","deferred_loss_shared64x64","deferred_loss_shared64x32"};
struct Test {
    ...
    std::vector<deferred::Mask> expected_mw,expected_mx;
    std::vector<float> original_directional_lower,original_directional_upper;
    std::vector<unsigned> expected_wf,expected_xf;
    Buffer<uint16_t> w,x;
    Buffer<Row> pw,px;
    Buffer<deferred::Mask> mw,mx;
    Buffer<unsigned> wf,xf,ids,counter;
    Buffer<float> centers,errors,upper_errors,output,dc;
    ...
    Test(unsigned r,unsigned n,unsigned k,std::vector<uint16_t> hw,std::vector<uint16_t> hx,
        ...
        :rows(r),tokens(n),width(k),cells(r*n),captured(!ref.empty()),weights(std::move(hw)),inputs(std::move(hx)),reference(std::move(ref)),
         ...
         mw(size_t(r)*((k+63u)/64u)),mx(size_t(n)*((k+63u)/64u)),wf(r),xf(n),ids(cells),counter(1u),centers(cells),errors(cells),upper_errors(cells),output(cells),dc(cells) {
        require(rows&&tokens&&width&&width%16u==0u&&width<=16384u,"test shape");w.upload(weights);x.upload(inputs);
        ...
        for(unsigned side=0u;side<2u;++side){
            ...
            auto& masks=side?expected_mx:expected_mw;const unsigned blocks=(width+63u)/64u;
            masks.resize(size_t(flags.size())*blocks);
            for(unsigned row=0u;row<flags.size();++row)for(unsigned block=0u;block<blocks;++block){
                const auto value=loss::summarize(raw.data()+size_t(row)*width+block*64u,std::min(64u,width-block*64u));
                masks[size_t(row)*blocks+block]={value.negative,value.nonzero};}
        }
        // Original directional bounds are an observer computed outside all
        // owner clocks. They never enter candidate production or selection.
        eligibility();
        hipLaunchKernelGGL((directed::produce<64u,true,1u>),dim3((rows+63u)/64u,(tokens+31u)/32u),dim3(256u),0u,nullptr,
            w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),upper_errors.data(),rows,tokens,width);
        check(hipGetLastError());finish();original_directional_lower=errors.read();original_directional_upper=upper_errors.read();
    }
    void prepare(){
    ...
    template<unsigned Rows,bool Prefetch>void shared_producer(){
        ...
        hipLaunchKernelGGL((shared::produce<Rows,Prefetch,true>),dim3((rows+Rows-1u)/Rows,(tokens+Tokens-1u)/Tokens),dim3(256u),0u,nullptr,
    ...
    void producer(unsigned variant){
        eligibility();
        if(variant>=2u){
            hipLaunchKernelGGL(deferred::prepare,dim3((mw.count+7u)/8u),dim3(256u),0u,nullptr,w.data(),mw.data(),rows,width);check(hipGetLastError());
            hipLaunchKernelGGL(deferred::prepare,dim3((mx.count+7u)/8u),dim3(256u),0u,nullptr,x.data(),mx.data(),tokens,width);check(hipGetLastError());
        }
        ...
        else if(variant==2u)hipLaunchKernelGGL((deferred::produce<2u>),dim3((rows+63u)/64u,(tokens+63u)/64u),dim3(256u),0u,nullptr,
            w.data(),x.data(),wf.data(),xf.data(),mw.data(),mx.data(),centers.data(),errors.data(),upper_errors.data(),rows,tokens,width);
        else hipLaunchKernelGGL((deferred::produce<1u>),dim3((rows+63u)/64u,(tokens+31u)/32u),dim3(256u),0u,nullptr,
            w.data(),x.data(),wf.data(),xf.data(),mw.data(),mx.data(),centers.data(),errors.data(),upper_errors.data(),rows,tokens,width);
        check(hipGetLastError());
    }
    Measurement run(unsigned variant){
        mw.reset();mx.reset();ids.reset();centers.reset();errors.reset();upper_errors.reset();output.reset();check(hipMemset(counter.data(),0,4u));finish();
    ...
    void verify(unsigned variant,const Measurement& measurement){
        ...
        for(unsigned cell=0u;cell<cells;++cell){
            ...
            if(variant>=2u)require(e[cell]>=original_directional_lower[cell]&&high>=original_directional_upper[cell],"deferred envelope narrower than original directed bound");
            else require(e[cell]==baseline_errors[cell]&&high==baseline_errors[cell],"control envelope changed");
        ...
        if(variant>=2u){mw.unchanged(expected_mw);mx.unchanged(expected_mx);}
        else {marker_tail(mw.read(),0u);marker_tail(mx.read(),0u);}
    ...
    void execute(unsigned mode){
        ...
        for(unsigned variant=0u;variant<variants;++variant){
            if(variant<2u)require(warm[variant].selected==warm[0].selected,"control candidate count changed");
            auto sorted=std::array<double,3>{samples[variant][0],samples[variant][1],samples[variant][2]};std::sort(sorted.begin(),sorted.end());
            std::printf("{\"kernel\":\"deferred_loss_projection\",\"variant\":\"%s\",\"variant_id\":%u,\"rows\":%u,\"tokens\":%u,\"width\":%u,\"mode\":%u,\"weight_skew_words\":%u,\"input_skew_words\":%u,\"captured\":%s,\"cells\":%u,\"candidates\":%u,\"replay_groups\":%llu,",
            ...
            std::printf("\"bf16_mismatches\":0,\"full_replay_raw_mismatches\":0,\"interval_undercoverage\":0,\"raw_centers_unchanged\":true,\"deferred_envelopes_contain_original\":true,\"deferred_envelopes_unchanged_between_schedules\":true,\"original_directional_envelope_contained\":true,\"complete_candidate_permutation_checked\":true,\"all_attempts_verified\":true,\"all_prepared_words_checked\":true,\"all_prepared_masks_checked\":true,\"redzones_and_unused_tails_pass\":true,\"immutable_inputs\":true,\"hardware_error_bound_proven\":false,\"real_model_prompt\":false,\"inference_acceptance\":false,\"performance_acceptance\":false}\n");
            std::fflush(stdout);
        }
    }
};
void generated(unsigned rows,unsigned tokens,unsigned width,unsigned mode,unsigned weight_offset=0u,unsigned input_offset=0u){
    ...
    if(mode==5u){std::fill(weights.begin(),weights.end(),uint16_t(80u<<7u));std::fill(inputs.begin(),inputs.end(),uint16_t(80u<<7u));}
    if(mode==6u){std::fill(weights.begin(),weights.end(),uint16_t((174u<<7u)|127u));std::fill(inputs.begin(),inputs.end(),uint16_t((174u<<7u)|127u));}
    if(mode==7u){for(size_t i=0u;i<weights.size();++i)weights[i]=uint16_t(((i*37u)&0x807fu)|((80u+i%95u)<<7u));
        for(size_t i=0u;i<inputs.size();++i)inputs[i]=uint16_t(((i*53u)&0x807fu)|((80u+i%95u)<<7u));}
    if(mode==8u||mode==9u){for(auto& v:weights)v=uint16_t((v&0x7fffu)|(mode==9u?0x8000u:0u));for(auto& v:inputs)v&=0x7fffu;}
    if(mode==10u){for(size_t i=0u;i<weights.size();++i)weights[i]=uint16_t(0x3f81u|((i%width>=width/2u)?0x8000u:0u));std::fill(inputs.begin(),inputs.end(),0x3f85u);}
...
int main(int argc,char** argv)try{
    ...
    generated(17u,19u,1024u,5u);generated(19u,17u,8192u,6u);generated(17u,19u,512u,7u);generated(3u,3u,8208u,0u);
    generated(17u,19u,2048u,8u);generated(19u,17u,2048u,9u);generated(17u,19u,4096u,10u);
    return 0;
}catch(const std::exception& e){std::fprintf(stderr,"deferred_loss_projection_error=%s\n",e.what());return 1;}
