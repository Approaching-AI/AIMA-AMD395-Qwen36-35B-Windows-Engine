...
void finish() {
    ...
    for (;;) {
        ...
        require(std::chrono::steady_clock::now() < end,"scaled projection deadline"); std::this_thread::yield();
    }
...
__global__ void compare(const uint16_t* weights,const uint16_t* inputs,
    ...
    float value;
    if constexpr(Candidate) value=scaled_gpu::dot<true>(inputs+size_t(token)*width,weights+size_t(row)*width,width,
...
void run(unsigned rows,unsigned tokens,unsigned width,unsigned mode) {
    ...
    check(scaled_gpu::prepare(dw.data(),dwb.data(),rows,rows,width,nullptr));
    check(scaled_gpu::prepare(di.data(),dib.data(),tokens,tokens,width,nullptr));finish();
    for(unsigned side=0u;side<2u;++side) {
        auto& flags=side?wb:ib;const auto& values=side?weights:inputs;const unsigned count=side?rows:tokens;
        for(unsigned row=0u;row<count;++row) {
            flags[guard+row]=1u;
            for(unsigned k=0u;k<width;++k) flags[guard+row]&=scaled_gpu::scaled::eligible(values[guard+size_t(row)*width+k]);
        }
    }
    ...
    std::printf("{\"kind\":\"scaled_projection_safety\",\"rows\":%u,\"tokens\":%u,\"k\":%u,\"data_mode\":%u,\"cells\":%u,\"ordered_groups\":%zu,\"raw_bit_mismatches\":0,\"canonical_carry_mismatches\":0,\"scalar_aligned_groups\":%llu,\"original_fallback_groups\":%llu,\"unused_counter2\":%llu,\"unused_counter3\":%llu,\"cpu_metadata_pass\":true,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",
...
int main() try {
    ...
    for(const auto& shape:shapes) for(unsigned mode=0u;mode<6u;++mode) run(shape[0],shape[1],shape[2],mode);
    return 0;
} catch(const std::exception& e) {std::fprintf(stderr,"scaled_projection_error=%s\n",e.what());return 2;}
