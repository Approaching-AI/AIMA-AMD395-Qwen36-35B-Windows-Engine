...
#include "../../native/providers/moe_accumulator/sm121_byte_residue32_core.h"
#include <algorithm>
#include <chrono>
#include <cmath>
#include <cstddef>
...
namespace core = qrt_sm121_byte_residue32;
...
__global__ void prepare(const uint16_t* input, core::Row* output, OriginalRow* control, unsigned count) {
    ...
    OriginalRow old{};
    for(unsigned i=0u;i<16u;++i){const uint16_t x=qrt_sm121_integer_core::encode(result.original[i],result.unit);old.high[i/4u]=int(uint32_t(old.high[i/4u])|uint32_t(x>>8u)<<((i&3u)*8u));old.low[i/4u]=int(uint32_t(old.low[i/4u])|uint32_t(x&255u)<<((i&3u)*8u));}
    control[row]=old;
}
template<unsigned Headroom>
...
#pragma unroll
    for (unsigned i = 0u; i < 8u; ++i) {
        ...
        int32_t recovered=0;result.accepted=unsigned(core::recover(result.approximate,uint32_t(result.residue),&recovered));result.recovered=recovered;
...
template<unsigned Headroom,unsigned Variant>
__global__ void sequence(const core::Row* rows,const OriginalRow* control,uint64_t* output,unsigned groups) {
    const unsigned lane=threadIdx.x;uint64_t totals[8]{};
    for(unsigned group=0u;group<groups;++group) {
        const unsigned base=((blockIdx.x*groups+group)%tiles)*32u;
        if constexpr(!Variant) {
            const auto parts=qrt_blackwell_attention::blackwell_integer_prepared_products(control[base+lane%16u],control[base+16u+lane%16u]);
#pragma unroll
            for(unsigned i=0u;i<8u;++i)totals[i]+=uint64_t(int64_t(parts.value[0][i])*65536+(int64_t(parts.value[1][i])+parts.value[2][i])*256+parts.value[3][i]);
        } else {
            const auto parts=core::products(rows[base+lane%16u],rows[base+16u+lane%16u]);
#pragma unroll
            for(unsigned i=0u;i<8u;++i){int32_t value=0;const bool accepted=core::recover(parts.approximate[i],uint32_t(parts.residue[i]),&value);totals[i]+=accepted?uint64_t(value):0xdeadbeef395u;}
        }
    }
#pragma unroll
    for(unsigned i=0u;i<8u;++i)output[blockIdx.x*256u+(2u*i+lane/16u)*16u+lane%16u]=totals[i];
}
...
template<unsigned Headroom> void run() {
    ...
    if(recovery_bad || rejected || bound_bad)return;
    constexpr unsigned blocks=1024u;
    Device sequence_output((blocks*256u+2u*guard)*sizeof(uint64_t));
    for(unsigned groups:{16u,64u,256u}) {
        std::vector<uint64_t> expected(blocks*256u+2u*guard,UINT64_C(0xa5a5a5a5a5a5a5a5));
        for(unsigned block=0u;block<blocks;++block)for(unsigned cell=0u;cell<256u;++cell){
            uint64_t total=0u;for(unsigned group=0u;group<groups;++group)total+=uint64_t(actual[guard+((block*groups+group)%tiles)*256u+cell].integer);
            expected[guard+block*256u+cell]=total;
        }
        double samples[2][3]{};
        for(unsigned attempt=0u;attempt<4u;++attempt)for(unsigned position=0u;position<2u;++position){
            ...
            check(hipMemset(sequence_output.pointer,0xa5,expected.size()*sizeof(uint64_t)));finish();const auto start=std::chrono::steady_clock::now();
            if(!variant)hipLaunchKernelGGL((sequence<Headroom,0u>),dim3(blocks),dim3(32u),0u,nullptr,reinterpret_cast<core::Row*>(prepared.as<unsigned char>()+guard),controls.as<OriginalRow>()+guard,sequence_output.as<uint64_t>()+guard,groups);
            else hipLaunchKernelGGL((sequence<Headroom,1u>),dim3(blocks),dim3(32u),0u,nullptr,reinterpret_cast<core::Row*>(prepared.as<unsigned char>()+guard),controls.as<OriginalRow>()+guard,sequence_output.as<uint64_t>()+guard,groups);
            check(hipGetLastError());finish();if(attempt)samples[variant][attempt-1u]=elapsed(start);
            unchanged(sequence_output,expected);
        }
        unchanged(input,values);unchanged(prepared,encoded);unchanged(controls,control_bytes);
        for(unsigned variant=0u;variant<2u;++variant){
            double sorted[3]={samples[variant][0],samples[variant][1],samples[variant][2]};std::sort(sorted,sorted+3u);
            std::printf("{\"kind\":\"byte_residue32_core_sequence\",\"headroom\":%u,\"variant\":%u,\"groups\":%u,\"blocks\":1024,\"median_ms\":%.6f,\"samples_ms\":[%.6f,%.6f,%.6f],\"warmups\":1,\"measured_attempts\":3,\"cpu_integer_sequence_output_cells\":1048576,\"all_attempts_verified\":true,\"mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"canonical_carry_and_exceptions_included\":false,\"preparation_timed_separately\":true,\"performance_acceptance\":false,\"inference_acceptance\":false}\n",Headroom,variant,groups,sorted[1],samples[variant][0],samples[variant][1],samples[variant][2]);
        }
        std::fflush(stdout);
    }
}
