...
#include "../../native/providers/ck_fmha/compact_byte_qk.h"
#include <algorithm>
#include <array>
#include <chrono>
#include <cstdio>
...
constexpr unsigned variants[] = {0u, 4u, 8u};
...
using Row = qrt_compact_byte_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    unsigned tokens, unsigned start, unsigned count, uint16_t* transposed=nullptr) {
    const size_t rows=integer_row_count(Kind,tokens,count);
    hipLaunchKernelGGL((qrt_compact_byte_qk::prepare_rows<Kind>),
        dim3((rows+kThreads-1u)/kThreads),dim3(kThreads),0u,nullptr,input,output,tokens,start,count,transposed);
    check(hipGetLastError());
}
struct MatrixRows {
    unsigned tokens;
    ...
    std::vector<uint16_t> expected_raw;
    double key_ms=0.0;
    MatrixRows(unsigned n,unsigned count,const uint16_t* device_key,const uint16_t* host_key)
        : tokens(n),qbytes(integer_row_count(IntegerRowKind::Query,n,count)*sizeof(Row)),
          ...
          qp(qbytes+2u*guard),kp(kbytes+2u*guard),raw((size_t(n)*kKvHeads*kHeadDim+2u*guard)*2u),
          expected_raw(size_t(n)*kKvHeads*kHeadDim+2u*guard,0x5a5au) {
        check(hipMemset(kp.pointer,0xa5,kbytes+2u*guard));
        check(hipMemset(raw.pointer,0x5a,expected_raw.size()*2u));
        const auto begin=std::chrono::steady_clock::now();
        encode_rows<IntegerRowKind::Key>(device_key,key(),tokens,0u,0u,raw_key());finish();key_ms=elapsed(begin);
        ...
        for(unsigned token=0u;token<tokens;++token)
            for(unsigned feature=0u;feature<kKvHeads*kHeadDim;++feature)
                expected_raw[guard+size_t(feature)*tokens+token]=host_key[size_t(token)*kKvHeads*kHeadDim+feature];
        verify_keys();
    }
    ...
    uint16_t* raw_key(){return raw.as<uint16_t>()+guard;}
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            const Row value=qrt_sm121_compact_byte_core::prepare(original);
    ...
    void verify_keys(){unchanged(kp,expected_keys);unchanged(raw,expected_raw);}
};
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    unsigned start,unsigned count,unsigned stride,unsigned key_stride,Prepared& prepared,MatrixRows& matrix) {
    ...
    if(!variant) {
        ...
        if(variant==4u)hipLaunchKernelGGL((qrt_compact_byte_qk::scores<4u>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),q,matrix.raw_key(),out,start,count,stride,key_stride);
        else if(variant==8u)hipLaunchKernelGGL((qrt_compact_byte_qk::scores<8u>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),q,matrix.raw_key(),out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid compact byte QK variant");
    }
    check(hipGetLastError());
}
std::array<unsigned long long,4> audit_variant(unsigned variant,const uint16_t* q,float* out,unsigned start,unsigned count,
    unsigned stride,unsigned key_stride,MatrixRows& matrix) {
    constexpr unsigned long long marker=0xa5a5a5a5a5a5a5a5ull;
    std::vector<unsigned long long> expected(4u+2u*guard,marker);
    std::fill_n(expected.begin()+guard,4u,0ull);Device counters(expected.size()*8u);upload(counters,expected);
    const dim3 grid((stride+15u)/16u,kQueryHeads,(count+15u)/16u);
    if(variant==4u)hipLaunchKernelGGL((qrt_compact_byte_qk::scores<4u,true>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),q,matrix.raw_key(),out,start,count,stride,key_stride,counters.as<unsigned long long>()+guard);
    else if(variant==8u)hipLaunchKernelGGL((qrt_compact_byte_qk::scores<8u,true>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),q,matrix.raw_key(),out,start,count,stride,key_stride,counters.as<unsigned long long>()+guard);
    else throw std::runtime_error("invalid compact byte audit variant");
    check(hipGetLastError());finish();const auto actual=download<unsigned long long>(counters,expected.size());
    for(unsigned i=0u;i<guard;++i)if(actual[i]!=marker || actual[guard+4u+i]!=marker)throw std::runtime_error("compact byte audit counter guard");
    const unsigned long long groups=(static_cast<unsigned long long>(count)*(2u*start+count+1u)/2u)*kQueryHeads*16u;
    if(actual[guard]+actual[guard+1u]!=groups || actual[guard+2u]>16u*actual[guard] || actual[guard+3u]>actual[guard+1u])throw std::runtime_error("compact byte audit coverage");
    return {actual[guard],actual[guard+1u],actual[guard+2u],actual[guard+3u]};
}
...
void run(Case c) {
    ...
    MatrixRows matrix(c.tokens,c.count,dk.as<uint16_t>()+guard,k.data()+guard);
    ...
    std::array<unsigned long long,4> control_stats{};
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("compact byte QK differs from original scores");
            }
        }
        ...
        std::array<unsigned long long,4> stats{};
        if(variant) {
            Device audit((cells+2u*guard)*4u);check(hipMemset(audit.pointer,0xa5,(cells+2u*guard)*4u));
            stats=audit_variant(variant,dq.as<uint16_t>()+guard,audit.as<float>()+guard,c.start,c.count,stride,c.tokens,matrix);
            if(download<uint32_t>(audit,cells+2u*guard)!=b)throw std::runtime_error("generated production/audit parity");
            matrix.verify_query(q.data()+guard,c.start,c.count);
            if(variant==4u)control_stats=stats;else if(stats!=control_stats)throw std::runtime_error("generated matrix windows changed arithmetic counts");
        }
        matrix.verify_keys();
        std::printf("{\"kind\":\"compact_byte_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"audit_invoked\":%u,\"audit_fast_groups\":%llu,\"audit_original_groups\":%llu,\"audit_compensated_pairs\":%llu,\"audit_recovery_rejections\":%llu,\"production_diagnostic_parity\":true,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells,unsigned(variant!=0u),stats[0],stats[1],stats[2],stats[3]);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    Device reference(total_words*4u),candidate(total_words*4u),audit(total_words*4u),bad(4u);
    ...
    MatrixRows matrix(tokens,batch,dk.as<uint16_t>()+guard,k.data()+guard);
    std::vector<uint16_t> transposed(k.size(),0x5a5au);
    for(unsigned token=0u;token<tokens;++token)
        for(unsigned feature=0u;feature<kKvHeads*kHeadDim;++feature)
    ...
    unsigned long long totals[variant_count][4]{};
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned position=0u;position<variant_count;++position) {
            ...
            if(variant) {
                check(hipMemset(audit.pointer,0xa5,total_words*4u));
                const auto stats=audit_variant(variant,dq.as<uint16_t>()+guard,audit.as<float>()+guard,start,count,stride,tokens,matrix);
                hipLaunchKernelGGL(compare_scores,dim3((total_words+255u)/256u),dim3(256u),0u,nullptr,candidate.as<uint32_t>(),audit.as<uint32_t>(),total_words,bad.as<unsigned>());
                check(hipGetLastError());finish();
                if(download<unsigned>(bad,1u)[0])throw std::runtime_error("captured production/audit parity or tail");
                for(unsigned i=0u;i<4u;++i)totals[mode][i]+=stats[i];
                matrix.verify_query(q.data()+guard,start,count);
            }
    ...
    for(unsigned i=0u;i<4u;++i)if(totals[1][i]!=totals[2][i])throw std::runtime_error("matrix windows changed arithmetic path counts");
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        std::printf("{\"kind\":\"compact_byte_qk_capture\",\"variant\":%u,\"row_bytes\":76,\"conditional_native_error_bound_proven\":false,\"query_rows\":16,\"key_columns\":16,\"matrix_window_groups\":%u,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_slab_encoding_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"audit_invoked\":%u,\"audit_fast_groups\":%llu,\"audit_original_groups\":%llu,\"audit_compensated_pairs\":%llu,\"audit_recovery_rejections\":%llu,\"production_diagnostic_parity\":true,\"window_arithmetic_counts_equal\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,variant,tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
            preparation_ms,sorted[1],sorted[1]+preparation_ms,samples[mode][0],samples[mode][1],samples[mode][2],maximum_stage_ms[mode],unsigned(variant!=0u),totals[mode][0],totals[mode][1],totals[mode][2],totals[mode][3]);
        std::fflush(stdout);
    }
}
