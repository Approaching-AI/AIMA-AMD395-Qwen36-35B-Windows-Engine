...
#include "../../native/providers/gdn/absolute_bound_matrices.h"
...
void paired_launch(unsigned variant,unsigned count,Device& q,Device& k,Device& v,Device& beta,
 ...
 require(variant<=2u,"invalid absolute bound GDN variant");
...
#define QRT_BOUND_SCORE_ARGS q.data<uint16_t>()+size_t(offset)*2048u,k.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,n,exp
  hipLaunchKernelGGL(qrt_fla_paired_score::kernel,dim3(2u,16u,chunks*8u),dim3(256u),0u,nullptr,QRT_BOUND_SCORE_ARGS);
  check(hipGetLastError());
#undef QRT_BOUND_SCORE_ARGS
#define QRT_BOUND_WU_ARGS k.data<uint16_t>()+size_t(offset)*2048u,v.data<uint16_t>()+size_t(offset)*4096u,beta.data<uint16_t>()+size_t(offset)*32u,inverse.data<uint16_t>()+size_t(offset)*2048u,g.data<float>()+size_t(offset)*32u,w.data<uint16_t>()+size_t(offset)*4096u,actual_u+size_t(offset)*4096u,n,exp
  hipLaunchKernelGGL(scalar::wu_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_BOUND_WU_ARGS);
  check(hipGetLastError());
#undef QRT_BOUND_WU_ARGS
#define QRT_BOUND_STATE_ARGS k.data<uint16_t>()+size_t(offset)*2048u,actual_u+size_t(offset)*4096u,w.data<uint16_t>()+size_t(offset)*4096u,g.data<float>()+size_t(offset)*32u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,vn.data<uint16_t>()+size_t(offset)*4096u,state.data<float>(),n,exp
  if(variant){hipLaunchKernelGGL((qrt_fla_absolute_bound::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,QRT_BOUND_STATE_ARGS);}
  else{hipLaunchKernelGGL((lifetime::state_kernel<8u>),dim3(16u,32u),dim3(256u),0u,nullptr,QRT_BOUND_STATE_ARGS);}
  check(hipGetLastError());
#undef QRT_BOUND_STATE_ARGS
#define QRT_BOUND_OUTPUT_ARGS q.data<uint16_t>()+size_t(offset)*2048u,vn.data<uint16_t>()+size_t(offset)*4096u,h.data<uint16_t>()+size_t(offset/64u)*state_cells,g.data<float>()+size_t(offset)*32u,scores.data<uint16_t>()+size_t(offset)*2048u,output.data<float>()+size_t(offset)*4096u,n,exp
  if(variant==2u){hipLaunchKernelGGL(qrt_fla_absolute_bound::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_BOUND_OUTPUT_ARGS);}
  else{hipLaunchKernelGGL(lifetime::output_kernel,dim3(16u,32u,chunks),dim3(256u),0u,nullptr,QRT_BOUND_OUTPUT_ARGS);}
  check(hipGetLastError());
#undef QRT_BOUND_OUTPUT_ARGS
 }
}
...
void paired_run(unsigned count,unsigned mode,unsigned measured,Device& table,const std::vector<unsigned char>& host_table,const char* capture_root=nullptr){
 ...
 for(unsigned attempt=0u;attempt<attempts;++attempt)for(unsigned position=0u;position<6u;++position){
  ...
  require(!expected.empty() && !std::memcmp(actual.data(),expected.data(),actual.size()*4u),"absolute bound GDN output differs");require(!std::memcmp(next.data(),final.data(),next.size()*4u),"absolute bound GDN state differs");
  require(actual_w==old_w && actual_u==old_u && actual_h==old_h && actual_vn==old_vn,"absolute bound GDN intermediate or alias differs");
