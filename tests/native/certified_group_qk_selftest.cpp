...
#include "../../native/providers/ck_fmha/certified_group_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
constexpr unsigned variants[] = {4u, 5u};
...
struct Prepared {
    unsigned tokens;
    std::vector<uint32_t> qpacked, kpacked, qflags, kflags, qbounds, kbounds;
    Device qp, kp, qf, kf, qb, kb;
    double ms = 0.0, bounds_ms = 0.0;
    Prepared(const uint16_t* q, const uint16_t* k, uint16_t* transposed,
        ...
        : tokens(n), qpacked(size_t(n)*kQueryHeads*kHeadDim+2u*guard,0xa5a5a5a5u),
          ...
          qbounds(size_t(n)*kQueryHeads*16u+2u*guard,0xa5a5a5a5u), kbounds(size_t(n)*kKvHeads*16u+2u*guard,0xa5a5a5a5u),
          qp(qpacked.size()*4u), kp(kpacked.size()*4u), qf(qflags.size()*4u), kf(kflags.size()*4u), qb(qbounds.size()*4u), kb(kbounds.size()*4u) {
        ...
        check(hipMemset(qb.pointer,0xa5,qbounds.size()*4u));
        check(hipMemset(kb.pointer,0xa5,kbounds.size()*4u));finish();
        const auto bounds_begin=std::chrono::steady_clock::now();
        hipLaunchKernelGGL((qrt_certified_group_qk::prepare_bounds<false>),dim3((size_t(n)*kQueryHeads*16u+255u)/256u),dim3(256u),0u,nullptr,
            q,qb.as<uint32_t>()+guard,n);check(hipGetLastError());
        hipLaunchKernelGGL((qrt_certified_group_qk::prepare_bounds<true>),dim3((size_t(n)*kKvHeads*16u+255u)/256u),dim3(256u),0u,nullptr,
            k,kb.as<uint32_t>()+guard,n);check(hipGetLastError());finish();bounds_ms=elapsed(bounds_begin);
        for(unsigned key=0u;key<2u;++key) {
            ...
            for(unsigned row=0u;row<n*heads;++row) {
                ...
                auto& bounds=key?kbounds:qbounds;
                for(unsigned g=0u;g<16u;++g) {
                    unsigned maximum=0u,minimum=255u;bool valid=true;
                    for(unsigned i=0u;i<16u;++i) {
                        const uint16_t x=source[size_t(row)*kHeadDim+g*16u+i];
                        if(!(x&0x7fffu))continue;
                        const unsigned e=(x>>7u)&255u;valid &= e>=64u && e<=190u;
                        unsigned fraction=128u|(x&127u),trailing=0u;
                        while(!(fraction&1u)){++trailing;fraction>>=1u;}
                        maximum=std::max(maximum,e);minimum=std::min(minimum,e+trailing);
                    }
                    const size_t index=key?(size_t(row%heads)*16u+g)*n+row/heads:size_t(row)*16u+g;
                    bounds[guard+index]=maximum|(minimum<<8u)|(valid?1u<<16u:0u);
                }
            }
        }
        verify();
    }
    void verify() { unchanged(qp,qpacked);unchanged(kp,kpacked);unchanged(qf,qflags);unchanged(kf,kflags);unchanged(qb,qbounds);unchanged(kb,kbounds); }
};
struct Scratch {
    ...
    size_t verify(unsigned variant,unsigned stride,unsigned queries) {
        ...
        const size_t used=0u;size_t selected=0u;
        (void)variant;(void)stride;(void)queries;
...
#define QRT_LAYOUT_ARGS qp,kp,qf,kf,out,start,count,stride,key_stride
        if(variant==1u)hipLaunchKernelGGL((qrt_deferred_qk_fallback::scores<false>),grid,dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS,nullptr);
        else if(variant==2u)hipLaunchKernelGGL((qrt_microtile_exact_qk::scores<1u,2u>),
            dim3((stride+31u)/32u,kQueryHeads,(count+15u)/16u),dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS);
        else if(variant==3u)hipLaunchKernelGGL((qrt_microtile_exact_qk::scores<2u,1u>),
            dim3((stride+15u)/16u,kQueryHeads,(count+31u)/32u),dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS);
        else if(variant==4u)hipLaunchKernelGGL((qrt_microtile_exact_qk::scores<2u,2u>),
            dim3((stride+31u)/32u,kQueryHeads,(count+31u)/32u),dim3(kThreads),0u,nullptr,QRT_LAYOUT_ARGS);
        else if(variant==5u)hipLaunchKernelGGL((qrt_certified_group_qk::scores<2u,2u>),
            dim3((stride+31u)/32u,kQueryHeads,(count+31u)/32u),dim3(kThreads),0u,nullptr,
            qp,kp,qf,kf,prepared.qb.as<uint32_t>()+guard,prepared.kb.as<uint32_t>()+guard,out,start,count,stride,key_stride);
        else throw std::runtime_error("invalid certified-group exact QK variant");
#undef QRT_LAYOUT_ARGS
        check(hipGetLastError());
...
void run(Case c) {
    ...
    if(c.mode==8u) {
        // Reject only the second query/key cell owned by a thread; its neighbor
        // must retain its independent fast carry and complete causal mask.
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("microtile exact QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"certified_group_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile,unsigned tokens) {
    ...
    for(unsigned mode=0u;mode<variant_count;++mode) {
        ...
        const double preparation_ms=prepared.ms+(variant==5u?prepared.bounds_ms:0.0);
        std::printf("{\"kind\":\"certified_group_qk_capture\",\"variant\":%u,\"query_rows\":%u,\"key_columns\":%u,\"score_cells_per_thread\":%u,\"verified_flagged_tiles_after_last_attempt\":%zu,\"tokens\":%u,\"original_capture_tokens\":7169,\"real_model_prompt\":false,\"query_batch\":128,\"unique_score_cells\":%zu,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"one_time_preparation_ms\":%.6f,\"completed_query_with_replay_ms\":%.6f,\"completed_total_ms\":%.6f,\"completed_query_samples_ms\":[%.6f,%.6f,%.6f],\"warmup_per_slab\":1,\"samples_per_slab\":3,\"maximum_completed_slab_ms\":%.6f,\"all_attempts_verified\":true,\"redzones_pass\":true,\"unused_score_tail_pass\":true,\"immutable_inputs\":true,\"complete_cpu_encoding_check\":true,\"tile_flag_guards_and_tail_pass\":true,\"complete_replay_timing_included\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",
            variant,32u,32u,4u,flagged_tiles[mode],tokens,compared/variant_count/attempts,compared/variant_count,cpu_dots/variant_count,
