...
__global__ void range_projection_replay_kernel(const uint16_t* weights, const uint16_t* inputs,
...
void complete_range_projection() {
...
void run_range_projection_replays(DeviceBuffer<uint16_t>& dw, DeviceBuffer<uint16_t>& di,
    ...
    for (unsigned variant = 0u; variant < 4u; ++variant) {
        ...
        for (unsigned attempt = 0u; attempt < 2u; ++attempt) {
            ...
            const unsigned lanes = variant < 2u ? 4u : 16u;
            const dim3 grid((unsigned(selected.size()) * lanes + 255u) / 256u);
#define QRT_SCALAR_REPLAY_CASE(v) if (variant == v) hipLaunchKernelGGL(range_projection_replay_kernel<v>, grid, dim3(256u), 0u, nullptr, dw.data(), di.data(), dfw.data(), dfi.data(), indices.data(), dout.data(), raw.data(), rows, width, unsigned(selected.size()))
...
#undef QRT_SCALAR_REPLAY_CASE
            hip_ok(hipGetLastError(), "scalar_replay_launch"); complete_range_projection();
        ...
        std::cout << "{\"type\":\"range_projection_real_replay\",\"variant\":" << variant
