...
__global__ void compare_domain_bound(unsigned* failures,unsigned count){
    const unsigned index=blockIdx.x*blockDim.x+threadIdx.x;if(index>=count)return;
    uint32_t state=index+1u;
    auto next=[&](){state^=state<<13u;state^=state>>17u;state^=state<<5u;return state;};
    auto value=[&](unsigned maximum,bool signed_value){const unsigned exponent=next()%(maximum+1u),fraction=next()&0x7fffffu;
        return bound::scalar::value((exponent<<23u)|fraction|(signed_value?(next()&0x80000000u):0u));};
    const bound::State before{value(237u,true),value(230u,false)};
    const float partial=value(229u,true),positive=value(229u,false);
    const auto expected=bound::advance<4u,19u>(before,partial,positive);
    const auto actual=qrt_sm121_domain_coarse_bound::advance(before,partial,positive);
    if(bound::scalar::bits(expected.center)!=bound::scalar::bits(actual.center)||bound::scalar::bits(expected.error)!=bound::scalar::bits(actual.error))atomicAdd(failures,1u);
}
void native_bound_check(){
    Buffer<unsigned> failures(1u);check(hipMemset(failures.data(),0,4u));
    hipLaunchKernelGGL(compare_domain_bound,dim3(8192u),dim3(256u),0u,nullptr,failures.data(),2097152u);check(hipGetLastError());finish();
    require(!failures.read()[0],"native domain recurrence raw mismatch");
}
struct Measurement {double ms=0.0,prefix_ms=0.0;unsigned selected=0u;};
constexpr unsigned variants=5u;
const char* names[variants]={"c64_scalar","c64_vector","domain_vector","domain_shared64x64_prefetch","domain_shared128x32"};
struct Test {
    ...
    void producer(unsigned variant){
        ...
        else if(variant==2u)hipLaunchKernelGGL((matrix::produce<64u,1u,true,19u,true>),dim3((rows+127u)/128u,(tokens+15u)/16u),dim3(256u),0u,nullptr,w.data(),x.data(),wf.data(),xf.data(),centers.data(),errors.data(),rows,tokens,width);
        else if(variant==3u)shared_producer<64u,true>();
        else shared_producer<128u,false>();
        check(hipGetLastError());
    }
    ...
    void execute(unsigned mode){
        ...
        for(unsigned variant=0u;variant<variants;++variant){
            ...
            std::printf("{\"kernel\":\"domain_c64_projection\",\"variant\":\"%s\",\"variant_id\":%u,\"rows\":%u,\"tokens\":%u,\"width\":%u,\"mode\":%u,\"weight_skew_words\":%u,\"input_skew_words\":%u,\"captured\":%s,\"cells\":%u,\"candidates\":%u,\"replay_groups\":%llu,",
...
int main(int argc,char** argv)try{
    ...
    return 0;
}catch(const std::exception& e){std::fprintf(stderr,"domain_c64_error=%s\n",e.what());return 1;}
