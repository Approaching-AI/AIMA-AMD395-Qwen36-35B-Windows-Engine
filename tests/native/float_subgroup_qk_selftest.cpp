#include "../../native/providers/ck_fmha/float_subgroup_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstring>
...
struct Shape { unsigned lanes, staging, query_rows, key_columns; };
constexpr Shape shapes[] = {{1,1,8,32},{4,1,8,8},{4,4,8,8},{4,1,4,16},{4,4,4,16},
    {8,1,4,8},{8,4,4,8},{16,1,2,8},{16,4,2,8}};
...
template<unsigned Lanes, unsigned Staging, unsigned Rows>
void cooperative(const uint16_t* q, const uint16_t* k, float* out,
    unsigned start, unsigned count, unsigned stride, unsigned key_stride) {
    constexpr unsigned columns = kThreads / (Lanes * Rows);
    hipLaunchKernelGGL(HIP_KERNEL_NAME(qrt_float_subgroup_qk::scores<Lanes, Staging, Rows>),
        dim3((stride+columns-1u)/columns,kQueryHeads,(count+Rows-1u)/Rows),
...
void staged_variant(unsigned variant, const uint16_t* q, const uint16_t* k, float* out,
    ...
    case 0u:
        hipLaunchKernelGGL(blackwell_float_alignment_scores_kernel,
            ...
            dim3(kThreads),0u,nullptr,q,k,out,start,count,stride,key_stride);
        break;
    case 1u: cooperative<4,1,8>(q,k,out,start,count,stride,key_stride); break;
    case 2u: cooperative<4,4,8>(q,k,out,start,count,stride,key_stride); break;
    case 3u: cooperative<4,1,4>(q,k,out,start,count,stride,key_stride); break;
    case 4u: cooperative<4,4,4>(q,k,out,start,count,stride,key_stride); break;
    case 5u: cooperative<8,1,4>(q,k,out,start,count,stride,key_stride); break;
    case 6u: cooperative<8,4,4>(q,k,out,start,count,stride,key_stride); break;
    case 7u: cooperative<16,1,2>(q,k,out,start,count,stride,key_stride); break;
    case 8u: cooperative<16,4,2>(q,k,out,start,count,stride,key_stride); break;
    default: throw std::runtime_error("invalid float subgroup variant");
    }
...
void run(Case c) {
    ...
    for(unsigned variant:variants) {
        ...
        staged_variant(variant,dq.as<uint16_t>()+guard,dt.as<uint16_t>()+guard,candidate.as<float>()+guard,c.start,c.count,stride,c.tokens);
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("float subgroup QK differs from original scores");
            }
        }
        ...
        std::printf("{\"kind\":\"float_subgroup_qk_safety\",\"variant\":%u,\"lanes\":%u,\"staging_groups\":%u,\"query_rows\":%u,\"key_columns\":%u,\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",variant,shape.lanes,shape.staging,shape.query_rows,shape.key_columns,c.tokens,c.start,c.count,c.mode,cells);
        std::fflush(stdout);
    }
...
void captured(const char* qfile,const char* kfile) {
    ...
    for(unsigned start=0u;start<tokens;start+=batch) {
        ...
        for(unsigned mode=0u;mode<variant_count;++mode) {
            begin=std::chrono::steady_clock::now();
            staged_variant(variants[mode],dq.as<uint16_t>(),dt.as<uint16_t>(),candidate.as<float>()+guard,start,count,stride,tokens);
    ...
    const auto ineligible=[](uint16_t x){return !qrt_sm121_float_alignment::eligible(x);};
    const auto ineligible_q=std::count_if(q.begin(),q.end(),ineligible),ineligible_k=std::count_if(k.begin(),k.end(),ineligible);
    std::vector<uint16_t> expected_transposed(k.size());
    for (unsigned token=0; token<tokens; ++token)
        for (unsigned feature=0; feature<kKvHeads*kHeadDim; ++feature)
            expected_transposed[size_t(feature)*tokens+token] = k[size_t(token)*kKvHeads*kHeadDim+feature];
    unchanged(dt,expected_transposed);
    for (unsigned mode=0u; mode<variant_count; ++mode) {
        const auto shape=shapes[variants[mode]];
        std::printf("{\"kind\":\"float_subgroup_original_q7169\",\"variant\":%u,\"lanes\":%u,\"staging_groups\":%u,\"query_rows\":%u,\"key_columns\":%u,\"ineligible_q_values\":%zu,\"ineligible_k_values\":%zu,\"tokens\":7169,\"query_batch\":128,\"compared_score_cells\":%zu,\"cpu_dots\":%u,\"raw_bit_mismatches\":0,\"original_query_ms\":%.6f,\"key_transpose_ms\":%.6f,\"query_ms\":%.6f,\"maximum_completed_slab_ms\":%.6f,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false,\"performance_acceptance\":false}\n",variants[mode],shape.lanes,shape.staging,shape.query_rows,shape.key_columns,size_t(ineligible_q),size_t(ineligible_k),compared/variant_count,cpu_dots/variant_count,original_ms,transpose_ms,staged_ms[mode],maximum_stage_ms[mode]);
    }
...
int main(int argc,char** argv) {
    try {
        ...
        const Case cases[]={{17,0,8,0},{17,0,9,2},{33,0,16,0},{33,1,31,1},{1,0,1,0},{9,0,9,1},{35,3,17,2},{67,33,32,3},{67,64,3,4},
            {19,2,3,1},{31,3,4,2},{35,7,9,0},{71,31,33,3},{137,7,129,1},{129,1,128,0},{7169,7041,128,1},{8192,8064,128,0},{8192,8191,1,2},{8193,8191,2,3}};
