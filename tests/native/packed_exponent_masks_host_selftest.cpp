#include "../../native/providers/moe_accumulator/sm121_packed_exponent_masks.h"
#include <algorithm>
#include <cstdio>
#include <cstring>
#include <stdexcept>
...
void check_pack(const uint16_t* words, const uint32_t* packed) {
    ...
    uint32_t raw[8]{};
    for(unsigned i=0u;i<16u;++i)raw[i/2u]|=uint32_t(words[i])<<((i&1u)*16u);
    const auto metadata=qrt_sm121_packed_exponent_masks::prepare<1u>(raw);
    require(metadata.maximum==maximum && !std::memcmp(metadata.mask,positions,sizeof(positions)));
    for (unsigned i = 0u; i < 16u; ++i) {
...
void check_group(const uint16_t* left, const uint16_t* right, const uint32_t* a,
    ...
    uint32_t raw_left[8]{},raw_right[128]{};
    for(unsigned i=0u;i<16u;++i) {
        raw_left[i/2u]|=uint32_t(left[i])<<((i&1u)*16u);
        raw_right[(i/2u)*16u]|=uint32_t(right[i])<<((i&1u)*16u);
    }
    qrt_sm121_packed_exponent_masks::group<16u>(candidate,raw_left,raw_right,
        qrt_sm121_packed_exponent_masks::prepare<1u>(raw_left),
        qrt_sm121_packed_exponent_masks::prepare<16u>(raw_right),CarryAware?ce:-133);
    (void)a;(void)b;
...
int main() try {
    ...
    for (unsigned sample = 0u; sample < 65536u; ++sample) {
        ...
        if(sample%128u==0u) {
            uint32_t raw_a[8]{},raw_b[128]{};
            for(unsigned i=0u;i<16u;++i) {
                raw_a[i/2u]|=uint32_t(a[i])<<((i&1u)*16u);
                raw_b[(i/2u)*16u]|=uint32_t(b[i])<<((i&1u)*16u);
            }
            const auto ma=qrt_sm121_packed_exponent_masks::prepare<1u>(raw_a);
            const auto mb=qrt_sm121_packed_exponent_masks::prepare<16u>(raw_b);
            for(int carried : {-160,-134,128,160}) {
                alignment::Group result;
                qrt_sm121_packed_exponent_masks::group<16u>(result,raw_a,raw_b,ma,mb,carried);
                require(result.maximum==std::max(-133,std::max(product_maximum,carried)));
            }
        }
    ...
    std::printf("{\"kind\":\"packed_exponent_masks_host\",\"bf16_encodings\":65536,\"certificate_checks\":%u,\"certificates\":%u,\"misses\":%u,\"canonical_groups\":%u,\"declined_accumulations\":%u,\"raw_mismatches\":0,\"immutable_inputs\":true,\"declined_output_unchanged\":true,\"inference_acceptance\":false}\n", checks, certificates, misses, groups, declines);
    return 0;
