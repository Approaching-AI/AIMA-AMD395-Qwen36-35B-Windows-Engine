// Independent K16 matrix windows with original canonical carries.
#include "../../native/providers/ck_fmha/prepared_decoded_qk.h"
#include "../../native/providers/ck_fmha/windowed_residue_qk.h"
#include <algorithm>
#include <chrono>
#include <cstdio>
#include <cstdlib>
...
constexpr unsigned variants[] = {0u, 116u, 216u, 416u, 432u};
...
using Row = qrt_windowed_residue_qk::Row;
template<IntegerRowKind Kind> void encode_rows(const uint16_t* input, Row* output,
    ...
    hipLaunchKernelGGL((qrt_windowed_residue_qk::prepare_rows<Kind>),
...
struct MatrixRows {
    ...
    std::vector<unsigned char> cpu_encoding(IntegerRowKind kind,const uint16_t* input,
        ...
        for(size_t row=0u;row<rows;++row) {
            ...
            qrt_windowed_residue_qk::prepare(value);
...
void staged_variant(unsigned variant,const uint16_t* q,const uint16_t* k,float* out,
    ...
    if(!variant) {
        ...
        const unsigned columns=variant%100u;
        const dim3 grid((stride+columns-1u)/columns,kQueryHeads,(count+15u)/16u);
#define WINDOWED_RESIDUE_CASE(V,W,C) if(variant==V)hipLaunchKernelGGL((qrt_windowed_residue_qk::scores<W,C>),grid,dim3(kThreads),0u,nullptr,matrix.query(),matrix.key(),out,start,count,stride,key_stride)
        WINDOWED_RESIDUE_CASE(116u,1u,16u);
        else WINDOWED_RESIDUE_CASE(216u,2u,16u);
        else WINDOWED_RESIDUE_CASE(416u,4u,16u);
        else WINDOWED_RESIDUE_CASE(432u,4u,32u);
        else throw std::runtime_error("invalid windowed residue QK variant");
#undef WINDOWED_RESIDUE_CASE
    }
    check(hipGetLastError());
}
    ...
    for(unsigned variant:variants) {
        ...
        for(size_t i=0u;i<a.size();++i) {
            if(i<guard || i>=cells+guard) {
                ...
                throw std::runtime_error("windowed residue QK differs from original scores");
            }
        }
        ...
        matrix.verify_keys();
        std::printf("{\"kind\":\"windowed_residue_qk_safety\",\"tokens\":%u,\"query_start\":%u,\"query_count\":%u,\"mode\":%u,\"variant\":%u,\"cells\":%zu,\"cpu_dots\":64,\"raw_bit_mismatches\":0,\"redzones_pass\":true,\"immutable_inputs\":true,\"inference_acceptance\":false}\n",c.tokens,c.start,c.count,c.mode,variant,cells);
        std::fflush(stdout);
    }
