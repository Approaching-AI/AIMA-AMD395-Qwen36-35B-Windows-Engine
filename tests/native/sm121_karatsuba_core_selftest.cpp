...
struct CoreRow { int high[4], low[4]; };
...
int main() try {
    ...
    for (unsigned index = 0u; index < cells.size(); ++index) {
        ...
        for (unsigned i = 0u; i < 16u; ++i) {
            ...
            balanced[2] += (x.high + x.low) * (y.high + y.low);
        ...
        for (float part : cells[index].candidate)
            convertible = convertible && std::isfinite(part) && part >= -1048576.0f && part <= 1048576.0f;
        reconstruction_bad += unsigned(!convertible || mathematical != k3::reconstruct(
            int(cells[index].candidate[0]), int(cells[index].candidate[1]), int(cells[index].candidate[2])));
    }
    ...
    std::printf("{\"kind\":\"karatsuba_integer_core\",\"tiles\":%u,\"cells\":%u,\"integer_partial_mismatches\":%u,\"candidate_raw_partial_mismatches\":%u,\"reconstruction_mismatches\":%u,\"redzones_pass\":%s,\"immutable_inputs\":%s,\"inference_acceptance\":false}\n", tiles, unsigned(cells.size()), integer_bad, candidate_bad, reconstruction_bad, guards ? "true" : "false", immutable ? "true" : "false");
    ...
    for (unsigned groups : {16u, 64u, 256u}) {
        ...
        std::printf("{\"kind\":\"karatsuba_core_sequence\",\"groups\":%u,\"blocks\":%u,\"original_ms\":%.6f,\"candidate_ms\":%.6f,\"checksum_mismatches\":%u,\"redzones_pass\":%s,\"immutable_inputs\":%s,\"diagnostic_only\":true,\"inference_acceptance\":false}\n", groups, blocks, original_ms, candidate_ms, bad, ok ? "true" : "false", stable ? "true" : "false");
        if (bad || !ok || !stable) return 2;
    }
    return 0;
} catch (const std::exception& error) { std::fprintf(stderr, "karatsuba_core_error=%s\n", error.what()); return 1; }
