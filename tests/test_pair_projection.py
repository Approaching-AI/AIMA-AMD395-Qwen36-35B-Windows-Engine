"""Check lossless pair metadata and ordered carries against original integers."""
...
class PairProjectionTests(unittest.TestCase):
    def test_original_representation_products_and_carries(self):
        with tempfile.TemporaryDirectory(prefix="qrt-pair-projection-") as directory:
...
''')
            header = ROOT / "native/providers/moe_accumulator/sm121_pair_projection.h"
            source = '#include "' + str(header) + '"\n#include <chrono>\n'
            source += 'void instantiate(const qrt_sm121_pair_projection::Row* p) {\n'
            for staging in (1, 2, 4, 8):
                for audit in ("false", "true"):
                    source += f'qrt_sm121_pair_projection::dot<{staging},{audit}>(p,p,16);\n'
            source += '}\nint main() { return 0; }\n'
            subprocess.run(
            ...
            subprocess.run(
                [os.environ.get("CXX", "c++"), "-std=c++17", "-O2", "-Wall", "-Wextra", "-Werror",
                 ...
                 str(ROOT / "tests/native/pair_projection_host.cpp"), "-o", executable],
