...
hipError_t launch_attention_block(const AttentionBlockViews& v,const AttentionBlockTables& t,hipStream_t stream){
 ...
 events.push_back({1,v.normalized_input,v.cache.prefix.keys,v.q_weights,t.rsqrt,v.output,v.staged_kv});return finish();
}
}
'''.replace('TYPES', types)
        main = r'''
...
int main(){
 ...
 const uint16_t* AttentionBlockViews::* block_reads[]={&AttentionBlockViews::q_weights,&AttentionBlockViews::k_weights,
  ...
  &AttentionBlockViews::k_norm_weights};
 ...
 l.scores=fake<float>(slot++);l.float_context=fake<float>(slot++);l.cache.prefix={fake<uint16_t>(slot++),fake<uint16_t>(slot++),0u,263680u,512u,2u};l.cache.staged=l.staged_kv;
 ...
 for(unsigned position:{0u,7169u,262143u,263678u})for(unsigned blocks:{1u,7u,1024u}){
  l.first_position=position;l.cache.prefix.tokens=position;l.score_stride=(position+33u)&~31u;expected_blocks=blocks;
  for(unsigned fault=0;fault<=4u;++fault){events.clear();fail_phase=fault;
   ...
   if(events.size()>1){const auto& b=events[1];assert(b.kind==1 && b.input==v.normalized_input && b.output==l.output && b.residual==l.cache.prefix.keys && b.next_residual==l.staged_kv);}
 ...
 bad=v;bad.moe_workspace=const_cast<void*>(l.cache.prefix.keys);reject(bad,t);
 ...
 bad=v;bad.moe_input=static_cast<uint16_t*>(const_cast<void*>(l.cache.prefix.values))+1u;reject(bad,t);
 bad=v;bad.attention.cache.decoded={v.output_residual,v.output_residual+512u,0u,2u,1024u,2u};reject(bad,t);
