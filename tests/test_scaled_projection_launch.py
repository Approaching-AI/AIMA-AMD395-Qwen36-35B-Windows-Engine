"""Execute scaled projection metadata and launch guards."""
...
class ScaledProjectionLaunchTests(unittest.TestCase):
    def test_metadata_capacity_and_launch_errors(self):
        header = (ROOT / "native/providers/moe_accumulator/sm121_scaled_projection.h").read_text()
        functions = "inline hipError_t prepare(" + header.split("inline hipError_t prepare(", 1)[1].split(
            "} // namespace qrt_sm121_scaled_projection", 1)[0]
        code = r'''
#include <cassert>
#include <cstddef>
#include <cstdint>
...
int main() {
    ...
    for(bool error:{false,true}) {
        ...
        assert(calls==1u&&queries==1u&&grid.x==65u&&block.x==256u&&seen_stream==stream&&seen_kernel==normal_rows_kernel);
    }
