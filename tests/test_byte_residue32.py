...
class ByteResidue32Tests(unittest.TestCase):
    def test_encodings_recovery_and_canonical_groups(self):
        with tempfile.TemporaryDirectory(prefix="qrt-byte-residue32-") as tmp:
            exe = str(Path(tmp) / "byte-residue")
            subprocess.run([os.environ.get("CXX", "c++"), "-std=c++17", "-O2",
                            "-Wall", "-Wextra", "-Werror", "-ffp-contract=off",
                            "-fsanitize=undefined", "-fno-sanitize-recover=all",
                            str(ROOT / "tests/native/byte_residue32_host_selftest.cpp"),
