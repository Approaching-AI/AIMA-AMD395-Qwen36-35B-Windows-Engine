"""Run the prepared exact-image replay after the current AMD owner is clean."""
...
D = B / 'runtime-attention-images-windows-r1'
assert socket.gethostname().split('.')[0].lower() != 'baiying'
read = lambda p: json.loads(p.read_text(encoding='utf-8-sig'))
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
assert sha(D / 'manifest.json') == '8c0376259fb928822f0bd39a5ff74fc0186441bbac70c074f21e76145007ddbc'
m = read(D / 'manifest.json')
assert sha(D / 'worker.py') == m['worker_sha256']
local_entries = [*m['images'].values(), *[e for e in m['inputs'].values() if 'file' in e]]
...
spec = dict(executable=m['python_executable'], arguments=[remote + '/worker.py', remote],
    ...
    model=m['model_reference'], model_loaded=False, actual_original_model_operands_loaded=True,
    purpose='Layer19 263292-token original operands: exact observed-DLL and qualified-probe HIP images; separate QK, PV, padding, prefix split and repeat controls',
...
assert report['manifest_sha256'] == sha(D / 'manifest.json') and report['cleanup_pass'] and report['sources_unchanged']
assert len(report['qk_records']) == 16 and len(report['pv_records']) == 48
