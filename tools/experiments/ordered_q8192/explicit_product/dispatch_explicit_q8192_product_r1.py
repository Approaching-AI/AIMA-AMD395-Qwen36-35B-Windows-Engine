...
import sys
sys.path.insert(0,'/Users/jiawei-macmini/projects/AIMA-explicit-q8192-candidate/tools')
...
stage=read(B/'linux-core-windows-explicit-q8192-stage-r1.json')
...
assert sha(B/'qualify_explicit_q8192_components_r1.py')==stage['qualifier_sha256']
assert sha(B/'qualify_native_gdn_explicit_r1.py')==stage['gdn_qualifier_sha256']
...
D=B/'linux-core-windows-explicit-q8192-r1';D.mkdir()
...
def ps(script,timeout=30):
 ...
 remote='P:/projects/explicit-q8192-dispatch-r1-'+str(ps_calls)+'.ps1'
...
try:
 from qualify_explicit_q8192_components_r1 import qualify
 selection=qualify(B,Path('/Users/jiawei-macmini/projects/AIMA-explicit-q8192-candidate'),**plan['component_selection_options'])
...
'''
 ...
 activation['gdn']=gdn==([dict(event='native_gdn_prefill',layer=i,tokens=8192,stages=8,aot_launches=390,chunk_tokens=64,
  original_preparation=True,ordered_integer_accumulator=True,cold=True)for i in range(40)if i%4!=3]if env['AIMA_PORT_NATIVE_GDN_PREFILL']=='1'else[])
