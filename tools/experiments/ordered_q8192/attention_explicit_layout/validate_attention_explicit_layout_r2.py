...
O = B / 'attention-explicit-layout-r2'
sha = lambda p: hashlib.file_digest(p.open('rb'), 'sha256').hexdigest()
read = lambda p: json.loads(p.read_text())
owner = read(B / 'attention-explicit-layout-r1/dispatch.json')
assert owner['returncode'] == 1 and not owner['gpu_processes_after']
if sys.argv[1:] == ['--check']:
...
sources = {
    'attention_explicit.py': B / 'attention_explicit_layout_r2.py',
...
class StripLayouts(ast.NodeTransformer):
    ...
    def visit_Subscript(self, node):
        ...
        if (isinstance(node.value, ast.Call) and isinstance(node.value.func, ast.Attribute)
                and node.value.func.attr == 'broadcast' and isinstance(node.slice, ast.Constant)
                and node.slice.value == 0):
            assert ast.unparse(node.value.args[1]) == 'probability'
            return ast.Call(func=ast.Attribute(value=ast.Name(id='tl', ctx=ast.Load()), attr='broadcast_to', ctx=ast.Load()),
                args=[node.value.args[0], ast.parse('(probability.shape[0], 32)', mode='eval').body], keywords=[])
        return node
...
manifest = dict(source_hashes={name: sha(path) for name, path in sources.items()},
    ...
    prior_compile_failure_dispatch_sha256=sha(B / 'attention-explicit-layout-r1/dispatch.json'),
    prior_compile_failure_log_sha256=sha(B / 'attention-explicit-layout-r1/container.log'),
    gluon_broadcast_api_adaptation='Pinned Gluon has broadcast(input, other); use its first result with probability as the shape peer.',
    ...
    arithmetic_ast_unchanged_after_normalizing_explicit_layouts_and_equivalent_broadcast=unchanged + ['_pair16', '_group16'],
...
remote = '/home/qujing/qrt-attention-explicit-layout-20260923-r2'
