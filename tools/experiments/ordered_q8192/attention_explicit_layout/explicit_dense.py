"""Experimental selected dense replay with an explicit K16 lane layout.
...
@gluon.jit(do_not_specialize=['N', 'K', 'W_ROW', 'W_K'])
def selected_replay_kernel(X, W, Counts, Indices, Output, Debug,
    ...
    layout: tl.constexpr = tl.BlockedLayout([1, 1], [2, 16], [4, 1], [1, 0])
    pairs: tl.constexpr = tl.SliceLayout(1, layout)
    products_layout: tl.constexpr = tl.SliceLayout(0, layout)
    offsets = tl.arange(0, BM, layout=pairs)
    products = tl.arange(0, 16, layout=products_layout)
