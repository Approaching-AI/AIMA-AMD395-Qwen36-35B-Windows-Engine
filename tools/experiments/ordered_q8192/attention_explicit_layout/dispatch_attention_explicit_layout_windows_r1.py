"""Run paired full-q8192 attention trials after the current AMD owner is clean."""
...
D = B / 'attention-explicit-layout-windows-r1'
assert socket.gethostname().split('.')[0].lower() != 'baiying'
read = lambda p: json.loads(p.read_text(encoding='utf-8-sig'))
sha = lambda p: hashlib.sha256(p.read_bytes()).hexdigest()
assert sha(D / 'manifest.json') == 'c9c6166d37626ff9dada5da4d3bea09dba8da8ebe5a6a348de885084c8175a00'
...
spec = dict(executable=m['python_executable'], arguments=[remote + '/worker.py', remote],
    ...
    purpose='Full q8192 attention: original unsigned32 baseline versus explicit layouts; two passes in reversed order; all QK, probability, scales, context and partial queue controls',
...
assert len(report['records']) == 512 and len(report['passes']) == 8 and len(report['selected_queue_controls']) == 4
