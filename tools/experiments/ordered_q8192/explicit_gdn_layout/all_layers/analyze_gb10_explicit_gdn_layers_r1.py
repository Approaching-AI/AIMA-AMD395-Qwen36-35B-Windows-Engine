...
for file in ('ordered_pipeline.py', 'group16.py', 'exp2.py'):
    original = subprocess.check_output(['git', '-C', str(P), 'show', base + ':native/providers/gdn_explicit_layout/' + file], timeout=15)
...
for file in ('ordered_pipeline.py', 'ordered_inverse.py', 'integer_u.py'):
    original = subprocess.check_output(['git', '-C', str(P), 'show', 'e15c6cd1a0e91243ebd330fa55ac24348603e151:native/providers/gdn/' + file], timeout=15)
...
for row in rows:
    ...
    assert [v['variant'] for v in row['variants']] == ['u64', 'explicit_layout']
...
assert counts == dict(core_bf16_values=2013265920, final_state_fp32_values=31457280, cumsum_fp32_values=7864320)
old = read(B / 'qrt-gb10-q8192-first64-20260922-r1/capture/q8192-out512.json')
result = dict(kind='original_q8192_all_linear_layers_explicit_layout_gdn_comparison',
    ...
    layers=30, chunks_per_layer=128, variants=['u64', 'explicit_layout'], compared_values=counts,
