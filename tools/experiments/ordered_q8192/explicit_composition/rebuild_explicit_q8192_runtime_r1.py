...
G=Path('/Users/jiawei-macmini/projects/AIMA-explicit-q8192-candidate')
assert socket.gethostname().split('.')[0].lower()!='baiying'
D=B/'qrt-explicit-q8192-runtime-rebuild-20260923-r1';D.mkdir()
sha=lambda p:hashlib.file_digest(p.open('rb'),'sha256').hexdigest()
files=['tools/compile_linux_core_q8192_explicit.py',
       *['native/providers/explicit_q8192/'+f for f in ('attention_explicit.py','explicit_dense.py','routed_explicit.py','explicit_group16.py','explicit_exp2.py','pack_value.py')]]
...
with tarfile.open(p.with_suffix('.tar'))as tar:
 members=tar.getmembers();assert len(members)==8 and all(m.isfile()and not Path(m.name).is_absolute()and '..'not in Path(m.name).parts for m in members)
...
command=['docker','run','--name',name,'--hostname','aitopatom-66c4','--network','none','--memory','3g','--cpus','2','--pids-limit','256',
 ...
 '--entrypoint','timeout','sha256:822f5c399ce6a08c0583302e0d82ff3938158b73977b6e1fbdf00ed72735a30d','--signal=TERM','--kill-after=5','240',
 'python3','/work/project/tools/compile_linux_core_q8192_explicit.py','--out','/work/aot']
start=time.monotonic();reason='completed'
with(p/'stdout.log').open('xb')as out,(p/'stderr.log').open('xb')as err:
 process=subprocess.Popen(command,stdout=out,stderr=err)
 try:process.wait(timeout=260)
...
r=subprocess.run(cmd,input=driver,capture_output=True,text=True,timeout=300)
