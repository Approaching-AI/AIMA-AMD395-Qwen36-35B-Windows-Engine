...
def persistent_kernel(Q, K, W, U, G, Scores, Table, Initial, Final, Out,
    ...
    state_k = gl.arange(0, 128, layout=gl.SliceLayout(0, right_layout))
    state_offsets = (head * 128 + state_v[:, None]) * 128 + state_k[None, :]
    initial = gl.load(Initial + state_offsets)
    state = gl.allocate_shared_memory(gl.float32, [8, 128], shared, initial)
    ...
    for first in range(0, T, 64):
        ...
        if CAPTURE:
            checkpoint = state.load(right_layout).to(gl.bfloat16)
            checkpoint_offsets = ((first // 64 * 32 + head) * 128 + state_v[:, None]) * 128 + state_k[None, :]
            gl.store(Checkpoints + checkpoint_offsets, checkpoint)
        ...
        for row_group in range(0, 8):
            ...
            for start in gl.static_range(0, 128, 16):
                ...
                right = state.slice(start, 16, 1).load(right_layout).to(gl.bfloat16)
            ...
            v_new.reshape([8, 8, 8]).index(row_group).store(r.to(gl.bfloat16))
            residual.reshape([8, 8, 8]).index(row_group).store(scaled.to(gl.bfloat16))
            if CAPTURE:
        ...
        for column_group in range(0, 8):
            ...
            view = state.reshape([8, 8, 16]).permute([1, 0, 2]).index(column_group)
            old_state = view.load(result_layout)
            accumulator = _zero(8, 16)
            for start in gl.static_range(0, 64, 16):
                left = residual.slice(start, 16, 0).permute([1, 0]).load(left_layout)
                source = first + start + str_
                right = gl.load(K + (source[None, :] * 16 + head // 2) * 128 + column_r[:, None],
                                source[None, :] < T, other=0)
                accumulator = _group16(left, right, accumulator)
            view.store(gl.fma(old_state, decay, accumulator))
        gl.thread_barrier()
    gl.store(Final + state_offsets, state.load(right_layout))
