...
D = B/'native-gdn-compact-windows-r1'; D.mkdir()
...
source = R/'tools/experiments/ordered_q8192/persistent_gdn/compact/persistent.py'
assert subprocess.check_output(['git','-C',str(R),'show',commit+':tools/experiments/ordered_q8192/persistent_gdn/compact/persistent.py'],timeout=15) == source.read_bytes()
...
m.update(candidate_source_commit=commit,worker_sha256=sha(D/'worker.py'),images=images,cases=cases,
    ...
    active_owner_local_record='native-gdn-persistent-windows-r1/run/run-record.json',
    active_owner_remote_record='D:/projects/native-gdn-persistent-20260923-r1/run/run-record.json',
    remote_directory='D:/projects/native-gdn-compact-20260923-r1',native_timeout_seconds=600,
...
dispatcher=dispatcher.replace("D=B/'native-gdn-layout-controls-windows-r2'","D=B/'native-gdn-compact-windows-r1'")
...
compile(dispatcher,'dispatcher','exec');(B/'dispatch_native_gdn_compact_r1.py').write_text(dispatcher)
