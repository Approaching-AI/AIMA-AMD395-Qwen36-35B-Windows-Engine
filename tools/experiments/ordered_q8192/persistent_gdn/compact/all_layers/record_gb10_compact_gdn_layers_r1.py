...
abi=read(B/'native-gdn-compact-abi-controls-r1.json')
...
files={archive/'all_layers'/file:B/file for file in (
    'prepare_gb10_compact_gdn_layers_r1.py','dispatch_gb10_compact_gdn_layers_r1.py',
    'run-'+name+'.py','analyze_gb10_compact_gdn_layers_r1.py','record_gb10_compact_gdn_layers_r1.py')}
files.update({archive/'all_layers'/'observer.py':D/'scripts/capture_ordered_gdn_layers.py',
    ...
    archive/'native'/'prepare_native_gdn_compact_r1.py':B/'prepare_native_gdn_compact_r1.py',
    archive/'native'/'dispatch_native_gdn_compact_r1.py':B/'dispatch_native_gdn_compact_r1.py',
    archive/'native'/'check_native_gdn_compact_abi_r1.py':B/'check_native_gdn_compact_abi_r1.py'})
repro={}
...
report=dict(schema=1,classification='original_model_all_layer_compact_persistent_gdn_cuda_comparison',
    ...
    limitations=['CUDA comparisons preserve original model outputs and never inject candidate outputs.',
        'Actual gfx1151 stability and timing remain pending; the compact Windows runtime is prepared only.',
...
relative=Path('benchmarks/correctness/persistent-gdn-compact-all-layers-20260923.json')
