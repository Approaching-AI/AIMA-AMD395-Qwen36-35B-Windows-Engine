...
class Pipeline:
    def __init__(self):
        ...
        self.variants = {}
        previous = {key: sys.modules.get(key) for key in ('integer_u', 'group16', 'exp2', 'ordered_pipeline')}
        try:
            ...
            self.variants['persistent'] = (pipeline, pipeline.integer_u_kernel)
...
class OrderedGdnCapture(TokenMatrixCapture):
    def qrt_arm_token_matrix(self, directory, prompt_tokens):
        ...
        result['ordered_gdn'] = dict(layer_container=name, layers=LAYERS, tokens=8192,
            variants=['persistent'], source_inputs_sha256=sha(ROOT / 'source-inputs.json'),
