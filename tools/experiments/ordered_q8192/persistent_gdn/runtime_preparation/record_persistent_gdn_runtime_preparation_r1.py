...
out=F/'build/persistent-gdn-preparation-controls-r1';out.mkdir()
current=F/'build/persistent-gdn-prepare-r1';control=E/'build/explicit-q8192-prepare-r1'
...
norm=lambda text,repo,directory:text.replace(str(repo),'<checkout>').replace(str(directory),'<preparation>')
...
helpers=('prepare_persistent_gdn_runtime_assets_r1.py','rebuild_persistent_gdn_runtime_r1.py',
    'compare_persistent_gdn_rebuild_r1.py','compare_gdn_explicit_rebuild_r2.py','record_persistent_gdn_runtime_preparation_r1.py')
...
report=dict(schema=1,classification='optional_persistent_gdn_runtime_prepared_native_pending',controller_host=socket.gethostname(),
    ...
    build_preparation=dict(command=['python3.12','tools/prepare_linux_core_windows.py','--out',str(current),
        '--windows-rectangular-ck','--current-text-decode','--gb10-convolution','--gb10-gdn','--gb10-projections',
