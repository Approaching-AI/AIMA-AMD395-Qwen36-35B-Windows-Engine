...
from qualify_persistent_gdn_components_r1 import COMMIT, qualify

B = Path(__file__).resolve().parent
P = B.parent.parent.parent / 'AIMA-persistent-gdn-candidate'
...
options = dict(dense_batch=args.dense_tile, routed_batch=args.routed_tile,
               attention=not args.without_attention)
...
repo = 'P:/projects/AIMA-public-persistent-gdn-20260923-r1'
plan = json.loads(json.dumps(old).replace(old['repo'], repo).replace('linux-core-batch-replay', 'persistent-gdn'))
...
assert len(plan['build_source_inputs']) == 441
compiled = read(P / 'native/linux_core_port/explicit_q8192_compile.json')
gdn = read(P / 'native/linux_core_port/gb10_gdn_persistent_compile.json')
generator_paths = sorted({*compiled['source_files'], *gdn['source_files'],
    'tools/compile_linux_core_gdn_persistent.py', 'tools/compile_linux_core_q8192_explicit.py'})
...
plan['local_gdn_host_contract_sha256'] = sha(P / 'build/persistent-gdn-host-r1/result.json')
plan['local_prefill_projection_host_contract_sha256'] = sha(P / 'build/persistent-prefill-host-r1/result.json')
plan['local_ordered_attention_host_contract_sha256'] = sha(P.parent / 'AIMA-explicit-q8192-candidate/build/explicit-attention-host-r1/result.json')
plan['local_gdn_preparation_sha256'] = sha(P / 'build/persistent-gdn-prepare-r2/prepare.json')
plan['local_prefill_projection_preparation_sha256'] = plan['local_gdn_preparation_sha256']
plan.pop('ordered_gdn_trial')
plan['persistent_gdn_trial'] = dict(candidate_commit=COMMIT,
    compiled_manifest_sha256=sha(P / 'native/linux_core_port/explicit_q8192_compile.json'),
    preparation_sha256=sha(P / 'native/linux_core_port/persistent_gdn_preparation.json'),
    gdn_compiled_manifest_sha256=sha(P / 'native/linux_core_port/gb10_gdn_persistent_compile.json'),
    q8192_embedded_images=13, q8192_image_bytes=521376, gdn_images=6, gdn_image_bytes=1545744, gdn_aot_launches_per_layer=7,
...
prior_root = B / 'native-gdn-persistent-windows-r1'
remote_root = read(prior_root / 'manifest.json')['remote_directory']
family = 'persistent_gdn'
...
manifest = B / 'linux-core-windows-persistent-gdn-source-r1.json'
with manifest.open('x') as file:
    json.dump(plan, file, indent=2)
    file.write('\n')
dispatcher = B / 'dispatch_persistent_gdn_product_r1.py'
compile(dispatcher.read_bytes(), str(dispatcher), 'exec')
bundle = B / 'linux-core-windows-persistent-gdn-source-r1.bundle'
...
stage = dict(source_commit=COMMIT, seed_repo=old['repo'], manifest=manifest.name, manifest_sha256=sha(manifest),
    ...
    qualifier_sha256=sha(B / 'qualify_persistent_gdn_components_r1.py'),
    other_qualifier_sha256=sha(B / 'qualify_explicit_q8192_components_r1.py'),
    stage_generator_sha256=sha(Path(__file__)), inventory_helper_sha256=sha(B / 'linux_core_build_inventory_r1.py'))
with (B / 'linux-core-windows-persistent-gdn-stage-r1.json').open('x') as file:
