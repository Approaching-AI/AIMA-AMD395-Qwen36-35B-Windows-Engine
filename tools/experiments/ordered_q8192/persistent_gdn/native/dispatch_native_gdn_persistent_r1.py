...
B=Path(__file__).resolve().parent;D=B/'native-gdn-persistent-windows-r1'
...
assert sha(D/'manifest.json')=='11db30bb44542a931e243b5aa1eaa7b7100861bce943cd0745ad3ae248c2cc3b'
m=read(D/'manifest.json');assert sha(D/'worker.py')==m['worker_sha256']
inputs=list({e['file']:e for case in m['cases']for e in case['inputs'].values()}.values())
for e in inputs:assert sha(D/'inputs'/e['file'])==e['sha256']
for e in m['images'].values():assert sha(D/e['file'])==e['sha256']
...
if sys.argv[1:]==['--check']:
    print(json.dumps(dict(prepared_manifest_verified=True,active_owner_complete_and_clean=bool(ready),remote_calls=0,executed=False)));sys.exit(0)
assert not sys.argv[1:]
assert ready,'The preceding native component has no completed, clean host record; zero remote calls.'
...
setup=admission+'if(Test-Path '+quote(remote)+"){throw 'Component directory exists'};$null=New-Item -ItemType Directory -Path "+quote(remote+'/inputs')+' -Force'
r=ps(setup);save('setup.json',dict(command_script=setup,**record(r)));r.check_returncode()
for local in [D/'worker.py',D/'manifest.json',*[D/e['file']for e in m['images'].values()],*[D/e['file']for e in m['source_inputs']]]:
    subprocess.run(['scp',*opts,str(local),'baiying:'+remote+'/'+local.name],capture_output=True,text=True,check=True,timeout=45)
# Reuse the hash-verified, immutable original native input directory.
spec=dict(executable=m['python_executable'],arguments=[remote+'/worker.py',remote],working_directory=m['execution_checkout'],
    repo_commit=m['execution_checkout_commit'],candidate_source_commit=m['candidate_source_commit'],source_manifest_sha256=sha(D/'manifest.json'),
    command_file=remote+'/dispatch.ps1',model='component only: original u64 upstream stages and repeated unfused/persistent/capture recurrence on first64 and full continuous q7169; timings diagnostic only',model_loaded=False,
...
script+='& '+quote(m['guard_file'])+' -SpecPath '+quote(remote+'/spec.json')+' -OutDir '+quote(remote+'/run')+' -TimeoutSeconds 600|Out-Null\n'
...
r=ps('& '+quote(remote+'/dispatch.ps1'),660);save('dispatch.json',dict(command_file=remote+'/dispatch.ps1',command_file_sha256=sha(D/'dispatch.ps1'),**record(r)))
for folder,names in [('run',['run-record.json','preflight.json','launch.json','product.stdout.jsonl','product.stderr.log','telemetry.jsonl']),('outputs',['result.json'])]:
    (D/folder).mkdir()
    for name in names:
        t=subprocess.run(['scp',*opts,'baiying:'+remote+'/'+folder+'/'+name,str(D/folder/name)],capture_output=True,text=True,timeout=40)
        t.check_returncode()
r.check_returncode();run=read(D/'run/run-record.json');report=read(D/'outputs/result.json')
assert run['host_checks_pass'] and all(run['host_checks'].values()) and not run['after_processes']
assert report['manifest_sha256']==sha(D/'manifest.json') and report['cleanup_pass']
for case in report['cases']:
    folder=D/'outputs'/case['name'];folder.mkdir()
    files=['result.json','w.bin'] if case['tokens']==64 else ['result.json']
    subprocess.run(['scp',*opts,*['baiying:'+remote+'/outputs/'+case['name']+'/'+n for n in files],str(folder)],capture_output=True,text=True,check=True,timeout=60)
    for e in case['records']:
        p=folder/e['file']
        if p.exists():assert p.stat().st_size==e['bytes'] and sha(p)==e['sha256']
print(json.dumps(dict(host=report['host'],source_commit=report['source_commit'],all_components_match=report['all_components_match'],
    native_exit_code=run['exit_code'],result_sha256=sha(D/'outputs/result.json'),run_sha256=sha(D/'run/run-record.json'),
    inference_acceptance=False)),flush=True)
