...
def extra_prefill_positions(case, prompt_tokens):
    ...
    for name, positions in plan.items():
        match = re.fullmatch(r'[a-z0-9][a-z0-9-]{0,63}-out([1-9][0-9]*)', name)
        if (match is None or not 2 <= int(match.group(1)) <= 512 or
                not isinstance(positions, list) or not 1 <= len(positions) <= 96 or
                any(type(position) is not int or not 0 <= position < 263168 for position in positions) or
                len(set(positions)) != len(positions)):
            raise ValueError('prefill positions require at most96 distinct original rows')
...
def selected_prefill_moe_observation(prompt_tokens, first_position, token_count):
    """Allow existing selected-row hooks on one actual bounded prefill chunk."""
    value = os.environ.get('QRT_GB10_PREFILL_MOE_SELECTED_ROWS', '0')
    if value not in ('0', '1'):
        raise ValueError('selected prefill MoE observation requires 0 or 1')
    return (value == '1' and
            all(type(v) is int for v in (prompt_tokens, first_position, token_count)) and
            1 <= prompt_tokens <= 263168 and 0 <= first_position < prompt_tokens and
...
def product_prefill_operands(case, prompt_tokens):
    """Observe the original q8192 matrix case without replacing its inputs."""
    value = os.environ.get('QRT_GB10_FULL_PREFILL_PRODUCT_OPERANDS', '0')
    if value not in ('0', '1'):
        raise ValueError('product prefill operands require 0 or 1')
    if value == '0':
        return False
    if (os.environ.get('QRT_GB10_FULL_PREFILL_LINEAR_WINDOWS') is not None or
            os.environ.get('QRT_GB10_FULL_PREFILL_ATTENTION_WINDOWS') is not None or
            full_prefill_linear_core_only()):
        raise ValueError('product prefill operands conflict with a separate window plan')
    if case != 'q8192-out512':
        return False
    if type(prompt_tokens) is not int or prompt_tokens != 8192:
        raise ValueError('product prefill operands require the original q8192 extent')
    return True


def full_prefill_linear_labels(layer, core_only, projection_only=False):
    if projection_only:
        if core_only:
            raise ValueError('projection and core-only observations are mutually exclusive')
        return {f'layer-{layer:02d}-input-rmsnorm', f'linear-{layer:02d}-qkv'}
...
def full_prefill_linear_window(case, prompt_tokens):
    ...
    if product_prefill_operands(case, prompt_tokens):
        return dict(layer=0, first_position=0, tokens=8192)
...
def full_prefill_attention_window(case, prompt_tokens):
    """Observe one original suffix and its complete logical KV history."""
    if product_prefill_operands(case, prompt_tokens):
        return dict(layer=3, first_position=0, tokens=8192)
    value = os.environ.get('QRT_GB10_FULL_PREFILL_ATTENTION_WINDOWS')
    if value is None:
        return None
    plans = json.loads(value)
    if not isinstance(plans, dict) or not 1 <= len(plans) <= 12:
        raise ValueError('invalid full attention prefill observation plan')
    for name, plan in plans.items():
        if (not isinstance(name, str) or re.fullmatch(r'[a-z0-9][a-z0-9-]{0,63}-out[1-9][0-9]*', name) is None or
                not isinstance(plan, dict) or set(plan) != {'layer', 'first_position', 'tokens'} or
                any(type(item) is not int for item in plan.values()) or
...
class RuntimeBoundaryCapture(TokenMatrixCapture):
    def qrt_arm_token_matrix(self, directory, prompt_tokens):
        ...
        full_linear_projection_only = product_prefill_operands(case, prompt_tokens)
        full_linear_required = (full_prefill_linear_labels(
            full_linear_window['layer'], full_linear_core_only,
            full_linear_projection_only) if full_linear_window else set())
        full_linear_labels = set()
        self._qrt_boundary_full_linear_window = (dict(plan=full_linear_window, transaction=None,
            core_only=full_linear_core_only, projection_only=full_linear_projection_only,
        ...
        def attach_moe(index):
            ...
            def moe_stage(label, value, width):
                ...
                if prefill and not (short_prefill_moe_observation(
                        prompt_tokens, transaction["first_position"], transaction["token_count"]) or
                        selected_prefill_moe_observation(
    ...
    def qrt_finish_token_matrix(self):
        ...
        for transaction in self._qrt_boundary_transactions:
            if transaction["rows"]:
                ...
                if (short_prefill_moe_observation(
                        ...
                        transaction["token_count"]) or selected_prefill_moe_observation(
                            self._qrt_boundary_prompt_tokens, transaction["first_position"],
        ...
        if window is not None:
            required = full_prefill_linear_labels(window['plan']['layer'], window['core_only'],
                                                 window.get('projection_only', False))
        ...
        attention_window = self._qrt_boundary_full_attention_window
        if attention_window is not None:
            if (attention_window['transaction'] is None or attention_window['cache'] is None or
                    set(attention_window['labels']) != self._qrt_boundary_full_labels | {'cache-k', 'cache-v'}):
                raise ValueError('selected full prefill attention transaction was not completely observed')
            boundaries['full_prefill_attention_window'] = attention_window
