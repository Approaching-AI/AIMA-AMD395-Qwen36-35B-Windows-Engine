...
foreach ($required in @(
    ...
    (Join-Path $sourceDir "..\moe_accumulator\sm121_float_alignment.h"),
...
[pscustomobject]@{
    ...
    sm121_canonical_normalize_header_sha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath (Join-Path $sourceDir '..\moe_accumulator\sm121_canonical_normalize.h')).Hash.ToLowerInvariant()
    ...
    sm121_float_subgroup_header_sha256 = (Get-FileHash -Algorithm SHA256 `
        -LiteralPath (Join-Path $sourceDir '..\moe_accumulator\sm121_float_subgroup.h')).Hash.ToLowerInvariant()
    ...
    sm121_float_alignment_header_sha256 = (Get-FileHash -Algorithm SHA256 `
        -LiteralPath (Join-Path $sourceDir '..\moe_accumulator\sm121_float_alignment.h')).Hash.ToLowerInvariant()
